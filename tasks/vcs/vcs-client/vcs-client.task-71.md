...
- **Given** GitLab возвращает 200
- **When** `resolveDiscussion({ ..., resolved: false })`
- **Then** PUT с `resolved=false`

**Scenario:** Типизация [`contract`]
- **Given** `VcsResolveDiscussionQuery`
- **Then** `project: string`, `iid: string|number`, `discussionId: string`, `resolved: boolean`

**Scenario:** 403/404 ошибки [`unit`]
- **Given** GitLab возвращает 403 или 404
- **When** `resolveDiscussion(...)`
- **Then** выбрасывается VcsError со статусом
<!--/SECTION:BDD-->
