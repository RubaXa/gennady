...
### Шаг 3.5 — TSK-169 (UI по Carbon & Steel) и TSK-170 (serve-хвосты)

Аудит 2026-08-08: TSK-167 R1 содержал over-skip (17 describe вместо 5 red) — раскатано архитектором
([TSK-168](agent-inbox.task-168.md)); дашборд TSK-164 собран НЕ по дизайн-системе (GitHub-dark вместо
Carbon & Steel, секции вместо kanban-лейнов) → [TSK-169](agent-inbox.task-169.md) до приёмки, иначе
приёмочные скриншоты зафиксируют неправильный UI. [TSK-170](agent-inbox.task-170.md) (утечка хэндлов,
orphan-restart, v1-легаси red) — параллельно, но P1 (stop/force-exit) блокирует краш-тест шага 4.
Оба тикета — для dev-агента, цикл тот же: один тикет → перепроверка архитектором → следующий.
