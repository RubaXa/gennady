...
| TSK-147 | DONE   | —                                     | utils/test: git-worktree фикстур-хелпер (настоящий git для тестов, энейблер test-hardening тира)                                                  |
| TSK-148 | DONE   | TSK-147                               | inbox-roles: детерминированный тест SV-19 commit-only-hold (закрывает дыру из D-212)                                                              |
| TSK-149 | DONE   | TSK-147                               | inbox-serve: два pre-existing флейка run-mode → зелёные (корень: устаревшие мок-сиды vs 4-полевой synthesis-gate, не git)                         |
| TSK-150 | DONE   | —                                     | inbox-serve: комбинированный full-flow black-box (VcsInboxReal+OpenCodeReal за сетью)                                                             |
...
Стабилизация сетевого black-box tier'а (D-212) и закрытие честной дыры, оставленной там же. Тикеты
**адаптивны**: зависимости кодируют обратную связь — результат/проблема одного корректирует следующий,
а батч-аудит реопенит нижележащие при дрейфе. Это не «выполнить и забыть», а самокорректирующийся flow.

| Task-ID | Что делает                                                                                | Владеет     | Адаптивная связь                                                                               |
| ------- | ----------------------------------------------------------------------------------------- | ----------- | ---------------------------------------------------------------------------------------------- |
| TSK-147 | git-worktree фикстур-хелпер `utils/test/git-fixture.ts` (реальный git, не сеть, не мок)   | utils/test  | энейблер; если git-в-тестах медленный/флейки → Handoff переводит TSK-148/149 на live-only+skip |
| TSK-148 | детерминированный SV-19 commit-only-hold в `role-scheduler.observation.test.ts`           | inbox-roles | ← TSK-147; при «git нельзя» — задокументированный `skip`, не молчание                          |
| TSK-149 | два флейка `run-mode.test.ts` → зелёные через настоящий worktree, без ослабления ассертов | inbox-serve | ← TSK-147; корень сверяется по логам до правки, не угадывается                                 |
| TSK-150 | full-flow black-box: `VcsInboxReal`+`OpenCodeReal` за undici, `OPENCODE_PORT` skip-spawn  | inbox-serve | если живой git мешает в этом пути → реопен-триггер TSK-149, git не чинится молча здесь         |

**Порядок исполнения:** TSK-147 (root) → { TSK-148, TSK-149 } (параллельно — файлы не пересекаются);
TSK-150 независим (харнесс готов), но его находки могут реопенить TSK-149. Запуск — `sdd-execute`
(по одному) или `sdd-execute-batch` (очередь со слоями + авто-аудит).

**D-116-толкование (test-infrastructure тир):** «реальность» здесь = TSK-150 гоняет РЕАЛЬНЫЕ адаптеры
(продуктовый код) через подменённую сеть; TSK-148/149 используют НАСТОЯЩИЙ git (не мок). Это не
нарушение запрета фикстур — фикстура спрятана за сетевым/git-слоем, продуктовый путь не раздваивается.

## Baseline-cleanup тир (product-readiness, 2026-07-23 — D-215)

Погашение red-baseline ветки `recover-sdd-v2` как первый шаг product-readiness (разблокирует честную
приёмку). **Триаж всех 12 красных дал ключевой результат: 0 реальных багов продукта** — всё тест-долг,
отставший от намеренных изменений продукта. Тикеты независимы (файлы непересекающиеся → Layer 0,
параллельно). Адаптивные форки: TSK-152 (base-URL: test-only vs узкая продуктовая инъекция) и TSK-154
(URL-кейс: мок-seam vs gated-integration) решаются в тикете с `decision`-логом, не молчком.

| Task-ID | Корень (из триажа)                                                             | Класс               | Файлы                               |
| ------- | ------------------------------------------------------------------------------ | ------------------- | ----------------------------------- |
| TSK-151 | TSK-131 переструктурировал `state-paths` (nested `/worktree`), 3 теста отстали | STALE-TEST          | cli/cmd/vcs-worktree                |
| TSK-152 | f0a991c убрал `format`; 240a3514 same-origin; тесты отстали                    | STALE-TEST          | inbox-api, inbox-dashboard          |
| TSK-153 | нет DI часов в `readPhaseAnalytics`; сид-форма D-120; отставший снапшот        | ENV-FLAKE + STALE   | inbox-roles, ai-kit                 |
| TSK-154 | TSK-139 заменил стаб пайплайном; sync/async skip-баг; негерметичность          | STALE-TEST + gating | cli/cmd/mr-stats, services/mr-stats |

**Порядок:** все 4 — Layer 0 (параллельно). После зелёного baseline — приёмка: TSK-123 (живой дашборд +
скрин) и TSK-117 (golden real-smoke, ручной). Запуск — `sdd-execute-batch` или параллельные фазы.
