...
- **Purpose:** `node:http` сервер на порту 4174. Роуты `/api/board`, `/api/mr/:id/assign`, `/api/mr/:id/action`, `/api/mr/:id/audit`. Статика. Все зависимости — моки.
...
- **Target Files:**
  ...
  - `services/agent-inbox/modules/inbox-api/routers/board.router.ts` — GET /api/board → состояние доски (мок)
  - `services/agent-inbox/modules/inbox-api/routers/mr.router.ts` — POST /api/mr/:id/assign, /api/mr/:id/action
