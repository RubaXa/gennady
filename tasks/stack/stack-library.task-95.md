...
| R11 | 2026-08-19 | PASS | Verdict ladder fixed (D-STACK-015, panel finding 1 — the original blocking one): `envFail` predicates are consulted right after a spawn failure, i.e. BEFORE timeout, violation and the stdout contract. Previously they were last, so rules never fired in the three shapes a broken environment actually takes — a hang (TIMEOUT with no cause and no do-not-edit-code note), tree debris (VIOLATION blaming the gate) and exit 0 with diagnostics (stdout contract). TIMEOUT now carries its own note pointing at `requires`; an env-fail over a dirty replica still lists the debris. Precedence order written into §8.2.1 as a checkable contract. 4 unit + 4 E2E fixtures (hang, hang-with-rule, mutating gate, broken-env-with-debris). 1310/1311 unit, 16/16 e2e. |

| R12 | 2026-08-19 | PASS | node (12) and config (14) fixture matrices authored — 42 fixtures total, all green under STRICT with zero skips. Caught a HOLLOW GREEN in the harness itself: the sed-generated node suite probed `[go, nodeci-lint]`, so every `requires: [npm]` was unsatisfiable and all 12 node fixtures skipped while the run reported "12 pass". Class fix: one shared `declareStackSuite` (the three suites were copy-paste divergent) that derives the probe list from the UNION of fixture `requires`, rejects unknown toolchain ids loudly, and fails an empty suite — "0 tests, 0 failures" is the same lie as a silent skip. Also applied the tsconfig fixture exclusion the spec mandated (an earlier patch had silently no-op'd; a fixture .ts was being typechecked). Two fixture bugs found by the validator working correctly: YAML parses `argv: [true]` as a boolean, and a skipped gate does not fail a run. 1310/1311 unit, tsc+lint clean, self-verify 4/4. |

| R13 | 2026-08-19 | PASS | Closed the last three panel findings. (11) A matched env-fail rule now reports its source file, so a machine-global `$HOME/.gennadyrc` rule is identifiable as personal at match time rather than only in `--plan`; proven by a fixture using the new `expect.homeRc`, which also gave the config matrix its missing third source. (12) Config regexes now see the printed head+tail window instead of unbounded output — classifying exactly what is displayed, and with `^`-anchored /m patterns an elided middle can only lose a match, so it fails open to FAIL (unit test covers head/tail/middle). (4, human half) Non-JSON `--plan` now prints each gate's `requires`, env-fail rule descriptions with their source, and its fixer — a plan that hid classification could not be reviewed before running. 1311/1312 unit, 44 e2e fixtures green under STRICT (16 golang, 12 node, 16 config), tsc+lint clean. |

| R14 | 2026-08-19 | PASS | TODO items 2–6 implemented (1 and 7 recorded as deferred maintainer actions). Golang matrix completed: 39 fixtures — baseline verdicts (vet/build/test/panic/empty-scope/generate ok+none+stale), flag coverage for `--skip`, `--root`, `--full-output` and default `changed` mode, and the config surface (skipGates with source, --only lifting a skip, CLI --skip winning, argv/env/timeout overrides, extraGates stdout contract, --plan rule rendering, drift gate, dirty-tree replication, no-HEAD env-fail). Three harness bugs and one product bug found by the new fixtures: `treeUnchanged` compared against a pristine tree so any `dirty` fixture failed by construction; the harness appended a second `--json`, which made parse-args fall back to the human report; plan runs were asserted as if they had results; and `git grep` without `--untracked` hid a directive in a new file, silently skipping the drift gate. cli/e2e fixed: `build:publish` instead of `build` (its tarball was a subset of the published package while D-015 claimed identity) plus a registry parameter and `--offline` fallback — that suite went from 18/45 with exit 1 to 45/45 exit 0 behind the corporate proxy, which also unblocks `prepublishOnly` there. 1311/1312 unit, 67 e2e fixtures green under STRICT, dogfood 4/4. |

| R15 | 2026-08-19 | PASS | TODO items 8–9 closed: the last five golang fixtures (44 total). Only `go-lint-exit2` was truly blocked on a toolchain — the other four were writable and my earlier "environment-dependent" label was wrong. Two product bugs found by writing them: (a) the module-fetch predicate was anchored to `^go: `, so `go build -mod=mod` reporting «module lookup disabled by GOPROXY» on a `main.go:3:8:` line read as a CODE failure — added a deliberately narrow second pattern for the file-position form, keeping `no required module provides` and `missing go.sum entry` as FAIL because an agent fixes those in the repo (§5.2); (b) `gennady fix` printed `ENV-FAIL` while `verify` prints `ENV_FAIL` — one spelling now, since agents match on it. CI gained pinned golangci-lint v2.12.2, required by `go-lint-exit2`: the fixture-derived probe list made adding the fixture force the CI change rather than allowing a silent skip. 1313/1314 unit, 72 e2e fixtures green under STRICT (44 golang, 12 node, 16 config). |

| R16 | 2026-08-19 | PASS | `config` promoted from a `stack` module to a peer scope (`specs/config/config.spec.md`): per-repo config is a general mechanism, and the next consumer adds a top-level section without touching it. Per the operator's rule «peers are visible to each other» the spec moved WHOLE — it still documents the `stack` section schema (GateSpec/CmdSpec/EnvFailRule), because a config author reads one document; the alternative (machinery here, section schema at each consumer) was rejected for scattering one YAML across specs. Added §3.1.1 top-level section registry. Code split along the reusable line: `services/config/config-loader.ts` owns discovery of the three sources, deep-merge, per-key provenance, duration grammar and did-you-mean, and is section-parameterized (`loadConfigSection(root, name)`); `services/stack/stack-config.ts` keeps the `stack` schema, its validation and `applyStackConfig`. 5 new loader tests; every spec link verified resolvable. 1318/1319 unit, 72 e2e green, dogfood 4/4. |
