...
### 7.1.1 Найдено при написании фикстур: `git grep` и новые директивы

В режиме `all` наличие `//go:generate` искалось `git grep`, который читает **отслеживаемое** содержимое; новый файл с директивой не находился, и drift-гейт молча скипался — ровно на той кодогенерации, которую вносит текущее изменение. Добавлен флаг `--untracked`. Под предусловием чистого дерева (stack.spec D-STACK-017) новые файлы приходят коммитами, так что сценарий сузился до «свежезакоммиченная директива с отсутствующим выводом» — гейт обязан дать FAIL, а не skip; покрыто фикстурой `go-generate-new-directive`.
