...
Прочитай `golden-chat-output.example.md` ещё раз и сверь:

| # | Секция | Есть? |
|---|--------|-------|
| 0 | Golden Example прочитан, сверена СТРУКТУРА (не формат диаграмм) | □ |
| 0b | Транспорт объявлен (action 0); виджет использован, если был доступен | □ |
| 0c | Существующее обсуждение подгружено (review-issues) или явно сказано «тредов нет» | □ |
| 1 | Шапка + ссылка на MR | □ |
| 2 | Scope-итог + таблица категорий + карта файлов | □ |
| 3 | C4-диаграмма (виджет при наличии, иначе ASCII) | □ |
| 4 | Разбор сущностей [E1]..[EN] с вердиктами | □ |
| 5 | Таблица кандидатов (ID/Файл/Строка/Проблема/Ось/Kind) | □ |
| 6 | Подвал + ссылка | □ |
