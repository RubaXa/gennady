...
4. **`ai/directives/agent-inbox/visual-vocabulary.directive.xml`**
   - `<DiagramSelection>` — какую диаграмму под какое намерение (не всё flowchart'ом)
   - `<Syntax>` — скелеты ASCII (чат) и Mermaid (GitLab) для каждого типа ядра
   - `AX_INFOGRAPHIC_FIRST` — реляционное картинкой, не прозой; риск = недопредъявленные требования
...
1. Шапка — markdown-ссылка `[ref](webUrl)` (кликабельная, не голый текст/URL) + title
2. **Карта изменения ПЕРВОЙ** (`AX_CHANGE_MAP`): граф сущностей (цвет по типу изменения + файлы подписью + серые соседи + строка «что произошло»). Даже на однострочный MR — минимальная карта. Ниже: строка размера + таблица категорий + вторичная карта файлов. Диаграмма в `README.md` — Mermaid (её потребует валидатор).
3. Крупный MR → отдельный C4; малый/средний → карта из п.2 его заменяет (не дублировать)
...
9. Подвал — повтор markdown-ссылки `[ref](webUrl)`

Допиши запись в `HISTORY.md` (append-only: дата, `headSha`, режим full/delta, итог, ссылки на
task-файлы этого визита).

**Гейт перед выводом в чат:** `inbox-review-plan --validate <dir>` (filled) — он теперь требует
в `README.md` ≥1 Mermaid-диаграммы (секция Архитектура непуста). `{ok:false}` с «no diagram» →
диаграмма забыта, вернись и нарисуй, НЕ выводи в чат. Диаграмму нельзя пропустить — это гейт кода.

**Чат — проекция проверенного `README.md`, не сочинение заново.** После `{ok:true}` выведи в чат
то, что в README: карту изменения + карту файлов + вердикты, конвертируя Mermaid → виджет (при
наличии `mcp__visualize__show_widget`) или ASCII (fallback, транспорт по action 0). Диаграмма из
README ОБЯЗАНА попасть в чат — раз валидатор подтвердил её в документе, в чате её пропуск = брак.
Сырые md/`mermaid`-блоки в чат не вставлять (чат их не рендерит). Ссылки — markdown `[…](…)`
(инвариант 6). Файлы — для GitLab/оркестрации, чат — для оператора (правило 8).
...
| — | `--validate` (filled) прошёл: в README есть ≥1 Mermaid-диаграмма (нет «no diagram») — диаграмму нельзя забыть | □ |
| — | Карта изменения (`AX_CHANGE_MAP`) — ПЕРВЫЙ визуал; есть хотя бы одна диаграмма даже на однострочный MR | □ |
| — | Чат — проекция проверенного README (карта + диаграмма спроецированы в виджет/ASCII), не сочинён заново; диаграмма из README попала в чат | □ |
| — | Шапка и подвал чат-вывода — markdown-ссылка `[ref](webUrl)` (кликабельная, не голый текст/URL); пути артефактов тоже `[…](…)` (инвариант 6) | □ |
| — | Реляционное отрисовано медиумом по action 0 — виджет при наличии `mcp__visualize__show_widget`, ASCII только fallback (инвариант 2) | □ |
