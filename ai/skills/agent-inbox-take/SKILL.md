...
**Итог оператору — в чат**, по-прежнему по golden-формату (виджет при наличии, иначе ASCII —
транспорт по action 0; инвариант 2): реляционное — картинкой, не прозой. Ссылки на MR и пути
артефактов — кликабельным markdown `[…](…)` (инвариант 6). Содержимое md-файлов и mermaid **в чат
не вставлять** — файлы для GitLab/оркестрации, чат для оператора (правило 8 `agent-inbox/SKILL.md`).
