...
| `check.sh`           | Deterministic mechanical checks — [TASKID], [TRACKER_SYNC], [RULES], [LOG], and [HEADERS] (`--files`). Single source shared by `sdd-check` and `sdd-audit`. Exit 0 clean / 3 findings / 2 structural / 4 bad-invocation. `findings=0` is only ever printed after something was checked: a malformed Task-ID exits 4, and an empty scope (`NO_TICKETS_FOUND`) or an unparseable ticket Meta Task-ID (`TICKET_ID_UNREADABLE`) exit 2. |
