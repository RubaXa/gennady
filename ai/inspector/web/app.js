...
const KIND = {
  skill: ['k-skill', 'skill'], section: ['k-tag', 'tag'], step: ['k-step', 'step'],
  tool: ['k-tool', 'cmd'], read: ['k-read', 'read'], run: ['k-run', 'run'],
  directive: ['k-run', 'directive'], axiom: ['k-axiom', 'axiom'], halt: ['k-halt', 'halt'],
  switch: ['k-switch', 'switch'], branch: ['k-switch', 'branch'], text: ['k-tag', 'tag'],
...
let REPO_ROOT = '';
let EDITOR = (typeof localStorage !== 'undefined' && localStorage.getItem('inspector.editor')) || 'vscode';
...
function renderNode(node, depth) {
  ...
  if (node.note) { lab.appendChild(document.createTextNode(' ')); const n = el('span', 'note'); n.innerHTML = '— ' + renderInline(node.note); lab.appendChild(n); }
  row.appendChild(lab);
  ...
  if (hasKids) {
    ...
    if (open) chev.classList.add('open'); else kids.classList.add('collapsed');
    wrap.appendChild(kids);
    row.classList.add('clk');
    row.addEventListener('click', () => { kids.classList.toggle('collapsed'); chev.classList.toggle('open'); });
  }
  return wrap;
}
...
function runsOutsideSwitch(node) {
  ...
  (function walk(n) {
    for (const c of n.children || []) {
      if (c.kind === 'run') { out.push(c); continue; }
...
function stepMd(unit) {
  ...
  if (action && (action.detail || action.note)) parts.push('**Действие:** ' + (action.detail || action.note));
  return parts.join('\n\n');
}
...
function transOf(unit) {
  if (unit.kind === 'switch') {
    return (unit.children || []).map((br, i) => ({ type: 'branch', i, label: br.label, run: (br.children || []).find((c) => c.kind === 'run') }));
  }
...
function simulate(skill, moves) {
    ...
    for (let g = 0; g < 64; g++) {
      ...
      if (t.node.kind === 'skill' && unit && unit.attrs && unit.attrs.id === 'EMBODY') {
        ...
        if (main) {
          ...
          push({ div: 'загружена ' + main.dir.label + ' — раскрыть ниже, чтобы прочитать (' + base(main.run.ref) + ')', dir: main.dir });
          continue;
        }
      }
      break;
  ...
  for (const m of moves) {
    ...
    if (m.type === 'next') {
      ...
      if (tr) {
        ...
        if (dir) { stack.push({ node: dir, u: unitsOf(dir), idx: 0, resume: t.idx }); push({ div: 'загружена ' + dir.label + ' — раскрыть ниже, чтобы прочитать (' + base(tr.run.ref) + ')', dir }); }
        else t.idx++;
      } else t.idx++;
      ...
      if (tr) {
        ...
        if (dir) { stack.push({ node: dir, u: unitsOf(dir), idx: 0, resume: t.idx }); push({ div: 'ветка → загружена ' + dir.label + ' — раскрыть ниже, чтобы прочитать', dir }); }
        else { push({ div: 'ветка: ' + tr.label }); t.idx++; }
      } else t.idx++;
    }
    autoEnter();
  }
  ...
  return { log, stack, current: unit ? { unit, transitions: transitionsFor(t, unit), depth: stack.length - 1 } : null, done: !unit && stack.length === 1 };
}
...
function renderDebug(container, skill) {
  ...
  function draw() {
    ...
    sim.log.forEach((e) => {
      ...
      const extra = [];
      const tools = toolsOf(e.step).map((t) => t.label).join(', ');
    ...
    if (sim.done) {
      ...
      card.appendChild(hd);
      if (unit.kind === 'switch') {
        if (unit.note) { const n = el('div', 'dbg-note'); n.innerHTML = renderInline(unit.note); card.appendChild(n); }
      } else {
        const md = stepMd(unit);
        if (md) { const d = el('div', 'md'); d.innerHTML = renderMarkdown(md); card.appendChild(d); }
        const tools = toolsOf(unit);
        if (tools.length) card.appendChild(el('div', 'dbg-mut', 'выполняет: ' + tools.map((t) => t.label).join(', ')));
        gather(unit, 'unparsed').forEach((u) => card.appendChild(el('div', 'dbg-mut', '⚠ ' + u.label + (u.note ? ' — ' + u.note : ''))));
      ...
      transitions.forEach((t) => {
        ...
        if (t.type === 'branch') {
          ...
          btn.appendChild(el('span', 'cond', t.label === 'DEFAULT' ? 'DEFAULT — иначе' : 'когда ' + t.label));
          btn.appendChild(el('span', 'to', run ? '→ ' + base(run.ref) + ' ↘' : (txt ? txt.label : '→ дальше')));
        } else if (t.type === 'into') {
        ...
        btn.addEventListener('click', () => { moves.push(t.type === 'next' ? { type: 'next' } : { type: t.type, i: t.i }); draw(); });
        tr.appendChild(btn);
      });
      host.appendChild(tr);
    ...
    back.addEventListener('click', () => { moves.pop(); draw(); });
    const reset = el('span', 'dbg-ctl', '⟲ сброс');
    reset.addEventListener('click', () => { moves = []; draw(); });
...
async function main() {
  ...
  const select = (i) => {
    ...
    struct.appendChild(renderNode(data.skills[i], 0));
  ...
  if (ed) {
    ed.value = EDITOR;
    ed.addEventListener('change', () => { EDITOR = ed.value; localStorage.setItem('inspector.editor', EDITOR); select(cur); });
  }
