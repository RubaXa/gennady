...
## Ограничение текущего RC-расследования

После стоп-кадра оператора новые прогоны запрещены. Следующий authoring-прогон разрешён только после
отдельного согласования, завершения правки и зелёных локальных проверок. Его результат завершает
итерацию: `pass` — зафиксировать и отчитаться; `fail` — ничего больше не исправлять и вернуть evidence.
