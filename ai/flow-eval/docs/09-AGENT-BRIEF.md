...
- [ ] Код выхода батча (`cli.ts`) считается только по `worker-error` и провалу детерминированных гейтов
      (R1/R-COMPLETE/MIGRATION); judge-вердикт — диагностика и на код выхода не влияет (E-21).
