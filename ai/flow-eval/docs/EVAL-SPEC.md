...
| `node --import tsx ai/flow-eval/scripts/sandbox.ts dependencies [--dry] [--clean] --root DIR` | read-only report по умолчанию; `--clean` удаляет inactive после lock-protected recheck            | active leases не удаляются; corrupted evidence остаётся quarantined для operator investigation                                                                  |
...
| `dependency-store.ts`  | Проверяет root/installed lock, allowlist, Node ABI, platform и arch; создаёт один content-addressed symlink store без install/copy fallback. Lease capability содержит exact path+owner token; heartbeat/release дополнительно сверяют host/PID/token. Heartbeat каждые 30с; stale >2мин или same-host dead PID очищается до active-cap/retention. Старый `{leaseId,createdAt}` lease получает bounded 2-минутное compatibility-окно, затем очищается как orphan. Все scan→identity recheck→delete проходят под store lock; identity race защищает весь store на текущий GC pass. Fresh incomplete lock fail-closed, старше 1мин восстанавливается; валидный foreign lock по возрасту не снимается. Corrupted lease quarantined и блокирует reuse fail-closed. Неактивные stores ограничены 2 каталогами и 7 днями. |
