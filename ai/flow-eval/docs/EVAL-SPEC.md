...
гейтит код выхода, см. таблицу «Что детерминировано» ниже), а исчерпанный бюджет наблюдений
автоматически даёт `fail` независимо от качества работы. Известный дефект: `R1` даёт FAIL на
...
| `python3 ai/flow-eval/scripts/session-metrics.py record\|gate\|compare` | детерминированные метрики сессии и completion-гейт                                                | `.results/metrics-ledger.jsonl`                                                                                                                            |
...
| `trajectory.ts`        | Опционально (`scenario.checkpoints`): модель траектории + эмиссия `.sdd-eval-trajectory.<id>.json` (tool-события + checkpoint-вердикты) из `cli.ts`, сразу после judge. Матчеры для `*.trajectory.test.ts` — в `__tests__/trajectory-assert.ts`. Подробности — «Траектория» ниже.                                                        |
...
- **Эксперимент над гипотезой о флоу** («если поменять X — станет лучше?»): одно изменение за прогон,
  ...
  исхода — удачный и неудачный — фиксируются: числа прогона в `.results/metrics-ledger.jsonl`, гипотеза
