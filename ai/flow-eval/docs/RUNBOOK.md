...
`maxObservations >= 1` целое, `runner.ts`), но исчерпанный observation или wall-clock budget
автоматически даёт `outcome: 'budget-exhausted'`, а не `fail`/`worker-error`. Такой прогон виден
отдельно, не красит CI и не входит в pass/fail-статистику — заниженный бюджет портит диагностику, а
не только экономит.
