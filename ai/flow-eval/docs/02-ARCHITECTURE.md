...
| `trajectory.ts`        | Модель траектории + эмиссия `trajectory.json` (tool-события + checkpoint-вердикты); крутится в `cli`. Матчеры для `*.trajectory.test.ts` — в `__tests__/trajectory-assert.ts`. Как писать — [06-NEW-EVAL.md §6](./06-NEW-EVAL.md#6-траектория--детерминированный-тест-пути-агента-опционально). |
...
| `trajectory.ts` — checkpoints  | Да          | Exit-код фиксированной CLI-команды на файлах песочницы (без LLM) → `green`; путь ассертит `*.trajectory.test.ts` офлайн ([06-NEW-EVAL.md §6](./06-NEW-EVAL.md#6-траектория--детерминированный-тест-пути-агента-опционально)). |
