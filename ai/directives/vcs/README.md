...
- [`git.xml`](git.xml) — git policy: gitignore baseline (incl. `.claude/settings.local.json`), branch strategy, commit conventions (Conventional Commits default), history hygiene, semver tags, hooks integration, LFS/submodules, repo bootstrap, secrets discipline. **Mandatory** — infrastructure scope discovery halts without it (per `AX_VCS_CATEGORY_MANDATORY`).
