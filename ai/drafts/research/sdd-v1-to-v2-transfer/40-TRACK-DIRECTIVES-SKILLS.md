...
### 3.7 Вердикт-матрица: 8 инвариантов v1 без вердикта (P-01)

**Источник предмета.** `05-SUMMARY.md` §5 («Два собственных пробела сводки») и `10-MAIN-DELTA.md` §3 (инвентарь инвариантов `C1–C20`/`S1–S10`/`R1–R6`/`D1–D10`, курсив «*добавлено V-A1*») — восемь строк, добавленных верификатором `V-A1` сверх исходного инвентаря `A1`, ни разу не получивших вердикт «перенесено / осталось в v1-скилле / не переносится». Ранее — вопрос `O-8` (`00-INDEX.md` §4), переведён в задачу плана без открытого решения (05 §5).

**Метод.** Для каждого инварианта: main `file:line` — сверен `git show d37d5910:<path>` (main заморожен на этом коммите, `00-INDEX.md` шапка, `05:20`); RC `file:line` — сверен по чекауту `scratchpad/rc-w2` (только чтение; на диске ветка `eval`, но `ai/kit/**` и вся кодовая база соответствуют голове RC (`codex/sdd-v2-rc52-followup`) + PR #28 — состояние совпадает с чекаутами, которыми пользуются остальные разделы этого трека, см. `20:20`). Вердикт — один из трёх: **аксиома** (инвариант перенесён в v2, обнаружен код/тест по обеим сторонам), **не переносится** (в v2 либо архитектурно заменён другим механизмом, либо отсутствует и не запланирован ни одной задачей доски), **нужен разбор** (частичный перенос или множественный предмет, решение о заведении новых задач — за оператором/Lead, не решается этой строкой).

| # | Инвариант (кратко) | main `file:line` (`d37d5910`) | RC `file:line` (`rc-w2`) | Вердикт |
|---|---|---|---|---|
| **C16** | Канонический предикат «это rule-файл»: один regex `SDD_RULE_PATH_RE` на пять cascade-категорий, единственный источник истины для `[RULES]`/`[TASKID]` | `ai/skills/sdd-execute/scripts/_sdd-lib.sh:12-19` (`sdd_lib_is_rule_path`) | не найден (`grep -rn 'isRulePath\|SDD_RULE_PATH\|is_rule_path' shared/ cli/` → 0) | **Не переносится** (по построению). Функция замещена другой архитектурой: v2 резолвит правило по явной ссылке `Rules:`-буллета + `<DependsOn>` (`shared/sdd/rules-cascade.ts`, см. R6 ниже), а не по regex-классификации каталога — классифицировать «это ли rule-файл» отдельно не нужно, любой файл, на который сослались, уже проверяется существованием и содержимым. Предмета для замка нет, как у issue `#9.3` (`20:119-123`) |
| **C17** | Обнаружение тикетов в `scan.sh` — контентное (`grep -l 'Task-ID:'` по любому `*.md`), а не по имени файла | `ai/skills/sdd-execute/scripts/scan.sh:226-230` | `shared/sdd/ticket.ts:128` (`metaBody.match(/\*\*Task-ID:\*\*\s*.../)`) | **Аксиома** (перенесена). Другая грамматика id (`<ACR>-<slug>` вместо `TSK-...`, см. `20 §#9.1`), тот же принцип: тикет ищется по содержимому META, не по имени файла |
| **C18** | `extract-section.sh` код выхода 5 `ANCHOR_EMPTY` — «маркеры на месте, payload секции пуст» | `ai/skills/sdd-execute/scripts/extract-section.sh:203,216` | `cli/cmd/sdd-extract/sdd-extract.types.ts:24` (`ERR_CLI_SDD_EXTRACT_ANCHOR_EMPTY`), `:53` (докстринг «`exitCode` mirrors extract-section.sh»), `:174` (`exitCode: 2`) | **Аксиома** (перенесена). Код 5→2 в новой нумерации, но докстринг v2 прямо цитирует зеркалирование `extract-section.sh` — тот же контракт под другим именем |
| **C19** | `sdd check --help` — контракт секций: перечисляет весь набор находок (`[REOPENS]`, `[RULES]`, `[LOG]` вкл. post-close, `[TASKID]`, `[TRACKER_SYNC]`, `[HEADERS]`) | `ai/skills/sdd-execute/scripts/sdd` (справка), закреплено `4a47b9e8` | `cli/cmd/sdd-check/help.ts:44-56` документирует свой (другой) набор проверок (`RULES_CASCADE_CLOSURE`, `BDD_COVERAGE`, `BDD_NEGATIVE`, `BDD_TRACE`, `COVERAGE_POLICY`, `PHASE_RECEIPT`…); `REOPENS`/`TRACKER_SYNC` как отдельные секции в v2 не существуют (см. `20 §#13`) | **Нужен разбор.** Буквальный предмет C19 (v1-контракт секций) не переносим — в v2 другой набор проверок. Открытый вопрос — не в этой строке решаемый: гарантирует ли что-нибудь, что `help.ts` перечисляет **все** актуальные коды `sdd-check` (полнота, а не соответствие v1)? Смежная задача **`B2-11`** («Механический шаг аудита знает ровно те коды, что есть в проверке», Пачка 4, Волна 0, `61:633`) чинит рассинхрон в `STEP_1_MECHANICAL.xml`, но не в `sdd-check --help`; отдельной задачи-владельца на полноту `help.ts` на доске нет — заводить её или нет, решает оператор/Lead при постановке, не эта строка |
| **C20** | Вход в регион Execution Log сознательно узкий: только `## 7. Execution Log`, выход — на любом `## ` (см. C7); тикет, где лог не 7-я секция, вообще не проверяется `[LOG]` | `ai/skills/sdd-execute/scripts/check.sh:544-558` | v2-тикеты: явные маркеры `<!--SECTION:EXECUTION_LOG-->`/`<!--/SECTION:EXECUTION_LOG-->` (`shared/sdd/ticket.ts:144`) — позиция секции не имеет значения, узкий-вход неприменим; legacy-тикеты (без маркеров): `shared/sdd/anchor-inject.ts:51-63` (`legacyHeaderBody`) находит секцию по имени заголовка **на любом уровне заголовка**, тоже не воспроизводит v1-узость дословно | **Не переносится буквально** (архитектура другая — маркеры вместо порядковых заголовков делают саму дыру C20 неприменимой к v2-формату). Смежная, но не идентичная дыра для **legacy**-тикетов — `nextRoundNumber` считает раунды по всему файлу, а не по секции лога (`cli/cmd/sdd-log/sdd-log.types.ts:76-79`, `shared/sdd/group-receipt.ts:64-68`) — уже задача плана **`B2-02`** (Пачка 14, Волна 3, `61:625`; issue `#15`, `20:250-252`), эта строка нового предмета не добавляет |
| **S10** | `SyncDeps` меняет сигнатуру удаления с `rmdir?: (path, {recursive})` на `rm?: (path, {recursive, force})` (Node 20+) | `shared/common/sync/sync-deps.type.ts:62` (`rm?: (path, options?: {recursive: boolean; force: boolean})`) | `shared/common/sync/sync-deps.type.ts` (поле `rmdir?: (path, options?: {recursive: boolean})` — без `force`); фактическое удаление — `cli/cmd/sync-skills/sync-skills.cmd.ts:136` (`rmSync(path, {recursive: true, force: true})`, дефолт мимо типа) | **Не переносится** (расхождение типа и реализации). Эффект де-факто достигнут (дефолтная реализация уже вызывает `rmSync` с `force: true`), но инжектируемый контракт `SyncCmdDeps` не переименован и не несёт `force` — типовая грязь, не поведенческий регресс; вне задач текущих пачек (S), riск низкий, отдельной задачи не заводится этой строкой |
| **R6** | *(разрешает противоречие R2)* `audit.directive.xml` — `<DependsOn>` объявлен «optional and unchecked» (`:240`), но тут же требует полной транзитивной замыкания и вводит `RULES_CASCADE_MISMATCH` (`:253,264`) | `ai/directives/sdd/audit.directive.xml:240` (противоречит), `:253,264` (реализует) | `shared/sdd/rules-cascade.ts:56` (`checkRulesCascadeClosure`), код `SDD_RULES_CASCADE_UNRESOLVED` (`:78`), потребитель `cli/cmd/sdd-check/sdd-check.cmd.ts:417`; справка `cli/cmd/sdd-check/help.ts:49` (`RULES_CASCADE_CLOSURE`) | **Аксиома** (перенесена и усилена). В v2 транзитивное замыкание `<DependsOn>` — отдельный чистый модуль с собственным юнит-тестом (`shared/sdd/__tests__/rules-cascade.test.ts`), которого у main нет вовсе (main-инвариант R6 сам отмечает «нет отдельного контрактного теста») |
| **D10** | `critic-protocol.xml` v1.1→v1.2: `AX_ISOLATION` инвертирован («confusion → underspecification» ⟶ «confusion alone never proves underspecification»), новые `AX_FINDING_PROVENANCE`/`AX_CONFUSION_TRIAGE`/`AX_REVIEW_MODE`/`AX_AUTHOR_BOUNDARY`, антипаттерны `AP_COAUTHOR`/`AP_CONFUSION_AS_FINDING`/`AP_VERIFICATION_RESET`, расширенный формат finding-строки | `ai/directives/sdd/critic-protocol.xml:10,12-15` (новые аксиомы), `:100` (`AX_UNCERTAINTY_NOT_FINDING`), `:150` (`AP_CONFUSION_AS_FINDING`), `:157-158,177-178` | `ai/directives/sdd-v2/critic-protocol.directive.xml:4` (`AX_ISOLATION`, всё ещё старая формулировка «Confusion → underspecification, not a question to ask»), `:7` (`AX_UNCERTAINTY_IS_SIGNAL` — старое имя/формулировка, не `AX_UNCERTAINTY_NOT_FINDING`); ни один из `AX_FINDING_PROVENANCE`/`AX_CONFUSION_TRIAGE`/`AX_REVIEW_MODE`/`AX_AUTHOR_BOUNDARY`/`AP_COAUTHOR`/`AP_VERIFICATION_RESET` в `ai/` не найден | **Нужен разбор.** RC несёт **старую**, pre-D10 семантику критика — регресс относительно main, структурно тот же класс, что и issue `#9.4`/`#24` (ветка отошла до фикса). Задача **`T-B6-23`** (Пачка 5, Волна 0, `61:95`, `62 §Пачка 5`) чинит ровно кусок «confusion не доказывает дефект сама по себе» (её формулировка «замешательство критика само по себе больше не считается доказательством дефекта спеки» текстово совпадает с `AX_UNCERTAINTY_NOT_FINDING`). Остальные пять элементов D10 (`AX_FINDING_PROVENANCE`, `AX_CONFUSION_TRIAGE`, `AX_REVIEW_MODE`, `AX_AUTHOR_BOUNDARY`, `AP_COAUTHOR`, `AP_VERIFICATION_RESET`, расширенный формат finding-строки) **не покрыты ни одной задачей доски** — заводить ли для них новые задачи (и в какую волну) или сознательно принять частичный перенос, решает оператор/Lead при следующей постановке; эта строка фиксирует пробел, не закрывает его |

**Итог P-01.** 8/8 строк получили вердикт с доказательством `file:line` по обеим сторонам. Из них: **4 аксиомы** (C17, C18, R6 — перенесены и подтверждены кодом/тестом; R6 даже усилена), **3 не переносятся** (C16, C20, S10 — архитектурная замена или типовая грязь без поведенческого риска, новых задач не требуют), **2 нужен разбор** (C19 — открытый вопрос о полноте `help.ts`, без задачи-владельца; D10 — частичный перенос, пять из шести элементов не покрыты доской). Ни одна строка не переведена в статус «решено» без доказательства; строки «нужен разбор» **не решают** вопрос о заведении новых задач — это оставлено оператору/Lead, как того требует постановка P-01 (`61:244`).

Задача **P-01**: ВЫПОЛНЕНО (пачка 8, документная часть, 2026-09-08).
...
| **T-B6-28** | **Новая (дельта гейта адекватности, 06 §5.1 п.19).** Reconcile: механическая операция (`sdd-sync` = **tracker-status/rollup**) размечена отдельно от `agent-reviewed`; обещание `back-sync` убрано там, где механизма нет. **Уточнённое доказательство (V-06):** формулировка `sdd-sync.cmd.ts:130` («read the ticket Status and bring matching tracker rows into agreement») **уже честная**, фразы «brings … into agreement» в `reconcile.directive.hbs` больше нет — остаточное расхождение только в keywords директивы `reconcile.directive.hbs:1` (`sync-from-code`, `back-sync`) и в `AUTHORING.md:67`, где `back-sync` объявлен «сигналом». Обоснование ослаблено с «доказанный дефект v2» до **«остаточный дрейф словаря»**; размер S подтверждён | `reconcile.directive.hbs:1`, `ai/kit/AUTHORING.md:67` | grep-замок: `back-sync` не встречается как обещание механизма; `sdd-sync` описан как tracker-status/rollup во всех поверхностях | **S** | G3; критерий **A20**, координировать с `SO-14` (имена в help) |
| **T-B6-29** | **Новая (дельта гейта адекватности, 06 §5.1 п.20).** Assurance wording: `proof`/`verified`/`100%` заменены там, где механизм доказывает лишь presence/shape/linkage; введено различение `mechanically checked` / `agent-reviewed` / `test-observed` / `unknown`; **уровень доказательства по языку** для сканеров (`exact` для ts/tsx, `approximate` для остальных); зафиксирована формулировка **«golden доказывает неизменность поведения, а не его корректность»** (предупреждение R2 №11, `R2:396` — «доказана не та спецификация»). **Диаграммная половина решена D-43:** ID требований обязательны **только в подписях диаграмм требований**, Overview — **без** ID; чекер проверяет **существование** указанных ID; правило применяется к новым **и** мигрируемым спекам | `.hbs`/`axiom/**` (словарь assurance), `formats/**` (подписи диаграмм), чекер существования ID | grep-замок словаря assurance; детерм. «подпись диаграммы требований без существующего ID → finding»; «Overview без ID не даёт finding» | **S** | G3; критерий **A20**; **разблокирована D-43** (до неё держало открытое решение `O-10`) |
...
B4 T-1), T-B6-04/07/15/22/24 — по решению оператора или по готовности соседних треков. **T-B6-28 и T-B6-29** (дельта гейта) идут в Волне 3 после T-B6-08, обе `ГОТОВ К ВЫДАЧЕ`.

**Правки после гейта адекватности (06/V-06 + D-38..D-46), затрагивающие этот трек:**

- **Аксиома `AX_FILE_HEADER_TASK_TRACEABILITY` меняется под `@spec` (D-40).** Принят **полный переход на `@spec`** в `2.0.0-draft`: новый v2-источник хранит один resolvable `@spec`, v1 `@tasks` остаётся совместимым legacy-**входом**, правило «каждый historical `@tasks` — owning task» для v2 **отменяется**. Правятся: сама аксиома, шапка в `shared/sdd/templates.ts`, `tasks-append-only`, `reconcile.directive.hbs` (в частности `:116-122` перестаёт reopen-ить по историческому `@tasks`) и парсер `cli/cmd/orient/core/extract-header.ts` (снятие hard-coded `/^TSK-\d+$/`, `:27-32`). **Владельцы — задачи подтрека `FO-*`** (`32-TRACK-SYNC-OWNERSHIP.md` §4.1-bis): `FO-1` (ADR), `FO-3a` (парсер), `FO-5` (аксиома, шаблоны, `tasks-append-only`, reconcile), `FO-6` (миграционный шаг переписывания шапок). Критерий приёмки — **A17**. Задачи этого трека, трогающие те же файлы (`T-B6-05`, `T-B6-13`, `T-B6-17`), координируются с `FO-5` по порядку правок.
- **`GAP-3`** (дельта 06 §5.1 п.29): `H_ASK_WITHOUT_CARD` объявлен в директиве **либо** снят из аллоулиста kit-аудита; область `lint-axioms` покрывает этот класс. Выдаётся вместе с **`T-B6-08`**/**`T-B6-24`** (Волна 3), критерий **A22**.
- **`P-01`** (`61 §1`): восемь инвариантов v1 без вердикт-матрицы (C16–C20, S10, R6, D10) дозаводятся строками «аксиома / остаётся в скилле / не переносится» с доказательством `файл:строка` в **§3.7 этого документа** (`40:3.7`). Ранее это был вопрос оператору `O-8`; теперь — задача плана, критерий **A22**. **ВЫПОЛНЕНО (пачка 8, документная часть, 2026-09-08)** — 4 строки «аксиома», 3 «не переносится», 2 «нужен разбор» (`C19`, `D10`), см. итог в `§3.7`.
- **`P-02`** (`61 §1`): противоречие D3/D4 снимается единым владельцем самомиграции и порога WARN→ERROR + перекрёстными ссылками `31 §4.2` ↔ `40 §5` (эта строка). Ранее — вопрос `O-9`. **ВЫПОЛНЕНО (пачка 8, документная часть, 2026-09-08).**

  **Разбор противоречия и снятие.** Метка «D3/D4» смешивала два разных предмета под одним ярлыком: (а) **глобальные решения оператора** `D-3` (самомиграция `gennady`, 127 тикетов, `01-INTERVIEW-DECISIONS.md:67`) и `D-4` (порог `group-receipt` WARN→ERROR, `01-INTERVIEW-DECISIONS.md:68`); (б) **локальные пункты трека 31** `31-TRACK-CHECK-LOG.md` §4.2 — там же, независимо от глобальной нумерации, идут собственные развилки-для-оператора «**D3.** Что означает `[x] DONE` в v2» и «**D4.** Политика грандфазеринга» (`31:666-676`). Локальные D3/D4 трека 31 формулировали грандфазеринг-политику (маркер `<!--PHASE_RECEIPTS:v1-->`, три опции: opt-in / invert / dated) как **всё ещё открытую развилку**, не называя ни владельца, ни решённые глобальные `D-3`/`D-4`, ни задачу `B2-20`, ни `O-6` — хотя эта же тема на доске уже закрыта постановкой: единственный владелец порога WARN→ERROR — **`B2-20`** (`61:83`, зависит от `B2-16`, `E-14`, `B2-17`, `E-22`; критерий **A13**; решение **D-4**/**O-6**), единственный владелец самомиграции — **`E-14`** (`61:193`, критерий **A1**, решение **D-3**; обязательна по **D-39** — самомиграция единственный путь совместимости, мягкого v1/mixed-режима нет). Противоречие было текстовым: локальный §4.2 трека 31 читался как «решение ещё не принято», хотя оно уже принято и назначено. Снятие: (1) в `31-TRACK-CHECK-LOG.md` §4.2 добавлена явная сноска, отделяющая локальные ярлыки D1–D5 трека от глобальных `D-1..D-51`, и прямая ссылка от локального D4 к `B2-20`/`D-4`/`O-6`/`40 §5` (см. `31:4.2`); (2) эта строка — обратная ссылка `40 §5` → `31 §4.2`. Единый явный владелец: **самомиграция → `E-14`** (трек 50/EVAL), **порог WARN→ERROR → `B2-20`** (трек 31/CHECK-LOG, но исполняется только после того, как `E-14` дала самомигрированный корпус). **Учтено:** `D-39` (мягкого режима нет — единственный путь совместимости — самомиграция, поэтому до её завершения корень красный и это ожидаемо), `D-40` (полный переход на `@spec`, не меняет владельца B2-20/E-14, но `FO-6` — тоже потребитель самомиграции по цепочке заголовков), `D-51` (прецедент: аналогичные развилки — размещение `V14-2*` — были закрыты явным решением оператора без пересмотра владельца задачи; здесь владелец был назначен раньше и решением не пересматривается).

**Правки после триажа #14 (D-48/D-49), затрагивающие этот трек:**

- **`T-B6-13` (§1.6): объём уточнён D-48.** Владелец единственный, но предмет расширен явно: не «до v1-текста», а **до обобщённой формы** iter 3 `8bb38477` целиком — `ax-ssot-traceability` (генерализация), буллет BDD «факты по ссылке», advisory `dangling-spec-ref`. Причина: D-48 обязывает v2 быть не хуже v1 по ясности спек, а не воспроизводить его буквально.
- **`V14-1`** (новый трек `V14`, Волна 3, после `T-B6-08`): `scope.directive.hbs`/`module.directive.hbs` объявляют `AX_SCALE_PROPORTIONAL_DEPTH` в `deps` + EXPRESS-ветка по образцу `infra.directive.xml:316-323` — сегодня `deps="AX_TOOL_INVOCATION"` и глубина авторинга не масштабируется. Критерий **A8**.
- **`V14-2a`** (пост-релиз, после `2.0.0-draft`, D-49): активирует `ax-deviation-self-resolve` в BeliefState `execute`, делает LOW-confidence ветку `ax-dialogue-discipline` flow-scoped и объявляет `H_ASK_WITHOUT_CARD` — детали и остальные задачи умбреллы (`V14-2b..e`) в `61-TASK-BOARD.md §1` (трек `V14`) и `_raw/V14-2-AUTONOMOUS-EXECUTE.md` §5.
