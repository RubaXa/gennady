...
- **`P-01`** (`61 §1`): восемь инвариантов v1 без вердикт-матрицы (C16–C20, S10, R6, D10) дозаводятся строками «аксиома / остаётся в скилле / не переносится» с доказательством `файл:строка` в **§3 этого документа**. Ранее это был вопрос оператору `O-8`; теперь — задача плана, критерий **A22**.
- **`P-02`** (`61 §1`): противоречие D3/D4 между этим треком и треком 31 снимается единым владельцем самомиграции и порога WARN→ERROR (`B2-20`, O-6) + перекрёстными ссылками `31 §4` ↔ `40 §5`. Ранее — вопрос `O-9`.
