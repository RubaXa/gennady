ОТЧЁТ 61 §1 — REL-11: устранены остальные 13 находок `npm audit` (после REL-14 закрыл 3 из исходных 16, не «2 из 15/16» — правка по независимому пересчёту `V-BATCH-03.md` §3/F4: доска ошибочно говорила «11 шт.», доаудитное состояние `e7b5ba1e` пересчитано верификатором напрямую — 16, из них REL-14 закрыл 3 (`undici`, `esbuild`, транзитивно `release-it`), REL-11 — оставшиеся 13)
...
**Правка по вердикту `V-BATCH-03.md` §3 (независимый пересчёт доаудитного состояния):** доска (`61-TASK-BOARD.md` строка 145) и трек говорили «11 находок» — это число никогда не измерялось повторно, база `e7b5ba1e` не перезапускалась ни этим отчётом, ни R-REL-14. Верификатор восстановил `package.json`+`package-lock.json` всех трёх состояний в изолированные каталоги и прогнал `npm audit --package-lock-only --json` напрямую: **`e7b5ba1e` (до пачки) = 16** (0 info, 5 low, 4 moderate, 7 high, 0 critical), **`4be0c612` (после REL-14) = 13** (совпадает дословно с числом в этом параграфе), **`fcd1d118` (после REL-11) = 0**. Итог: **16 → 13 → 0**, а не «11 → 0»; REL-14 закрыл 3 находки (`undici`, `esbuild` и транзитивно `release-it`, уязвимый через `undici`), а не 2 — доска ошибочно указывала «закрывает 2 из 11». Дополнительно (F7, некритично для этого отчёта): пересчёт lock-диффа `4be0c612 → fcd1d118` показал 21 изменённый пакет и 2 новых в цепочке поставки (`fastdom@1.0.12`, `strictdom@1.0.1`, пришли с бампом `mermaid 11.16.0 → 11.17.2`) — §1 ниже перечисляет 14 (13 + 1 транзитивный), полный список не приводился; существо утверждения (non-major, 0 мажорных скачков) не пострадало.
