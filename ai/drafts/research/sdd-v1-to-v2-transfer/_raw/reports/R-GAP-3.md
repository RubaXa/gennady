...
## 5. Правка по вердикту верификатора (V-BATCH-18 F-5)

Находка: `ai/kit/audit-halt-activation.mjs`'s failure-hint (`✗`-вывод при находке необъявленного `H_*`) всё ещё отсылал к «`root/scope -> router H_ASK_WITHOUT_CARD` entries» как к живому примеру записи `ALLOWLIST_CROSS_DIRECTIVE_REFS` — но именно ЭТА задача (GAP-3) обе такие записи удалила. Читатель отсылается к образцу, которого больше нет; строка не мёртвый код — она реально печатается (наблюдалась в прогонах §3 п.2 выше).

Что сделано (коммит `86fe23f4`, дерево `rc-v6`): подсказка заменена на живой, всё ещё существующий пример — `critic/interview-protocol/root -> H_UNFORMATTED_ASK` (halt, объявленный один раз в `router.directive.hbs` и процитированный несколькими директивами через общий партиал контракта `question-format`). `npm run audit:halts` и `npm run check` зелёные после правки.
