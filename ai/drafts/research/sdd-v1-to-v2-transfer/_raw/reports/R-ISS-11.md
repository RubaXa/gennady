...
1. Нужен ли отдельный follow-up-бриф на `sdd-log.cmd.ts` + `audit/steps/STEP_2_SEMANTIC.xml` для полного закрытия ISS-11 (warn-обвязка), или это остаётся на усмотрение батча, который и так трогает эти файлы (упомянутая координация с B2-14 — тот же скелет Handoff)?
