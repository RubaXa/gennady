...
1. **«гейт «каждый кирпич подключён» — grep не должен давать 0»** — на момент этого коммита выполнено только ручным grep, не гейтом (см. отклонение в §6). ВЫПОЛНЕНО с явным отклонением по 4 из 7 (см. §2, разрешено формулировкой брифа «достроить ИЛИ удалить»).
   ```
   ...
   **Правка по вердикту верификатора (V-BATCH-15 F-3):** мутация M4b верификатора показала, что этот ручной grep НЕ был гейтом — новый неподключённый файл в `ai/kit/contract/process/` проходил `audit:contracts`/`build:directives`/`check:directives-fresh` все зелёными. Реальный гейт («PART 3» в `ai/kit/audit-contract-activation.mjs`, коммит `f80c27f2`) добавлен отдельным коммитом этой же пачки после ревью — перечисляет каждый `ai/kit/contract/**/*.xml`, требует хотя бы один `{{> "contract/<dir>/<name>"}}` в дереве шаблонов, с сокращающимся именованным allowlist для двух подтверждённых сирот вне зоны B2-18 (`contract/uikit/spec-structure`, `contract/critic/oc-structured`). Мутация того же вида, что и M4b, теперь даёт exit 1 (проверено вручную).
2. **Сборка/аудиты** — ВЫПОЛНЕНО.

   **Правка по вердикту верификатора (V-BATCH-15 F-5.1):** заявленное здесь «dangling-axiom 36→35» недостоверно (та же метрика, то же опровержение, что и в `R-T-B6-17.md` §3.1 — метрика этой пачкой не двигалась, T-B6-17 −1 и B2-18 +1 отреферены-в-шагах компенсируют друг друга). Воспроизводимое доказательство того же класса — см. `R-T-B6-17.md` (отвязка аксиома от директивы → `build:directives` даёт `✗ 5 undefined axiom reference(s)`, exit 1).
   ```
   $ node --experimental-strip-types ai/kit/build-directives.ts   → exit 0 (без НОВЫХ dangling/undefined)
   $ npm run check:directives-fresh   → ✓ matches a fresh rebuild
   $ node ai/kit/audit-axiom-activation.mjs      → ✓ clean
   $ node ai/kit/audit-contract-activation.mjs   → ✓ clean (после F-3: 28 template(s) + 33 assembled directive(s) + 54 contract file(s), против 55 template file(s) (recursive))
