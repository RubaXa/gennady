...
```
198 → 192 (−6). **ИСПРАВЛЕНО (V-BATCH-04, plan-verifier):** состав был назван неверно («ровно 3 коллизии × 2 находки» = 6). Фактический set-diff: ушли **3** `SDD_TASK_ID_COLLISION` (по одной находке на пару — `model.task-35`, `state.task-45`, `dbc-linter.task-88`) **и 3** `SDD_TRACKER_STATUS_DRIFT` (`tasks/agent-mon-cli/README.md`, `tasks/dbc/README.md`, `tasks/infra-npm-publish/README.md`) — разрешение коллизий попутно сняло дрейф трекер-статуса в двух НЕ тронутых этой задачей README (эффект в плюс, не заявленный ранее); отдельно, 2 ошибки (`SDD_DEP_UNRESOLVED`, `SDD_VERIFICATION_TABLE_INVALID`) не исчезли, а переехали с `task-88` на `task-186` (см. §«Известное следствие для гейта baseline» ниже — это они дают красный `gate:sdd-check-baseline`, закрыто в `R-BATCH-04-specs-match-code.md` §«Правки по V-BATCH-04 и ребейз на пачку 5»). Итог −6 верен, композиция — 3+3. ВЫПОЛНЕНО.
