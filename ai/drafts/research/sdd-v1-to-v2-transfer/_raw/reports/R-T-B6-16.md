...
```mermaid
sequenceDiagram
  participant EXE as execute.directive.hbs:22,42,179-183 STEP_6_AUDIT_REVIEW
  participant REV as fresh audit/code-review dispatch
...
**Отклонение 2 (сознательно НЕ реализовано — both-way про запись вердикта, по риску батча; уточнено по правке верификатора V-BATCH-17).** Бриф явно указывает: «both-way про запись отложенного решения — remains за отложенной темой (§3.4)». Реализован ТОЛЬКО правило останова (повтор набора без нового evidence → `H_NO_PROGRESS`); механизм записи `[verdict: pending-operator]` и явная оговорка про такую запись — НЕ реализованы, поскольку самого механизма записи отложенного решения в дереве ещё не существует (это предмет будущей задачи V14-2e/V14-2a). Первоначальная формулировка этого отклонения («текст сформулирован достаточно обще, чтобы не противоречить будущему both-way») СНЯТА как неточная. Точнее: `ax-re-dispatch.xml` перечисляет условия отсутствия прогресса («no changed diff, no new tool output, no new fact the prior attempt did not already have») и отдельно заканчивает «a genuinely new edit … resets the count» — запись `[verdict: pending-operator]` В ТИКЕТ есть изменённый дифф и новая правка тикета, а значит по этому же правилу «resets the count» цикл сбрасывается КОРРЕКТНО (не покупает лишний диспатч, но и не должен ложно давать `H_NO_PROGRESS` при легитимной эскалации оператору) — сегодня противоречия с механикой останова нет. Тем не менее явная письменная оговорка, разбирающая именно этот сценарий («запись вердикта в тикет — это genuinely new edit, поэтому останов не должен считать её попыткой без прогресса»), в `ax-re-dispatch.xml` пока отсутствует; её владелец — задача V14-2a, и Lead добавит соответствующую строку/уточнение на доску отдельно (вне зоны этого коммита — `ax-re-dispatch.xml` в этой пачке уже редактировался по T-B6-16, но эта конкретная формулировка ожидает V14-2a).
