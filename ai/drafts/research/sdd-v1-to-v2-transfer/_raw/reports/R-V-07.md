...
**Дополнено верификацией V-BATCH-11 (не этой сессией) + правкой этой сессии.** Верификатор
отметил два числовых расхождения в waiver-реестре (база 25→26, снято 8→9 — пятый снятый V-07
символ, `PROJECT_CONFIG_FILENAME`, не был назван ни в одном отчёте) и незакрытый разрыв: за
пределами exit-4 валидации слитый `config`/провенанс нигде не наблюдаемы ни в одном v2-выводе —
`loadStackConfig`'s результат используется в `index.ts` только для самой проверки, дальше в
`run()`/`resolvePhaseContext()` не передаётся. Оба пункта исправлены этой сессией:
`specs/cli/verify/verify.spec.md` (коммит `f83a4733`) — числа 26/9/5 везде, `PROJECT_CONFIG_
FILENAME` задокументирован (реальная вторая ссылка: `sdd-verify.types.ts:14,36`), провенанс-
разрыв назван явно как открытый кандидат «V-07b» (не закрыт — наблюдаемость config/provenance
в снимке/выводе так и не сделана; `phase-context.ts` (V-08b) теперь **потребляет** конфиг для
anystack-гейтов, но это не то же самое, что per-key provenance в выводе).
