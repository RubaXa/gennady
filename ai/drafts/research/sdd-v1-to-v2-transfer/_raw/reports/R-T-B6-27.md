...
flowchart TB
  ...
  COMMENT2["audit-halt-activation.mjs:150-167 NOTE — документирует удаление, объясняет почему обе записи были инертны"]
...
7:    {{> "axiom/process/ax-stateless-flow"}}
8:    {{> "axiom/process/ax-operator-language"}}
9:    {{> "axiom/process/ax-tool-invocation"}}
10:    {{> "axiom/process/ax-operator-agreement"}}
13:    {{> "contract/process/artifact-approval-flow"}}
14:    {{> "contract/process/artifact-approval-marker"}}
