...
**P5 (документировать exit 2 в help.ts)** — `node dist/gennady.js sdd-check --help | grep -A1 "Exit codes"`, текущий (после правки V-BATCH-04 §B2-05/17, см. ниже):
```
...
```
ВЫПОЛНЕНО. **Уточнение (V-BATCH-04, plan-verifier):** первая версия этой правки документировала только `SDD_NO_TICKETS_FOUND`, потеряв пред-существующий `ERR_CLI_SDD_CHECK_UNKNOWN_ID` (тоже exit 2, для `--task <неизвестный Task-ID>`, `sdd-check.types.ts#unknownIdError`) — код 2 общий для двух разных причин. Строка выше и инвариант в `sdd-check.spec.md` (см. `R-BATCH-04-specs-match-code.md` §«Правки по V-BATCH-04») исправлены.
...
**ИСПРАВЛЕНО (V-BATCH-04, plan-verifier).** Строка выше — «Пре-существующий несвязанный fail … `ENOENT` … `git stash` + повторный прогон … тот же единственный fail» — **не воспроизводится**. С правильным cwd (`rc-v6`, а не соседний worktree) `cli/cmd/sdd-check/__tests__/sdd-check.cmd.test.ts` даёт **82 pass / 0 fail**, без исключений (см. §3 выше, повторный прогон `node --import tsx --test …` из `rc-v6`). Причина ложной записи — сам исполнитель батча запускал сюиту не из корня `rc-v6` (обходной путь вокруг блокировки `cd` в песочнице rc-executor резолвил фикстуру в АБСОЛЮТНЫЙ путь другого дерева, `nice-panini-8aa14e`), а не дефект репозитория; `git stash`-проверка на том же неверном cwd лишь воспроизвела тот же артефакт, не опровергла его. Строка доказательства снята.
