...
**Правка по вердикту верификатора (V-BATCH-15 F-2):** §3 п.1 доказательство («94/94 зелёных без правок в существующих кейсах», см. `R-B2-19.md` до правки) читалось как подтверждение связки `check.ts` ↔ `## Blocker Trail`, но было доказательством ОТСУТСТВИЯ замка — мутация M1 верификатора (вернуть `check.ts` к одноаргументному `hasActiveBlocker(logSec.content)`) оставляла все 217/217 тестов зелёными. Both-way замок добавлен отдельным коммитом `b7360692` в `shared/sdd/__tests__/check.test.ts`: (а) 🛑 в закрытом раунде + ✅ с обратной ссылкой `(Round N / P<M>)` в `## Blocker Trail` → нет находки; (б) тот же тикет без секции `## Blocker Trail` → находка есть.
