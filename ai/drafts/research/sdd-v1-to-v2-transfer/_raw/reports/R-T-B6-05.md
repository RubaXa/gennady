...
Коммит: `0ea38e5f` `fix(T-B6-05): AX_DISPATCH_VIA_BATCH cites the step reconcile actually has`. Сам коммит `1da11d89` вердиктом признан не требующим правок по существу («T-B6-05 выполнен»); правка ниже — по смежной находке N-5, задевающей ровно ту же аксиому.

| Находка | Что сделано | Где |
|---|---|---|
| **N-5 (medium).** Тело аксиомы `AX_DISPATCH_VIA_BATCH` говорило «Reconcile … checks them at STEP_7» — в `reconcile.directive.xml` шага `STEP_7` нет вовсе, последний шаг — `STEP_6_VERIFY`; проза активации в `reconcile.directive.hbs:189` (`STEP_5_APPLY`) уже называла `STEP_6_VERIFY` верно — сама директива противоречила себе. Дословный перенос из v1 `fix.directive.xml`, где был `STEP_7_EXECUTE`/`STEP_9_VERIFY`. | Правка одной строкой: `STEP_7` → `STEP_6_VERIFY` в теле кирпича. Добавлен регрессионный кейс (2 проверки): тело кирпича говорит `STEP_6_VERIFY` и не содержит `STEP_7`; собранная `reconcile.directive.xml` реально содержит `<Step id="STEP_6_VERIFY">` (аксиома называет существующий якорь, не голословно). | `ai/kit/axiom/process/ax-dispatch-via-batch.xml`; `ai/kit/__tests__/review-critic-bounds.test.ts` (describe «AX_DISPATCH_VIA_BATCH cites the reconcile step that actually exists (V-BATCH-20 N-5)»). |

**Побочный эффект (не ручная правка):** `ai/directives/sdd-v2/reconcile.directive.xml` изменился как механическое следствие пересборки после правки кирпича (строка внутри собранного тела аксиомы) — `reconcile.directive.hbs` в этом коммите не редактировался.

**Доказательства:** `node --import tsx --test ai/kit/__tests__/review-critic-bounds.test.ts` — 29/29 pass, exit 0; `check:directives-fresh` / `audit:sdd-templates` — green; коммит прошёл pre-commit целиком (`ALL PASS 5/5`) после одного синхронного повтора (первая попытка упала на флаки под host-нагрузкой — `cancelled`, не `failed`; правило брифа про повтор до 3 раз применено).
