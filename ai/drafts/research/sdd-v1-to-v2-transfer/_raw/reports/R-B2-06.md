...
```mermaid
flowchart TB
  AUDIT2["audit worker: triggered-reopen=Round-2"] -->|"## Audit Rounds"| PARSE["parseAuditRounds (execution-log.ts:~565)"]
...
- Известный побочный эффект (не блокер): `sdd-task.cmd.ts`'s собственный вызов `scanBlockerTrail` не тронут — не относится к B2-06.
