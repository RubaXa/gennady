...
flowchart TD
  ...
  D --> E{"code ∈ MIGRATION_CRITICAL_CODES?\nmigration-grade.ts:43-49 (стало, на коммите 03d661d5)"}
  E -->|"+ SDD_VERIFICATION_TABLE_INVALID\n+ SDD_COVERAGE_POLICY_INVALID"| F["pass=false — E-07"]
  E -->|"прочее (BDD/язык/длина ячеек)"| G["backlog"]
  style F fill:#dfd
```

*(ИСПРАВЛЕНО V-BATCH-22 verdict B-9: строки выше были заявлены как `:64`/`:31-42`, фактические на
коммите `03d661d5` — `:78`/`:43-49`. Правкой по вердикту верификатора «критично» разделилось на
СТРУКТУРНЫЕ коды (baseline-diff, как здесь) и коды ИСПОЛНИМОСТИ (`SDD_VERIFICATION_TABLE_INVALID`/
`SDD_COVERAGE_POLICY_INVALID` — теперь считаются по ОСТАВШИМСЯ находкам, не только внесённым; B-2, см.
сводный отчёт), что сдвинуло эти же функции ещё раз — актуальные номера на HEAD после исправлений:
`diffIntroduced` → `migration-grade.ts:95`, Set → `migration-grade.ts:54-65`.)*
