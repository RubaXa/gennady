...
### Было — `<ScalePath when="scale=function|fix">` уже был (число модулей), но EXPRESS/`deps` до STEP_1_ORIENT не доезжали (глубина интервью)
...
**Правка по V-BATCH-19 (независимая проверка).** Диаграмма «было» выше и исходный текст п.1 отчёта утверждали «scope/module не знают о SCALE, всегда полный интервью-путь» — неверно наполовину: `<ScalePath when="scale=function|fix">` существовал в обоих файлах ДО этого коммита (на базе `c9b58636`: `scope.directive.xml:87`, `module.directive.xml:95` — единственные два места в корпусе с этим элементом; после регенерации этим коммитом — `scope.directive.xml:135`, `module.directive.xml:130`), он управляет числом модулей (один cohesive module вместо декомпозиции), а не глубиной интервью. Верно только про `deps=`/`LogicSwitch`/EXPRESS-ветвь — они действительно отсутствовали и добавлены этим коммитом (`1c4b746f`). Диаграмма «было» исправлена выше. Следствие по существу (не код, follow-up): после этого коммита в одном файле сосуществуют `ScalePath` (число модулей) и `LogicSwitch` (глубина интервью) с одинаковым триггером `scale=fix|function` — разные предметы, но один и тот же класс, на который смотрит `AX_PARALLEL_MECHANISM_REQUIRES_JUSTIFICATION`; объединение или явное разграничение — вне Зоны этой пачки, не сделано здесь.
