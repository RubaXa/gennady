...
3. **Markdown-секции сбалансированы** (не сломал структуру спеки правкой):
   ...
   ```
   ВЫПОЛНЕНО.
4. **`sdd-check --all` не даёт новых ОШИБОК** (docs-only diff, error-счётчик byte-в-byte тот же):
   ```
   $ npm run build && node dist/gennady.js sdd-check --all rc-v6
   [sdd-check] 198 error(s), 432 warning(s) across 212 file(s)
   ```
   exit 1 (ожидаемо — pre-existing corpus errors). **Уточнение (V-BATCH-04, plan-verifier):** предупреждений — не тот же счётчик, что на чистой базе `f16d7f17` (там 431, не 432, см. `R-BATCH-04-specs-match-code.md` §«Правки по V-BATCH-04»): "432" здесь — это уже 431 базовых **+1 новый**, `specs/cli/sdd-log/sdd-log.spec.md: warn: SDD_MODULE_OVERSIZED — Entity Inventory has 26 entities (> 20)`, прямое следствие того, что §3-инвентарь этой самой задачи дополнен девятью утилитами (см. §1 выше). Advisory (`AX_HIERARCHICAL_SPECS`), гейт не роняет, но не был назван явно нигде в этом отчёте — теперь зафиксирован. ВЫПОЛНЕНО (error-часть приёмки); warning-долг задокументирован, не «починен» размером спеки по прямому указанию Lead (декомпозиция `sdd-log.spec.md` — отдельная задача).
...
- **(добавлено V-BATCH-04)** §3-инвентарь `sdd-log.spec.md` дополнен до 26 сущностей — сама задача, спека-first по конструкции, завела `warn: SDD_MODULE_OVERSIZED` (порог 20) на собственный документ. Не исправлялось расширением инвентаря искусственно уже сейчас; зафиксировано как advisory-долг на декомпозицию.
