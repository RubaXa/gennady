...
flowchart LR
  subgraph before["БЫЛО"]
    ...
    C2["execute.directive.xml:67"] -->|"per `AX_AUDIT_HOOK`"| X
...
flowchart LR
  subgraph after["СТАЛО"]
    L1["ai/kit/axiom/process/ax-audit-hook.xml"] -->|"{{> }} audit.directive.hbs:24"| D["audit.directive.xml /<br/>audit/steps/STEP_1_MECHANICAL.xml<br/>(&lt;Axiom id=&quot;AX_AUDIT_HOOK&quot;&gt;, реальное определение)"]
    C1["code-review.directive.xml:6"] -->|"per `AX_AUDIT_HOOK`"| D
    C2["execute.directive.xml:67"] -->|"per `AX_AUDIT_HOOK`"| D
