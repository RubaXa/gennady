...
>
> **2026-09-24:** документ сохранён как evidence/history исходного аудита. Открытый D-33 закрыт
> решением D-70: central `knowledge.xml` мигрирует в colocated `*.rule.yaml`, выбор выполняет общий
> task/phase-aware `RuleResolver`. Нормативный cutover/file map — `64-VERIFY-RULES-UNIFICATION.md`.
...
## § Дельта гейта адекватности (D-44/D-45, внесено правкой V-61b)

> Трек 33 выпал из дельты `06-ADEQUACY-GAP.md` §5 и не входил в набор сверки `61 §4`, хотя **D-45** меняет статус трёх задач именно этого трека. Действующее состояние — по `61-TASK-BOARD.md` §1/§3/§5.3; формулировки §5 выше остаются исторической редакцией.

- **`T-2`** — **не отдельная задача, а указатель на `RULES-D2`** (владелец модуля `shared/sdd/rule-registry.ts`; **D-45**). Строка сохранена для трассируемости, в счётчиках волн считается у владельца, вне волн отдельной строкой не занимает. Пометка «*(только R-B)*» в §5 читается как историческая.
- **`T-8`** — **вне волн** (снята из Волны 3): её предпосылка `RULES-D2` пост-релизная, то есть блокировка **транзитивна через D-33**. Пометка «*(после трека VERIFY)*» в §5 неточна — трек VERIFY здесь не бар.
- **`T-14`** — **`ЗАБЛОКИРОВАН D-36`**, вне волн: `main` заморожен, правка main-side README без явного OK оператора не делается. Отдельного вопроса оператору не требует (`O-15` снят как дубль, **D-45**).
- **`RULES-D1..RULES-D6`** — вне волн, `ЗАБЛОКИРОВАН D-33` (вход инструмента и варианты A/B1/B2/C/D пересматриваются до постановки); **`T-7` ОТМЕНЕНА** по D-32, предмет у `RULES-D5`.
- **Правила python — после релиза** вместе с python-пресетом `V-10` (**D-44**); порядок стеков исполняется как anystack → golang → swift. D-25 (python-*правила* до первого проекта) этим не отменён, а лишь согласован с D-44.
- **В релиз идут** вариант-независимые `T-1`, `T-3`, `T-4`, `T-5`, `T-6`, `T-9`, `T-10`, `T-11`, `T-12`, `T-13` + `T-15` (§5.2, `33b §4` п.1) — критерий `A8` зафиксирован для релизного режима (A) по D-31.
