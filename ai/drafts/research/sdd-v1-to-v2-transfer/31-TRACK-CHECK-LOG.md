...
**D3. Что означает `[x] DONE` в v2.** `ax-audit-hook.xml` говорит «mechanical close, not verification», `AX_AUDIT_HOOK` при этом обещает блокировку зависимых задач других спек (задокументировано в четырёх местах — находка B), а `pickableTasks`/`sdd-task --audit-group` эту блокировку не делают.
