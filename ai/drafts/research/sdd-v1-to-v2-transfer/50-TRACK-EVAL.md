...
| **Документация для нового человека** | **C+** | `RUNBOOK.ru.md` (211 строк) даёт полную процедуру поднятия сервера и чтения наблюдений; `WRITING-EVALS.ru.md` (158) — как добавить эвал. Но: `README.md` «Documentation map» перечисляет 4 документа из **14** (в `ai/flow-eval/**` — 10 в корне + 4 в `docs/`; карта не включает `QUALITY-RULES.ru.md`, `ROADMAP.ru.md`, `EXPERIMENTS-LOG.ru.md`, `INFRA-TASKS-RESEARCH.ru.md`, `P9-*.md`, всех четырёх `docs/*.md`); `README.md:29` описывает `scenarios.json` как «(authoring, scaffold, execute, repair)», умалчивая три `task`-сценария; `ROADMAP.ru.md:63` заявляет «Итог (все цели закрыты)» и «suite 71/71» (`:59`) — устарело на 19 кейсов и на весь миграционный/round-trip трек; `types.ts:37-39` (и `:68-69`) обещает грейд миграции «sdd-state=v2 + sdd-check clean», что прямо противоречит `migration-grade.ts` (baseline-diff); `README.md` не упоминает необходимость `npm run build` перед suite |
...
| **E-13** | G1 | `E-G1-llm-go` — единственный LLM-сценарий группы: execute на go-фикстуре | `golang-slugify` | golden exit 0 И R-COMPLETE pass; 2 pass | 30–60 k ×2 (экстраполяция) | M |
...
- G4: E-16 → E-06 → E-07 → E-14 (обязателен, закрывает и E-08);
