...
| **▼ GAP — ГЕЙТ АДЕКВАТНОСТИ (06/V-06)** | — | — | — | — | — | — | — | — | — | — | — |
| GAP-B-1 | GAP | **Версионированный baseline находок** корневого прогона `sdd-check --all .` по `code/file/severity` (стартовая точка — измеренные **198 error / 431 warn** на `227c03a8`) + предикат **zero-new-error** в CI; **любое изменение baseline — отдельное operator-approved решение** | новый `_raw/baseline/rc-baseline-1/findings.json`, скрипт-предикат + гейт в `npm run check`/CI | S | REL-17 | детерм.: «прогон на теге `rc-baseline-1` даёт ровно baseline»; «error класса, которого нет в baseline → exit 1» | **A18, A1, A13**; точка отсчёта для **E-22**, **B2-22** | **D-38, D-39** | нет (предпосылка A1/A13/A18) | **ГОТОВ К ВЫДАЧЕ (Волна −1)** | новая задача (06 §5.1 п.1a, потерянный MUST первоисточника, найден V-06). Без неё `A1`, `A13` и `E-22` не имеют точки отсчёта. **Заменяет отменённую `B2-21` как предпосылку `E-07`** (D-39) |
...
| P-03 | Гейт адекватности плана по внешнему ресёрчу (D-37): R1..R4 → `06-ADEQUACY-GAP.md` → верификация V-06 → решения оператора **D-38..D-46** → правки доски/приёмки | Lead | M | Волна 0 (до брифа 1/5) | **ЗАВЕРШЁН** (гейт пройден: `06` ВЕРИФИЦИРОВАНО, дельта применена — см. § Итог правок гейта адекватности) | **O-7 закрыт** |
...
Ни одна из этих задач не входит в релиз `2.0.0-draft.<N>`; каждая ждёт решения оператора или пост-релизного этапа. Полный перечень (**21 строка**, из них 2 `ОТМЕНЕНА`/`ПОГЛОЩЕНА`):

- **Было до дельты (9):** **SO-3** (целевая хэш-модель владения, D-22 — пост-релиз), **SO-8** (релизность S2-регресса — решение оператора), **RULES-D1**, **RULES-D2**, **RULES-D3**, **RULES-D4**, **RULES-D5**, **RULES-D6** (весь блок слоя правил, `ЗАБЛОКИРОВАН D-33`; пересмотр — до постановки, материалы `33a-RULES-EXPLAINER.md`/`33b-RULES-VARIANTS.md`), **V-16b** (`gennady fix`, `ЗАБЛОКИРОВАН O-2`, вне релиза по D-13), **T-7** (`ОТМЕНЕНА` по D-32 — строка сохранена, предмет у RULES-D5).
...
Отдельно: **T-2** сохранена как **указатель на `RULES-D2`** (D-45) — считается в волне владельца, отдельной строкой волны не занимает; **T-B6-04** размещена в Волне 3 и остаётся заблокированной (D-26).
...
- Измеренная стартовая точка (V-06, чекаут 227c03a8, исполненный прогон): 198 error / 431 warn, exit 1;
...
ФАЙЛЫ
- трогать: тег rc-baseline-1; _raw/baseline/rc-baseline-1/** (before-отчёт, findings по
  code/file/severity, окружение, hash lockfile); скрипт-предикат zero-new-error и его подключение
  в npm run check / CI
...
- before-отчёт снят и лежит в репозитории плана; путь к baseline находок:
  ai/drafts/research/sdd-v1-to-v2-transfer/_raw/baseline/rc-baseline-1/findings.json
  команда: npx gennady sdd-check --all . ; echo "exit=$?"
  ожидаемо: exit=1, 198 error / 431 warn — ровно то, что записано в findings.json
- предикат zero-new-error работает в обе стороны;
  команда: <скрипт предиката> --baseline ai/drafts/research/sdd-v1-to-v2-transfer/_raw/baseline/
           rc-baseline-1/findings.json ; echo "exit=$?"
...
- npm run check зелёный вместе с новым гейтом;
  команда: npm run check && npx gennady sdd-check --all .
...
- если фактические числа расходятся с 198 error / 431 warn — остановись и покажи расхождение: baseline
  фиксируется по ФАКТУ прогона, но расхождение с измерением V-06 обязано быть объяснено, а не молча
  перезаписано.
...
| **GAP (ГЕЙТ АДЕКВАТНОСТИ, дельта 06/V-06)** | **7** (GAP-B-1, GAP-1, GAP-2, GAP-3, GAP-4, GAP-P-1, GAP-P-2) | 0 (GAP-B-1 — предпосылка A1/A13/A18, Волна −1) |
| **Итого строк** | **193** | **5** |
...
| Волна | Кол-во задач | Что изменилось после дельты гейта (06/V-06 + D-38..D-46) |
|---|---|---|
...
| Волна 0 | **67** | было 55; **+12**: `FO-3a`, `FO-1` (разблок. D-40), `IDN-1` (разблок. D-41), `IDN-2` (D-42), `GAP-E-1`, `GAP-E-2` (SHOULD), `GAP-E-4`, `GAP-E-5` (D-46), `GAP-1`, `GAP-2`, `GAP-P-1`, `GAP-P-2` |
...
| Волна 3 | **46** | было 39; **+9**: `FO-2`, `FO-3`, `FO-4`, `FO-5`, `B2-22`, `IDN-2a`, `T-B6-28`, `T-B6-29`, `GAP-3`; **−2**: `T-8` (предпосылка `RULES-D2` пост-релизная) и `T-14` (D-45) ушли вне волн |
...
Прирост «готовых» (+13) дали: две задачи Волны −1 (`REL-17`, `GAP-B-1`), снятие трёх блокировок решениями оператора (`FO-1` по D-40, `IDN-1` по D-41, `T-B6-29` по D-43) и восемь новых вариант-независимых задач Волны 0 (`FO-3a`, `IDN-2`, `GAP-E-1`, `GAP-E-4`, `GAP-E-5`, `GAP-1`, `GAP-2`, `GAP-P-1`, `GAP-P-2` — минус `GAP-E-2` и `T-B6-28`, попавшие в другие категории). Прирост «заблокированных» (+6) — целиком пост-релизные выносы по решениям оператора: `V-10` (D-44), `FO-7` (D-40), `IDN-3..IDN-6` (D-41), `T-8` (транзитивно D-33); `T-14` осталась заблокированной, но сменила основание на **D-36**.
...
## § Итог правок гейта адекватности (06/V-06)

> Источник: `06-ADEQUACY-GAP.md` §5 (нумерованная дельта плана) + § Итог верификации **V-06** + решения оператора **D-38..D-46** (`01-INTERVIEW-DECISIONS.md`). Где 06 и решения оператора расходились — **действует решение оператора**; каждое такое расхождение разобрано ниже и в §2.2 п.8–12.
...
**Приёмка:** заведены `A16`..`A23`; применены восемь правок существующих критериев (`A1`, `A3` ×2, `A6` ×2, `A9`, `A13`, `60 §3`); `A7` не менялся — адресация правки 60 исправлена верификацией V-06 на `A3`/`A1`.

**Брифы:** добавлен **Бриф 0/5 — `REL-17` + `GAP-B-1`** с командами-доказательствами (`git tag --list rc-baseline-1`, `git rev-parse rc-baseline-1` = `227c03a8`, `git status --porcelain=v1 --untracked-files=all`, путь `_raw/baseline/rc-baseline-1/findings.json`, предикат zero-new-error с обязательным негативным тестом). Бриф 1/5 (`V-01`) переведён на зависимость от `REL-17`.
...
- **Волны:** −1 → **2** · 0 → **67** · 1 → **4** · 2 → **17** · 3 → **46** · 4 → **14** · 5 → **11** · вне волн → **20**. Сумма **181** = 193 − 12 указателей.
...
Скрипт: разбор мастер-таблицы §1 (с учётом экранированных `\|`), сбор идентификаторов из §3 «Волны», регексп-сверка по девяти документам плана (`61`, `60`, `00`, `05`, `30`, `31`, `32`, `34`, `40`, `50`). `_raw/**` не редактировался и в сверку не входит; `06-ADEQUACY-GAP.md` и `01-INTERVIEW-DECISIONS.md` — источники дельты, их историческая формулировка сохраняется дословно и из проверки «живых» упоминаний исключена.

| Инвариант | Результат |
|---|---|
| Каждый id, упомянутый в `60`/`00`/`05`/трек-документах, существует в мастер-таблице `61 §1` | **выполнено.** Ровно два исключения, оба намеренные и документированные: `V-16` — исторический id, расщеплённый V-61 на `V-16a`/`V-16b` (ссылки на него в `30 §6`, `50`, `60`, `05` — историческая формулировка трека, доска держит обе половины); `V-61` — идентификатор **верификации**, не задачи. **Отдельно зафиксирована коллизия имён, унаследованная из `06-ADEQUACY-GAP.md`:** `V-06` означает и **верификацию** гейта адекватности, и **задачу** трека VERIFY (readiness-движок, `61 §1`). Переименование не делалось — id пришёл из источника дельты и уже процитирован в решениях D-38..D-46; при чтении различать по контексту («верификация V-06» vs «задача V-06»). Это ровно тот класс дефекта, который закрывает `IDN-1`/`IDN-2a` (репозиторная уникальность акронима + override) |
...
| В Волнах −1 и 0 нет заблокированных строк | **выполнено.** Все 22 блокировки — Волны 2/3/5 и вне волн |

### 5. Что осталось за пределами этого документа

- **Push тега `rc-baseline-1`** — запись в удалённый репозиторий; делает только оператор (Бриф 0/5, блок «ВОПРОСЫ НАЗАД»).
...
- **Честный остаток (06 §6, подтверждён V-06):** прогоны main-side, живой LLM-контур и масштаб дефекта `events` не проверены и **не проверяемы чтением**; стоимость `FO-2`, `IDN-3` и `B2-22` не измерена на текущем дереве. Ни одно из этих утверждений дельта не закрывает и не выдаёт за закрытое.
