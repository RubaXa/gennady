...
**Правило выдачи при непустой колонке «Зависит от» (записано по V-61b §C):** статус `ГОТОВ К ВЫДАЧЕ` совместим с непустой колонкой «Зависит от», если **все** предпосылки лежат в **более ранней** волне — тогда к моменту выдачи они уже исполнены. Таких строк **18** (`V-01`, `GAP-B-1`, `E-07`, `T-B6-08`, `T-B6-28`, `T-B6-29`, `E-04`, `SO-2b`, `T-3`, `T-6`, `B2-03`, `B2-05`, `B2-08`, `B2-09`, `B2-11`, `B2-13`, `B2-14`, `B2-15`); без этого правила счётчик `ГОТОВ К ВЫДАЧЕ` = 72 не воспроизводится третьей стороной. `ЧЕРНОВИК` означает предпосылку в **той же** волне или отсутствие волнового разведения.
...
Ни одна из этих задач не входит в релиз `2.0.0-draft.<N>`; каждая ждёт решения оператора или пост-релизного этапа. Полный перечень (**20 строк**, из них 2 `ОТМЕНЕНА`/`ПОГЛОЩЕНА`; правка V-61b — было объявлено «21», перечислено 20, и 20 же стоит в §5.4 и `05 §7`):
...
> Все числа §5 пересчитаны **машинно** по мастер-таблице §1 после дельты гейта адекватности (скрипт: разбор строк таблицы с учётом экранированных `\|`, классификация статуса по ключевому слову; строки-разделители `▼ ТРЕК` и мини-таблица `P-01..P-03` в счёт §1 не входят). Итог: **193 строки** мастер-таблицы (было 158, +35), из них **190 активных** (не считаются `ОТМЕНЕНА` `T-7`, `ОТМЕНЕНА` `B2-21`, `ПОГЛОЩЕНА` `GAP-E-3`), **12 дублей-указателей**.
>
> Дельта относительно V-61: 06 §5.1 давало 33 новые задачи и прогноз «158 → 191»; фактически добавлено **35** — к 33 задачам дельты прибавились две, заведённые прямо решениями оператора: **`IDN-2a`** (D-41) и **`GAP-E-5`** (D-46). Прогноз 191 заменён измеренным **193**.
...
**Блокеров релиза — 5 задач** (SO-1, SO-2, SO-2b, SO-7, SO-11 по D-22), закрывающих **2 issue-блокера** (#9.4, #24). Дельта гейта новых «ДА» в колонке блокера не добавила: `REL-17` и `GAP-B-1` — не блокеры релиза, а **предпосылки старта плана** (Волна −1), их отсутствие не задерживает релиз, а обесценивает все доказательства до него.

Активных задач: **190** (193 строки минус `ОТМЕНЕНА` `T-7`, `ОТМЕНЕНА` `B2-21`, `ПОГЛОЩЕНА` `GAP-E-3`). Из 193 строк **12** — дубли-указатели (ISS-1..7, ISS-9, ISS-12, ISS-13, SO-10, **T-2**), исполняются в волне своего владельца.

### 5.2 По размеру

Пересчёт по **190 активным** строкам (`T-7` и `B2-21` `ОТМЕНЕНА`, `GAP-E-3` `ПОГЛОЩЕНА` — исключены; их размеры были M, S–M и S).

| Размер | Кол-во | Было (V-61) |
|---|---|---|
| S (включая `S (в связке с B2-01)` у B2-02, `S (R-A′)` у T-4, `S (документ)` у GAP-P-1/GAP-P-2) | **94** | 77 |
| M | **77** | 64 |
| L | **13** (+ **FO-2**, **IDN-3**) | 11 |
| смешанные `S-M`/`S/M`/`S–M` (REL-11, E-10, E-15, B2-18, V-18, **IDN-2**) | **6** | 5 |
| **Итого** | **190** | 157 |
...
Пересчёт по перечислению §3; дубли-указатели (**12 строк**) считаются в волне владельца, не отдельно. Сумма по волнам = **181** = 193 строки − 12 указателей.

| Волна | Кол-во задач | Что изменилось после дельты гейта (06/V-06-GAP + D-38..D-46) |
|---|---|---|
| **Волна −1** | **2** | **новая волна** (06 §5.1 п.1/1a, утверждена **D-38**): `REL-17` (тег `rc-baseline-1` на `227c03a8`) + `GAP-B-1` (версионированный baseline находок, zero-new-error) |
| Волна 0 | **66** | было 55; **+12**: `FO-3a`, `FO-1` (разблок. D-40), `IDN-1` (разблок. D-41), `IDN-2` (D-42), `GAP-E-1`, `GAP-E-2` (SHOULD), `GAP-E-4`, `GAP-E-5` (D-46), `GAP-1`, `GAP-2`, `GAP-P-1`, `GAP-P-2`; **−1** (правка V-61b, C-1): `B2-12` ушла в Волну 3 — её ребро на `FO-4` нарушало критерий волны |
| Волна 1 | **4** | без изменений: V-02, V-03, V-04, V-04a |
| Волна 2 | **17** | было 18; **−1**: `V-10` (python) вынесена вне волн по **D-44**; порядок стеков читается как anystack → golang → swift |
| Волна 3 | **47** | было 39; **+9**: `FO-2`, `FO-3`, `FO-4`, `FO-5`, `B2-22`, `IDN-2a`, `T-B6-28`, `T-B6-29`, `GAP-3`; **−2**: `T-8` (предпосылка `RULES-D2` пост-релизная) и `T-14` (D-45) ушли вне волн; **+1** (правка V-61b, C-1): `B2-12` пришла из Волны 0 и идёт после `FO-4` |
| Волна 4 | **14** | было 11; **+3**: `E-22` (слой A), `E-23` (слой D), `FO-6` (миграционный адаптер шапок) |
| Волна 5 | **11** | было 9; **+2**: `REL-18` (evidence pack на одном чистом коммите), `GAP-4` (baseline + `0.9.0-next.4` по D-36) |
| Вне волн (пост-релиз / заблокировано / снято) | **20** | было 11; **−1** `T-2` (стала указателем, D-45); **+5** новых пост-релизных (`FO-7`, `IDN-3..IDN-6`); **+3** перенесённых (`V-10`, `T-8`, `T-14`); **+2** снятых (`B2-21` `ОТМЕНЕНА`, `GAP-E-3` `ПОГЛОЩЕНА`) |
...
**Инвариант «у каждой задачи есть волна»:** все 181 неуказательная строка отнесена либо к одной из волн −1…5, либо к «вне волн»; строк без волны нет — это проверено грепом, результат — в § Итог правок гейта адекватности, п. «Сверка консистентности».

### 5.5 По статусам (новый счётчик)

| Статус | Кол-во | Было (V-61) |
|---|---|---|
| `ГОТОВ К ВЫДАЧЕ` | **72** | 59 |
| `ЧЕРНОВИК` | **84** | 71 |
| `ЗАБЛОКИРОВАН` | **22** | 16 |
| дубль-указатель (владелец в другом треке) | **12** | 11 |
| `ОТМЕНЕНА` | **2** (`T-7` по D-32, `B2-21` по D-39) | 1 |
| `ПОГЛОЩЕНА` | **1** (`GAP-E-3` → `GAP-E-5` по D-46) | — |
| **Итого строк** | **193** | 158 |

**Прирост «готовых» (+13) — по формуле «+15 / −2»** (переписано правкой V-61b: прежний абзац объявлял «восемь новых задач» и перечислял девять, а оговорка «минус `GAP-E-2` и `T-B6-28`» ссылалась на id, которых в перечислении не было). Было 59, стало 72:

- **+15 строк получили `ГОТОВ К ВЫДАЧЕ`:** `REL-17`, `GAP-B-1` (обе — новая Волна −1); `FO-1` (разблокирована D-40), `IDN-1` (разблокирована D-41), `T-B6-29` (разблокирована D-43); новые вариант-независимые `FO-3a`, `IDN-2`, `GAP-E-1`, `GAP-E-4`, `GAP-E-5`, `GAP-1`, `GAP-2`, `GAP-P-1`, `GAP-P-2` (Волна 0) и `T-B6-28` (Волна 3, предпосылки в более ранней волне — см. правило выдачи в §1).
- **−2 строки ушли из «готовых» в `ЗАБЛОКИРОВАН`:** `V-10` (пост-релиз по D-44) и `T-8` (транзитивно D-33 через `RULES-D2`).
- 59 + 15 − 2 = **72**. `GAP-E-2` в прирост не входит: она новая, но со статусом `ЧЕРНОВИК (Волна 0, SHOULD)` — учтена в приросте `ЧЕРНОВИК`, а не «готовых».

Прирост «заблокированных» (+6) — целиком пост-релизные выносы по решениям оператора: `V-10` (D-44), `FO-7` (D-40), `IDN-3..IDN-6` (D-41), `T-8` (транзитивно D-33); `T-14` осталась заблокированной, но сменила основание на **D-36**.
