...
- [0. Карта потока](#sec-0)
- [1. Новичок](#sec-1)
  - [Сценарий 1 — «Выполни следующую задачу»](#scenario-1)
  - [Сценарий 2 — «Проект с нуля до первого тикета»](#scenario-2)
- [2. Повседневные сценарии](#sec-2)
  - [Сценарий 3 — Спроектировать новый scope](#scenario-3)
  - [Сценарий 4 — Продолжить / изменить спеку](#scenario-4)
  - [Сценарий 5 — Выполнить всю очередь](#scenario-5)
- [3. Продвинутые сценарии](#sec-3)
  - [Сценарий 6 — Критика артефактов](#scenario-6)
  - [Сценарий 7 — Аудит и починка](#scenario-7)
  - [Сценарий 8 — Инфраструктура и Go-роутинг](#scenario-8)
  - [Сценарий 9 — Целостность дерева](#scenario-9)
- [4. Справочники](#sec-4)
  - [4.1 Скиллы](#ref-skills) · [4.2 Директивы](#ref-directives) · [4.3 Артефакты](#ref-artifacts) · [4.4 CLI](#ref-cli) · [4.5 Связанные документы](#ref-docs)
...
**Правила:** критик видит **только** артефакт + родительскую спеку (изоляция); правит только текущий артефакт оркестратор; `CLEAN` завершает цикл сразу; после первого раунда критик проверяет только принятые правки и вызванные ими регрессии; пять раундов — аварийный предел. Непонимание само по себе не считается дефектом: блокирующая находка обязана сослаться на существующее требование и назвать конкретную поломку. По умолчанию репорятся только production-threatening дефекты (`Polish: off` — мелочи не гонят цикл).
...
**Когда:** хотите периодически проверять, что SDD-дерево не разъехалось.

```bash
# Read-only проверка целостности всего SDD-дерева (8 механических проверок + self-reflection)
@sdd-check
```
...
**Скилл:** `sdd-check` (read-only верификация, без директивы).
