...
/** @purpose Serializable adapter vocabulary emitted by deterministic authoring diagnostics. */
type CapabilityAdapterContract = {
  schema: 'sdd-capability-adapters/v1';
  adapters: Array<{
    id: string;
    capabilities: string[];
    artifacts: Array<{ id: string; path: string; order: number }>;
    layers: Array<{ capability: string; requires: string[] }>;
  }>;
};

/**
 * @purpose Teach agents the exact installed adapter ids and capability order without fixture search.
 * @param [registry] Installed/injected platform registry.
 * @returns Stable serializable adapter contract for CLI diagnostics and tests.
 */
export function deriveCapabilityAdapterContract(
    ...
    adapters: Object.values(registry)
      .map((adapter) => ({
        ...
        layers: adapter.layers
          .map((layer) => ({ capability: layer.capability, requires: [...layer.requires] }))
