...
const codes = (file: string, content: string): string[] => checkTicket(file, content).map((f) => f.code);

describe('checkTicket — phase graph + exec-log completeness', () => {
  it('clean ticket → no phase findings', () => {
    const c = codes('t.md', ticket({ rows: [{ id: 'P1' }, { id: 'P2', deps: 'P1' }], sections: ['P1', 'P2'] }));
    assert.ok(!c.some((x) => x.startsWith('SDD_PHASE') || x === 'SDD_DONE_PHASE_UNCHECKED'), c.join(','));
  });

  it('flags a phase dep that names an unknown phase', () => {
    const c = codes('t.md', ticket({ rows: [{ id: 'P1' }, { id: 'P2', deps: 'P9' }], sections: ['P1', 'P2'] }));
    assert.ok(c.includes('SDD_PHASE_DEP_UNRESOLVED'));
  });

  it('flags a cycle in phase deps', () => {
    const c = codes('t.md', ticket({ rows: [{ id: 'P1', deps: 'P2' }, { id: 'P2', deps: 'P1' }], sections: ['P1', 'P2'] }));
    assert.ok(c.includes('SDD_PHASE_DAG_CYCLE'));
  });

  it('flags an overview phase with no PHASE section', () => {
    const c = codes('t.md', ticket({ rows: [{ id: 'P1' }, { id: 'P2', deps: 'P1' }], sections: ['P1'] }));
  ...
  it('flags a DONE ticket with an unchecked phase', () => {
    const c = codes(
      't.md',
      ticket({ status: '[x] DONE', rows: [{ id: 'P1', status: '[x]' }, { id: 'P2', deps: 'P1', status: '[ ]' }], sections: ['P1', 'P2'] })
    );
