...
| `deployed-surface.tarball.golden.txt`    | `npm pack --dry-run --json` file list, **minus any `dist/**`path** (Vite build output — content-hashed chunk names and chunk count are not stable across builds of identical source, so freezing them fails deterministically after any`npm run build`; see `deployed-surface.test.ts`'s comment on the filter) | any task that changes `package.json` `files`/`.npmignore` or adds/removes a shipped file outside `dist/` |
