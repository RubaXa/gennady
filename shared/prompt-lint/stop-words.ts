...
export const STOP_WORDS: StopWord[] = [
  ...
  // Пойманы на описании MR!177 — оператор: «мозг плавится, это не читать». Только бесспорные случаи:
  // прижившиеся в русском IT слова (деградация, оркестрация, раунд) НЕ баним — точность важнее полноты.
  { re: stem('гейт'), why: 'калька (gate)', use: 'проверка / запрет / условие' },
  { re: stem('сентинел'), why: 'калька (sentinel)', use: 'служебное значение' },
  { re: stem('перевооруж'), why: 'калька (re-arm)', use: 'запустить заново / поставить заново' },
  { re: stem('поглощающ'), why: 'калька (absorbing state)', use: 'застревает навсегда' },
  { re: w('фикс(?:а|у|ом|е|ы|ов|ами)?'), why: 'сленг (fix)', use: 'правка / исправление' },
  { re: stem('блокер'), why: 'сленг (blocker)', use: 'блокирующий дефект' },
  { re: stem('терминальн'), why: 'калька (terminal)', use: 'окончательный / конечный' },
];
