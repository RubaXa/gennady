// @file: Golang stack e2e suite — runs every fixtures/config/* declaration against a real toolchain.
// @consumers: CI
// @tasks: TSK-95
...
const STRICT = process.env.CONFIG_E2E_STRICT === '1' || process.env.STACK_E2E_STRICT === '1';
...
describe('stack e2e — config', { skip: !IS_E2E, concurrency: false }, () => {
  let ctx: StackE2eContext;
  const skipped: string[] = [];

  before(() => {
    ctx = setupStackSuite('config', ['go', 'configci-lint']);
    const versions = [...ctx.toolchains.values()]
      .map((tool) => `${tool.id} ${tool.available ? tool.version : '✗'}`)
      .join(' · ');
    console.info(`[config] toolchains: ${versions}`);
  });

  after(() => {
    ctx?.cleanup();
    if (skipped.length > 0) {
      console.info(`[config] skipped: ${skipped.join(', ')}`);
      console.info('[config] → STACK_E2E_STRICT=1 turns missing toolchains into failures');
    }
  });
