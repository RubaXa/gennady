// @file: Golang stack e2e suite — runs every fixtures/node/* declaration against a real toolchain.
// @consumers: CI
// @tasks: TSK-95
...
const FIXTURES = path.join(import.meta.dirname, 'fixtures', 'node');
...
describe('stack e2e — node', { skip: !IS_E2E, concurrency: false }, () => {
  let ctx: StackE2eContext;
  const skipped: string[] = [];

  before(() => {
    ctx = setupStackSuite('node', ['go', 'nodeci-lint']);
    const versions = [...ctx.toolchains.values()]
      .map((tool) => `${tool.id} ${tool.available ? tool.version : '✗'}`)
      .join(' · ');
    console.info(`[node] toolchains: ${versions}`);
  });

  after(() => {
    ctx?.cleanup();
    if (skipped.length > 0) {
      console.info(`[node] skipped: ${skipped.join(', ')}`);
      console.info('[node] → STACK_E2E_STRICT=1 turns missing toolchains into failures');
    }
  });
