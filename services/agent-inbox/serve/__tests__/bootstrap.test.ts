...
describe('bootstrap — runtime safety boundary', () => {
  it('real effects without allowlist fail before adapters start', async () => {
    ...
    await assert.rejects(
      ...
      (error: unknown) => {
        ...
        assert.match(error.bootState.error ?? '', /effect allowlist identity/i);
        return true;
      }
    );
