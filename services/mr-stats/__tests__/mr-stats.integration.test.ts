...
describe('mr-stats integration', () => {
  ...
  it(
    'mr-stats on MR !14 returns valid JSON with all categories',
    { skip: !ctx.allTools ? 'glab/cloc/jscpd not installed' : false },
    async () => {
      ...
      // skip: CI-safe — tools may not be present in CI
      if (!ctx.allTools) return;
