# Task: IE-realeval — Adaptive real validation and product acceptance
...
- **Scope:** agent-inbox
- **Module:** inbox-eval
- **Dependencies:** IV-vcs-port, IP-control, IQ-actions, IA-proj, IM-cases, AI-cutover, ID-cockpit
- **Spec References:** [Eval inventory](./inbox-eval.spec.md#3-entity-inventory-closed-world), [DbC](./inbox-eval.spec.md#5-module-contracts-dbc), [Pipeline DbC](../inbox-pipeline/inbox-pipeline.spec.md#5-module-contracts-dbc), [Root acceptance](../agent-inbox.spec.md#acceptance-after-downstream-regeneration)
...
- **When** IQ-actions `TaskExecutorPort` executes through the shippable entrypoint and the process restarts
...
| `npx tsx cli/gennady.ts lint services/agent-inbox/modules/inbox-eval services/agent-inbox/serve/run-mode.ts e2e/inbox-serve`                                                                                                                                                                                                               | scoped DbC, headers and anchor pairing                    | extra |
| `! rg --follow --no-heading -n -e "^\s*enum " -e "^\s*namespace " -e "^\s*private " -e "#[a-zA-Z_]+\s*[:=]" -e "\bconsole\." -t ts services/agent-inbox/modules/inbox-eval services/agent-inbox/serve/run-mode.ts e2e/inbox-serve`                                                                                                         | typescript-rules: forbidden constructs                    | extra |
| `node --import tsx --test --experimental-test-module-mocks services/agent-inbox/modules/inbox-eval/__tests__/*.test.ts`                                                                                                                                                                                                                    | testing-common, node-test: scoped tests                   | extra |
| `node --import tsx --test --experimental-test-module-mocks --experimental-test-coverage services/agent-inbox/modules/inbox-eval/__tests__/*.test.ts`                                                                                                                                                                                       | testing-common, node-test: contract coverage              | extra |
| `! rg --no-heading -n -e "Step \d" -e "\.message.*\.includes\(" -e "let\s+threw\s*=" -t ts services/agent-inbox/modules/inbox-eval/__tests__`                                                                                                                                                                                              | node-test: forbidden test patterns                        | extra |
| `npm run inbox-serve:build && npm run test:e2e:review-flow`                                                                                                                                                                                                                                                                                | playwright-cli, playwright-e2e: real shippable entrypoint | probe |
| `npx prettier --check services/agent-inbox/modules/inbox-eval services/agent-inbox/serve/run-mode.ts e2e/inbox-serve tasks/agent-inbox/inbox-eval/inbox-eval.task-183.md tasks/agent-inbox/README.md`                                                                                                                                      | changed runtime/tests/e2e/ticket/tracker format           | extra |
| `for sdd_section in META PHASES_OVERVIEW PHASE_P1 PHASE_P2 BDD VERIFICATION TEST_COVERAGE EXECUTION_LOG; do test "$(rg -c "^<!--SECTION:${sdd_section}-->$" tasks/agent-inbox/inbox-eval/inbox-eval.task-183.md)" = 1 && test "$(rg -c "^<!--/SECTION:${sdd_section}-->$" tasks/agent-inbox/inbox-eval/inbox-eval.task-183.md)" = 1; done` | normative task-anchor check                               | extra |
| `ai/skills/sdd-execute/scripts/sdd check --task IE-realeval`                                                                                                                                                                                                                                                                               | SDD ticket/tracker/DAG integrity                          | extra |
| `git diff --check -- services/agent-inbox/modules/inbox-eval services/agent-inbox/serve/run-mode.ts e2e/inbox-serve tasks/agent-inbox/inbox-eval/inbox-eval.task-183.md tasks/agent-inbox/README.md`                                                                                                                                       | changed-scope diff integrity                              | extra |
...
- Deferred Test Ownership: AI-cutover concurrency and recovery runtime proof → `full-flow.blackbox.test.ts` :: `two merge requests progress without global blocking`
- Deferred Test Ownership: IP-control coverage and safe automation runtime proof → `review-orchestrator.integration.test.ts` :: `no incomplete blocked stale or semantically unfinished round publishes downstream`
- Deferred Test Ownership: IQ-actions partial effect runtime proof → `review-effect-coordinator.integration.test.ts` :: `real hybrid effects record failure independent continuation reconciliation and safe retry`
...
- Deferred Test Ownership: IQ-actions task executor runtime proof → `review-task-executor.integration.test.ts` :: `task executor port e2e preserves lane order parallel progress and crash recovery`
...
- [ ] `<ts>` ver `ai/skills/sdd-execute/scripts/sdd check --task IE-realeval` → `<pass|fail>` exit=`<code>`
- [ ] `<ts>` ver `git diff --check -- services/agent-inbox/modules/inbox-eval services/agent-inbox/serve/run-mode.ts e2e/inbox-serve tasks/agent-inbox/inbox-eval/inbox-eval.task-183.md tasks/agent-inbox/README.md` → `<pass|fail>` exit=`<code>`
- [ ] `<ts>` DONE
      **Handoff →** artifacts: [...]; decisions: [...]; open: [...]
...
- The eleven pipeline runtime hooks deferred by IP-control are separate named shippable-entry e2e cases, not one umbrella flow; every case requires product-written artifacts and PASS.
- The real `TaskExecutorPort` implementation belongs to IQ-actions, while this dependent task owns its separately named shippable-entry E2E and product-written acceptance artifacts.
