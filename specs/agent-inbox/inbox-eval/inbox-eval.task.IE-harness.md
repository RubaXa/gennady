# Task: IE-harness — inbox-eval: харнесс S1–S8 + метрики автономии
...
- **Scope:** `agent-inbox`
- **Module:** `inbox-eval`
- **Dependencies:** IP-review, ID-spa, AI-seed
- **Spec References:**
  - Module spec: [inbox-eval](./inbox-eval.spec.md) §2–§5
...
- **Inputs:** IP-review (пайплайн), ID-spa (дашборд для скриншотов), IC-decision (журнал/датасет)
...
| `npm test -- services/agent-inbox/modules/inbox-eval/__tests__/` | node-test        | probe |
