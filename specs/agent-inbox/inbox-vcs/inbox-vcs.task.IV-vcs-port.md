# Task: IV-vcs-port — Unified GitLab read, sync, effects and reconciliation
...
- **Scope:** agent-inbox
- **Module:** inbox-vcs
- **Dependencies:** IC-state
- **Spec References:** [Inventory](./inbox-vcs.spec.md#3-entity-inventory-closed-world), [Contracts](./inbox-vcs.spec.md#5-module-contracts-dbc)
...
- **Target Files:** `services/agent-inbox/modules/inbox-vcs/`, `services/agent-inbox/modules/inbox-core/vcs-inbox.real.ts`, `services/agent-inbox/modules/inbox-core/types/review-event.type.ts`, `services/vcs-client/`, `cli/cmd/inbox/help.ts`, `specs/agent-inbox/inbox-vcs/inbox-vcs.spec.md`
- **Inputs:** IC-state handoff
...
- **Target Files:** `services/agent-inbox/modules/inbox-vcs/__tests__/`, `services/vcs-client/__tests__/gitlab/vcs-gitlab-client.observation.test.ts`, `services/vcs-client/gitlab/__tests__/vcs-gitlab-inbox.test.ts`, `services/agent-inbox/modules/inbox-core/__tests__/vcs-inbox.real.blackbox.test.ts`
- **Inputs:** P1 handoff
...
- adapter parity: memory → `vcs-port.contract.test.ts` :: `memory runtime passes the common read/effect port contract`
- adapter parity: readonly → `vcs-port.contract.test.ts` :: `readonly runtime passes the common read/effect port contract`
- adapter parity: real GitLab → `vcs-port.contract.test.ts` :: `real-gitlab runtime passes the common read/effect port contract`
...
- legacy discovery compatibility → `vcs-inbox.real.blackbox.test.ts` :: `should discover, dedup, and role-merge MRs via the real GraphQL path`
- terminal reviewer flow → `full-flow.blackbox.test.ts` :: `drives a real reviewer MR to a terminal state with both backends faked at the network layer`
...
- [x] `2026-08-11T07:23:17Z` DONE
      **Handoff →** artifacts: common memory/readonly/real-GitLab port suite, dedicated endpoint tests, permission/reconciliation/sync tests and canonical real-effect case name; attribution excludes every IO-runtime inbox-opencode path. Open: fresh isolated round-2 audit required.
...
- [x] `2026-08-11T07:41:26Z` P2 DONE
      **Handoff →** artifacts: source-specific bounded-query adapter blackbox, assigned-source assertion, full-flow legacy regression; attribution: compatibility tests are IV-vcs-port, existing TSK-150/167/170 ownership remains; open: restarted fresh audit-r2 required.
...
- [x] `2026-08-11T08:04:29Z` P2 DONE — appended `IV-vcs-port` ownership to both affected legacy tests; preserved all 16 SECTION markers at column zero with non-rendering Markdown reference boundaries; ticket Prettier check passes.
...
- [x] `2026-08-11T08:04:29Z` DONE — audit-r2 F-01…F-04 remediated without inbox-opencode/IO-runtime edits; fresh isolated recovery audit required.
...
- [x] `2026-08-11T08:19:00Z` DONE — recovery audit F-01 remediated without discovery, legacy entrypoint, effect semantics or IO-runtime changes; fresh isolated audit required.
<!--/SECTION:EXECUTION_LOG-->
