# Task: IV-gitlab — inbox-vcs: двухъярусный sync + ось внимания + эффекты
...
- **Scope:** `agent-inbox`
- **Module:** `inbox-vcs`
- **Dependencies:** IC-journal
- **Spec References:**
  - Module spec: [inbox-vcs](./inbox-vcs.spec.md) §2–§6
...
- **Inputs:** IC-journal P1 handoff (EventJournal: gitlab_event writer; JournalPort контракт — inbox-core §2)
...
| `npm test -- "services/agent-inbox/modules/inbox-vcs/__tests__/*.test.ts"` | node-test        | probe |
...
- row 1: reviewer, head not reviewed → `attention.test.ts` :: `row 1 — reviewer, head not reviewed → ⏳`
- row 2: responses after me → `attention.test.ts` :: `row 2 — threads have responses after me → 💬`
- row 2: my threads await response → `attention.test.ts` :: `row 2 — my threads awaiting response → 💬`
- row 3: author unresolved threads → `attention.test.ts` :: `row 3 — author with unresolved reviewer threads → 💬`
- row 4: new commits → `attention.test.ts` :: `row 4 — new commits after last review → 🔀`
- row 5: approval missing → `attention.test.ts` :: `row 5 — all clear, only my approval missing → ✅`
- row 6: nothing left → `attention.test.ts` :: `row 6 — nothing left to do → 😴`
- resolved threads excluded → `attention.test.ts` :: `resolved threads are excluded from active thread detection`
...
- [x] `2026-08-06T10:15:59Z` decision `compareSha`=reserved ← оставлен в VcsPort, помечен: зарезервирован для IP-review (delta)
...
- [x] `2026-08-08T02:10:00+03:00` reconcile runtime ownership → changed production roots carry IV-gitlab ownership; `VcsGitlabPort` declares its direct `VcsPort` implementation contract.
