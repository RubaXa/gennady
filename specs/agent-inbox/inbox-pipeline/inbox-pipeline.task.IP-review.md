# Task: IP-review — inbox-pipeline: план-шаблон + 3 слоя + линзы + coverage + синтез + хвосты
...
- **Dependencies:** IQ-executor
- **Spec References:**
  - Module spec: [inbox-pipeline](./inbox-pipeline.spec.md) §2–§8
...
- **Inputs:** IQ-executor (queue), IO-session (сессии/tool-trace), IV-gitlab (changeset/discussions)
...
| `npx tsx --test services/agent-inbox/modules/inbox-pipeline/__tests__/*.test.ts` | node-test        | extra |
...
- [x] `2026-08-08T00:00:00Z` fix ownership ← IP-review добавлен append-only в изменённые runtime/queue заголовки, включая TaskRegistry
- [x] `2026-08-08T00:00:00Z` DONE
