# Task: IO-session — inbox-opencode: TTL-паркинг + единый пул + промпт-компиляция
...
- **Spec References:**
  - Module spec: [inbox-opencode](./inbox-opencode.spec.md) §2–§5
...
| `npm test -- services/agent-inbox/modules/inbox-opencode/__tests__/` | node-test        | probe |
...
- session registry сохраняет полную entry → `session-registry.test.ts` :: `should store entry by sessionId with taskId, mr, artifacts and model`
...
- пул → `session-pool.test.ts` :: `GIVEN pool with maxSessions=3 WHEN create() THEN returns sid and activeCount=1`
...
- лесенка: first failure → `session-lifecycle.test.ts` :: `should return continue on first non-OK outcome`
- лесенка: second failure → `session-lifecycle.test.ts` :: `should return restart on second non-OK outcome`
- лесенка: third failure → `session-lifecycle.test.ts` :: `should return accept on third non-OK outcome`
