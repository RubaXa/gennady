# Task: IA-rest-sse — inbox-api: REST/SSE + DTO-проекции
...
- **Scope:** `agent-inbox`
- **Module:** `inbox-api`
- **Dependencies:** IV-gitlab, IQ-executor
- **Spec References:**
  - Module spec: [inbox-api](./inbox-api.spec.md) §2–§4
...
- **Inputs:** IV-gitlab (sync-снимок), IQ-executor (очередь), IC-decision (журнал/lastReadAt)
...
| `npm test -- services/agent-inbox/modules/inbox-api/__tests__/` | node-test        | probe |
...
- degraded → `board-projection.test.ts` :: `degraded sync emits board_hint via SseHub to all channels`
...
- [ ] `2026-08-07T05:55:46Z` intro `StateRouter` ← GET /api/state?mr=<ref> → батч {card?, queue, widgets}
...
- [x] `2026-08-08T00:10:07Z` intro `SeedContract` ← AI-seed fixture now uses canonical task event payload and asserts v2 board DTO instead of internal stage
...
```
@audit task=IA-rest-sse round=1 after-exec-round=1 triggered-reopen=Round-2 status=FAIL counts=B2·M4·m1·I1 phases_to_fix=[P1,P2]
