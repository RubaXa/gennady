# Task: AI-seed — test-infra: seed-DSL + контракт-сьют портов + кассеты
...
- **Scope:** `agent-inbox`
- **Module:** N/A (test infrastructure; потребители: ID-spa P3, IE-harness)
- **Dependencies:** IC-journal, IV-gitlab
- **Spec References:**
  - Testing doctrine: [inbox-dashboard §5.1](./inbox-dashboard/inbox-dashboard.spec.md) · drift-sentinel: [inbox-eval §4.2](./inbox-eval/inbox-eval.spec.md)
- **Implementation Evidence:** Reuse: `utils/test/mock-http.ts` (undici MockAgent, D-212)
...
- **Inputs:** IC-journal (журнал), IV-gitlab (порты)
...
| `npm test -- services/agent-inbox/test/__tests__/` | node-test        | probe |
...
- [x] `2026-08-08T00:24:00+03:00` ver `npx tsx cli/gennady.ts lint <AI-seed files> && npx prettier --check <AI-seed files> && git diff --check` → `pass`
- [x] `2026-08-08T00:24:00+03:00` DONE
...
- [x] `2026-08-08T00:24:00+03:00` DONE — AI-seed remediation ready for independent audit.
...
- [x] `2026-08-08T00:30:00+03:00` DONE — AI-seed negative BDD path ready for independent audit.
<!--/SECTION:EXECUTION_LOG-->
...
```
@audit task=AI-seed round=1 after-exec-round=2 triggered-reopen=Round-3 status=FAIL counts=B0·M3·m0·I0
