...
| AI-13 | Полный VCS-инструментарий с политикой «когда что»: `inbox`/`vcs-worktree`/`review-issues`/`vcs-reply`/`vcs-approve`. `vcs-reply` несёт reply/line/discussion/**resolve** (`{discussionId,resolve:true/false}`, ± body). Резолв = «вопрос снят» (не «прочитал»); approve — только без открытых блокирующих замечаний; approve по MR, resolve по треду                                                                     |
