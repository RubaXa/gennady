# Task: ID-spa — inbox-dashboard: загрузка / доска / лента / чат-колонка
...
- **Dependencies:** IA-rest-sse, AI-seed
- **Spec References:**
  - Module spec: [inbox-dashboard](./inbox-dashboard.spec.md) §2–§4
...
- **Inputs:** IA-rest-sse (REST/SSE контракты)
...
- **Given** serve на temp stateDir с seed-фикстурой AI-seed (2 МР в заданных состояниях; dry-run эффектов)
...
| `npm test -- services/agent-inbox/modules/inbox-dashboard/__tests__/*.test.tsx`  | node-test                 | probe |
| `npx playwright test --config=e2e/inbox-serve/playwright.dashboard-v2.config.ts` | playwright-e2e            | extra |
| `npm run inbox-serve:build`                                                      | дисциплина бандла (D-204) | extra |
...
- [x] `2026-08-08T05:39Z` DONE — Playwright owned a temporary `GENNADY_STATE_DIR`, invoked real `gennady inbox serve --mocks` (no Vite/no route interception), seeded AI-seed journal snapshots, killed the process to create an actual SSE TCP disconnect, restarted it, and captured five staged screenshots.

#### Round close

- [x] `2026-08-08T05:40Z` DONE
      **Handoff →** audit: fresh independent audit required; evidence: `.codex-agent-status/sdd-execute-batch-20260808T013000/ID-spa/execute-r3/`.
...
- [ ] `2026-08-08T02:47Z` BLOCKED only on mandatory P3 real-data visual proof; P1/P2 are independently green. Evidence: `.codex-agent-status/sdd-execute-batch-20260808T013000/ID-spa/execute-r4/`.
