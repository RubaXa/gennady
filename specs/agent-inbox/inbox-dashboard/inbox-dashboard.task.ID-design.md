# Task: ID-design — inbox-dashboard: приведение UI к Carbon & Steel (design-system compliance)
...
- **Task-ID:** ID-design
- **Status:** [x] DONE
- **Purpose:** ID-spa собрал работающий v2-дашборд, но НЕ по утверждённой дизайн-системе: палитра GitHub-dark (`#0d1117/#161b22/#30363d`, синий `#58a6ff`) вместо Carbon & Steel (`#121416`, primary `#fc6d26`, бордеры `#2e3440`); доска — вертикальные секции вместо kanban-колонок с акцент-барами и rail'ом; нет severity-строк находок, sticky decision-бара, step-flow плана; шрифты Geist/JetBrains Mono не забандлены. Привести UI к спеке ДО финальной приёмки (иначе приёмочные скриншоты зафиксируют неправильный UI).
- **Scope:** `agent-inbox`
- **Module:** `inbox-dashboard`
- **Dependencies:** ID-spa
- **Spec References:**
  - [inbox-dashboard.spec.md](./inbox-dashboard.spec.md) §4 (состав компонентов)
- **Design References:**
  - [design-system.md](./design-system.md) — токены, палитра, типографика, elevation, shapes, компоненты (НОРМАТИВНО)
  - [ux-mockups.md](./ux-mockups.md) §1, §2′, §3, §4, §5′, §8′, sticky-бар, quick-chips (нормативно по СОСТАВУ)
...
- role lanes остаются в UI → `BoardPage.test.tsx` :: `renders a role Kanban block per role, active and inactive`
- dashboard visual flow → `dashboard-v2.spec.ts` :: `(1) first frame → (2) board lanes + accent bars → (3) rail close-up`
<!--/SECTION:TEST_COVERAGE-->
