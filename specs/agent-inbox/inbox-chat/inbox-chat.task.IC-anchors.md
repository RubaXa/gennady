# Task: IC-anchors — inbox-chat: якоря + operator-сессия + мутации
...
- **Scope:** `agent-inbox`
- **Module:** `inbox-chat`
- **Dependencies:** IA-rest-sse
- **Spec References:**
  - Module spec: [inbox-chat](./inbox-chat.spec.md) §2–§5
...
- **Inputs:** IA-rest-sse (роутеры chat/mutate), IQ-executor (маршрут сессий)
...
| `npx tsx --test services/agent-inbox/modules/inbox-chat/__tests__/*.test.ts` | node-test        | extra |
...
- undo LIFO → `mutation-flow.test.ts` :: `cas conflict is visible and undo is lifo per artifact`
...
- [x] Add `IC-anchors` ownership headers to the production chat/mutate routers.
...
- [x] `2026-08-08T01:45:45Z` Preserve prior task owners and append `IC-anchors` to the `@tasks` headers of `MutationApplier`, `HttpServer`, and production `bootstrap`; these are P1 target files that now participate in the durable mutation runtime.
...
- [x] `2026-08-08T02:02:15Z` Add `IC-anchors` append-only ownership to `chat.router.test.ts`; make P2 target files and BDD coverage name both production HTTP integration suites exactly.
