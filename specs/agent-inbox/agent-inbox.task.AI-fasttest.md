# Task: AI-fasttest — test-suite health: изоляция тяжёлых integration-тестов
...
- **Task-ID:** AI-fasttest
- **Status:** [ ] TODO
- **Purpose:** Полная сьюита `npm test` не завершается (>10 мин) после волны IA-rest-sse/163/166 — тяжёлые integration-тесты (реальные серверы/SSE/git) деградируют полное расписание. Вынести их за отдельный скрипт и вернуть основной гейт ≤ 2 мин.
- **Scope:** `agent-inbox`
- **Module:** N/A (test infrastructure; блокирует приёмку IE-harness)
- **Dependencies:** None
- **Spec References:**
  - Doctrine: [inbox-dashboard §5.1](./inbox-dashboard/inbox-dashboard.spec.md) (гранулярность тестов)
...
| -------------------------- | ---------------- | ----- |
| `npm test` (≤120с, 0 fail) | nodejs-npm-setup | probe |
| `npm run test:integration` | node-test        | probe |
| `npm run lint:contracts`   | baseline         | extra |
...
- полный детерминированный прогон → `test-topology.test.ts` :: `check reports the exact list counts`
- изоляция experimental-контура → `test-topology.test.ts` :: `experimental owns exactly the D-60 layer, run in isolation with no coverage/network guard`
- явные исключения не теряются → `test-topology.test.ts` :: `GAP-2: excluded prints every explicit exclusion with an owner and a reason`
<!--/SECTION:TEST_COVERAGE-->
...
- [x] 2026-08-08T08:06:27Z discovery bootstrap.test.ts зависает ← pre-existing: spawns реальный opencode serve без таймаута разрешения, не вызвано AI-fasttest
- [x] 2026-08-08T08:06:27Z insight bootstrap.test.ts hang блокирует integration-прогон → skip-marked с [D-216], нужен отдельный фикс (TSK-115/IO-session)
...
- [x] 2026-08-08T14:00Z fix bootstrap after(): server.stop() → gracefulShutdown (scheduler/opencode таймеры иначе держат процесс); остаточный open-handle срезан --test-force-exit в test:integration (root-cause → IA-health)
...
- [x] 2026-08-08T14:00Z re-reason оставшиеся skip: run-mode ×3 + full-flow + orphan → [IA-health] RED/superseded-by-v2, не D-216
