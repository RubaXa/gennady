...
```jsonc
{
  ...
  // Документация: спека плагина, которую читают агент, ревьюер и SDD-каскад.
  "spec": "spec.md",
...
Он означает: код в `plugin.ts`, спека в `spec.md` (конвенциональные дефолты), остальных surface'ов нет.
...
| `spec`        | нет   | `spec.md`      | Спека плагина                                         |
...
| Пакет           | `npm pack`, `prepare-publish-artifacts`     | `files` включает `plugins/**`; ассеты плагинов копируются в `dist/` как `ai/**`       |
...
3. **Проверка submodule-теста руками**: в scratch-клоне превратить `plugins/golang/` в submodule и прогнать полный набор. Пока это не сделано, критерий §1 не подтверждён.
...
**Structural changes:** `tsconfig.json` (+`plugins/**`, исключить `**/e2e/fixtures/**`), `.prettierignore`, `lint:contracts` таргеты, `package.json#files` и `scripts/prepare-publish-artifacts.ts`, `scripts/stack-e2e.ts` (динамические наборы), `specs/README.md` (регистрация scope).
