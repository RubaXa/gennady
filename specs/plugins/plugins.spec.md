...
| --------------- | ------------------------------------------- | ------------------------------------------------------------------------------------------ |
| Реализация      | `stack-registry`                            | объекты плагинов из статического `plugins/index.ts` (D-SP-009); порядок — из резолвера     |
...
| Директивы       | `gennady sync`                              | стейджатся в `ai/directives/**` зеркалом поддерева (D-SP-008); сама команда не меняется    |
| Скиллы          | `gennady sync-skills`                       | стейджатся в `ai/skills/**` (D-SP-008)                                                     |
...
**Почему `sync`/`sync-skills` не получили lookup'ов.** Обе команды читают **установленный пакет** (`resolvePackageDir` → `node_modules/gennady/ai/...`), а не репозиторий: из этого чекаута `gennady sync infra` не работает вовсе — «gennady package not found». Значит требование к плагину одно: его директивы и скиллы обязаны оказаться в опубликованном пакете, и это стейджинг (D-SP-008), а не мультикорневой обход. Мультикорневой сканер здесь было бы нечем проверить.
...
- **Consequences:** `prepare-publish-artifacts` стейджит объявленные манифестом surface'ы в `ai/**` (директивы и скиллы — зеркалом поддерева, чтобы `gennady sync infra` продолжал находить директиву плагина), `cleanup-publish-artifacts` удаляет ровно эти пути, а не по маске. Тест публикации сверяет `npm pack --dry-run` со списком из манифестов.
- **Побочный эффект, который стоит зафиксировать:** стейджинг в `ai/**` делает ту самую ловушку **структурно невозможной** — вложенный `.gitignore` вычитает из `plugins/**`, а мы `plugins/**` не публикуем вовсе. Проверено: с `plugins/golang/.gitignore`, содержащим `directives`, тарбол остаётся полным. То есть тест ловит не ловушку, а её причину — исчезновение стейджинга или правку `files`; это подтверждено отключением стейджинга (тест краснеет).
...
- ~~**Submodule-тест руками (§1).**~~ Выполнен. `plugins/golang/` превращён в отдельный репозиторий (со своим `.gitignore`: `dist`, `*.log`, `node_modules`) и подключён в клон хоста настоящим submodule'ом (`.git` — файл). Результат: резолвер видит оба плагина, `tsc` чист, locality/parity/resolver-тесты зелёные, `verify --plan` планирует, **все 44 go-фикстуры проходят из submodule'а**, тарбол остаётся полным несмотря на `.gitignore` плагина. `deinit` → плагин просто отсутствует без ошибок, а `tsc` падает на `plugins/index.ts` с внятным TS2307 — ровно то поведение, которое описывает D-SP-009.
