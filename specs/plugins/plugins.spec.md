...
- ~~**Динамический импорт против бандла.**~~ Закрыт на шаге 3: built-in'ы регистрируются статическим `plugins/index.ts` (D-SP-009), кодогенерация не понадобилась. Загрузка **внешнего** плагина в собранном CLI остаётся за D-STACK-001 — у него отдельная проблема: shipped CLI не умеет исполнять `.ts`.
