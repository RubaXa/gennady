...
- **Rejected alternatives:** `files: ["plugins/**"]` (см. выше); `!plugins/**/.gitignore`-негативы (npm вычитает по вложенному ignore, а не по имени файла — негатив не отменяет вычитание); требовать от плагинов не иметь `.gitignore` (несовместимо с «плагин может быть отдельным репозиторием»).
