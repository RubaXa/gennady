# Task: MS-config — Bootstrap: classifier config + CLI scaffolding
...
- **Spec References:**
  - Bootstrap Requirements: [mr-stats spec §8](./mr-stats.spec.md)
  - Categories definition: [mr-stats spec §4.1](./mr-stats.spec.md)
  - Architecture / canonical order: [mr-stats spec §5](./mr-stats.spec.md)
  - ClassifierRules contract: [mr-stats spec §10](./mr-stats.spec.md)
  - YAML example: [mr-stats spec §8](./mr-stats.spec.md)
- **Implementation References:** CLI registration: [cli/AGENTS.md](../../cli/AGENTS.md)
...
| ----------------------------------------------------------------------------------------------------- | ---------------- | ----- |
| `node -e "if (!require('fs').existsSync('services/mr-stats/classifier-rules.yaml')) process.exit(1)"` | P1               | probe |
| `npm run type-check`                                                                                  | typescript-rules | extra |
| `npm run lint`                                                                                        | P1               | extra |
...
- Scenario «Classifier config is valid and loadable» → `services/mr-stats/__tests__/classifier-rules.test.ts` :: `содержит ровно 10 категорий`
...
- Scenario «CLI invoked with URL probes runtime» → `cli/cmd/mr-stats/__tests__/mr-stats.cmd.test.ts` :: `mr-stats with URL exits early when glab is unavailable`
<!--/SECTION:TEST_COVERAGE-->
