...
| **Sandboxed gate**      | Гейт с `sandbox: true`: команда исполняется в эфемерной реплике рабочего дерева (`git worktree` + незакоммиченные изменения + untracked-файлы, забейзлайненные локальным коммитом реплики; ignored-файлы намеренно не реплицируются — семантика fresh clone, D-STACK-012). Вердикт = exit-код команды **и** порезультатный `git status` реплики: непустой → FAIL со списком файлов («drift»). Реальное дерево не затрагивается по построению. |
