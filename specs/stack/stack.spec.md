...
| **VIOLATION**           | Гейт **без** `driftMeansFailure: true` оставил реплику грязной — нарушение контракта «гейт наблюдает» (D-STACK-005). Отдельный статус: не FAIL по коду и не ENV_FAIL; отчёт называет гейт и файлы и предлагает перенести мутацию в fixers. Реплика сбрасывается к baseline перед следующим гейтом.                                                                                                                                                                                                                                                                                 |
...
- **`fixer` — поле гейта, а не отдельная секция.** Мутирующая операция всегда принадлежит конкретной проверке: «что чинит `golang:generate`» — вопрос без смысла в отрыве от гейта `generate`. Поэтому fixer объявляется прямо на гейте (`Cmd`, §4.5.1), а его квалифицированное имя — имя гейта: `gennady fix golang:generate`. Отдельной секции `fixers` и отдельного фасета `fix` у плагина нет: гейт и его лечение живут рядом и не могут разъехаться по id.
...
### 4.5.1 `Cmd` / `CmdSpec` — одна форма команды на три места употребления
...
Среда исполнения стека, живущая в ignored-путях (node: `node_modules`), symlink'уется по декларации плагина `sandboxLinks` — это среда, не состояние дерева, D-STACK-012 не ослабляется (golang не декларирует ничего). Пути реплики в выводе гейтов переписываются на реальные — иначе агент правит файлы в испаряющейся директории. Побочный выигрыш: `driftMeansFailure` в GateSpec становится тривиальным — реплика уже есть, флаг лишь выбирает семантику drift-вердикта (паттерн «быстрый скоуп-дефолт + канонический прогон адресно через skipGates + --only»).
...
### FR-STACK-16 — снимать группу процессов, а не только прямого ребёнка
