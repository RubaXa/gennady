...
| **Sandboxed gate**      | Гейт с `sandbox: true`: команда исполняется в эфемерной реплике рабочего дерева (`git worktree` + незакоммиченные изменения + untracked-файлы, забейзлайненные локальным коммитом реплики). Вердикт = exit-код команды **и** порезультатный `git status` реплики: непустой → FAIL со списком файлов («drift»). Реальное дерево не затрагивается по построению. |
...
type Gate = {
  ...
  /**
   * Исполнить в песочнице-реплике рабочего дерева (см. §2 «Sandboxed gate»).
   * Для команд, существующих только в мутирующей форме (кодогенерация):
   * мутации происходят в реплике, drift становится вердиктом, реальное дерево нетронуто.
   */
  sandbox?: boolean;
...
| `createTreeReplica`                            | Function     | Эфемерная реплика рабочего дерева для sandboxed-гейтов (§2); baseline-коммит внутри реплики           |
...
- Postconditions: RUN-ALL; SUPPRESS-ON-SUCCESS; `outputMeansFailure: true` + exit 0 + непустой stdout → `fail`; `sandbox: true` → команда исполняется в реплике рабочего дерева (включая незакоммиченные и untracked изменения), exit 0 + непустой drift-статус реплики → `fail` со списком файлов, реальное дерево байт-в-байт нетронуто, отказ создания реплики → `env-fail`; превышение `timeoutMs` → `timeout`; любой сработавший `envFail`-предикат → `env-fail` + запрет менять код в отчёте; `ok === true` ⇔ все исполненные гейты `pass`.
