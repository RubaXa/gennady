...
| Матрица                                        | Владелец                                                            |
| ---------------------------------------------- | ------------------------------------------------------------------- |
| Go-фикстуры (28)                               | [`plugins/golang` §7](../../../plugins/golang/specs/golang.spec.md) |
...
- **Risk accepted:** поля `--json` становятся тестируемым контрактом и не могут переименовываться молча. Это желаемое свойство — stack.spec §8.5 уже объявляет их стабильными для оркестраторов.
...
- **Serves:** [`plugins/golang`](../../../plugins/golang/specs/golang.spec.md), [`plugins/node`](../plugins/node/node.spec.md) — их матрицы исполняются этим механизмом
