# Task: DP-snaps — Обновить тесты и snapshot-ы под новую схему
...
- **Task-ID:** DP-snaps
- **Status:** [ ] TODO
- **Purpose:** Актуализировать все snapshot-тесты под изменения из DP-fields и DP-jsdoc. Новые кейсы: `@implements`, `format`, `inline`, обновлённый `CONTRACT_ORDER`.
- **Scope:** dbc
- **Module:** dbc-parser
- **Dependencies:** DP-jsdoc
- **Spec References:**
  - Scope FR-06…FR-16: [`Requirements`](../dbc.spec.md#31-functional-requirements)
  - Module: [`Handoff`](./dbc-parser.spec.md#9-handoff-to-task-scaffolding)
...
- **Objective:** Актуализировать parser tests и snapshots под новую схему.
- **Rules:** typescript-rules, node-test
- **Target Files:** `services/dbc/parser/implementations/jsdoc/__tests__/`
- **Inputs:** DP-jsdoc
- **Exit:** parser test suite и snapshots проходят.
  <!--/SECTION:PHASE_P1-->
  <!--SECTION:BDD-->
...
**Scenario:** Ошибка порядка: updated CONTRACT_ORDER [`unit`]
...
| npm test -- --experimental-test-coverage | node-test        | probe |
...
- Scenario "New test: parses @implements" → `__tests__/jsdoc-parser-fields.test.ts` :: `should parse implements tag with specifier and trailing value`
- Scenario "New test: format detection" → `__tests__/jsdoc-parser-syntax.test.ts` :: `should detect multi-line format for block contract text`
- Scenario "New test: inline entries" → `__tests__/jsdoc-parser-syntax.test.ts` :: `should parse inline tags from pipe-delimited single-line contract`
- Scenario "Ошибка порядка: updated CONTRACT_ORDER" → `__tests__/jsdoc-parser-validation.test.ts` :: `reports ERR_DBC_ORDER for implements order violation`
- Scenario "New test: @consumer outside order" → `__tests__/jsdoc-parser-validation.test.ts` :: `should not report order violation for valid implements, consumer, param, returns sequence`
