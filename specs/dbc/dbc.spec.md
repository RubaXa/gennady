...
| FR-06 | `@implements {ContractName} in path`: `specifier` = `ContractName` из скобок, `value` = остаток; порядок: первый                                                                                                                                                                                                                                                                                                                                                                                                                                   |
...
| FR-25 | Отчёт в ESLint-формате `file:line:col: severity: code: message`, все ошибки имеют `severity: error`                                                                                                                                                                                                                                                                                                                                                                                                                                                |
...
| ------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------- |
| function / method (не constructor)                                                          | Каждый параметр сигнатуры ↔ `@param` в порядке сигнатуры                                     | Для `void` `@returns` отсутствует, иначе присутствует                                                                            |
| method (внутри `class implements Interface` + `@see {Interface#method}` в контракте метода) | отсутствуют (redundant — описаны в интерфейсе). `ERR_DBC_LINT_PARAM_REDUNDANT_IN_IMPLEMENTS` | отсутствует. `ERR_DBC_LINT_PARAM_REDUNDANT_IN_IMPLEMENTS` (вытесняет `ERR_DBC_LINT_RETURNS_UNEXPECTED` — приоритет у redundancy) |
...
- **Логика детекта implements+@see (для реализации `removeRedundantInImplements`):**
  1. AST-адаптер (`DbcTsAstAdapter`) проставляет `implementsInterfaces: string[]` на `DbcExportedEntity` — массив имён интерфейсов из `implements` clause. Пустой массив / `undefined` = класс без implements.
  2. При вызове `DbcContractMatchValidator.validate()` для каждого method-члена: если родительская сущность имеет `implementsInterfaces.length > 0` И контракт метода содержит `@see {T#methodName}`, где `T` совпадает с одним из implemented-интерфейсов, а `methodName` совпадает с именем метода → срабатывает `ERR_DBC_LINT_PARAM_REDUNDANT_IN_IMPLEMENTS` на каждый `@param` и `@returns` в контракте.
  3. Если `@see` ссылается на другой интерфейс (не из `implements`-clause) — НЕ redundant, проверяется как обычный method.
  4. Если `@see` ссылается на другой метод того же интерфейса — НЕ redundant (несовпадение `methodName`).
  5. `@see` на методе обрабатывается position-agnostic (любое положение в контракте — валидно).
  6. `@throws`, `@pre`, `@post`, `@invariant`, `@purpose` — НЕ удаляются (только `@param`/`@returns`), валидируются обычным порядком.
- **BDD-сценарии для `ERR_DBC_LINT_PARAM_REDUNDANT_IN_IMPLEMENTS`:**
  - **Happy-1:** GIVEN класс `class Impl implements Agent`, метод `scan` с контрактом `/** @see {Agent#scan} | @param x - координата | @returns результат */`. WHEN линтер проверяет контракт. THEN `@param x` → `ERR_DBC_LINT_PARAM_REDUNDANT_IN_IMPLEMENTS`, `@returns` → `ERR_DBC_LINT_PARAM_REDUNDANT_IN_IMPLEMENTS`. Autofix удаляет оба тега.
  - **Edge-1 (без @see):** GIVEN класс `class Impl implements Agent`, метод `scan` с контрактом `/** @param x - координата | @returns результат */` (без `@see`). THEN параметры НЕ redundant — проверяются как обычный method.
  - **Edge-2 (без implements):** GIVEN класс `class Helper` (без `implements`), метод `scan` с контрактом `/** @see {Agent#scan} | @param x | @returns result */`. THEN НЕ redundant — `implementsInterfaces` пуст/undefined, проверяется как обычный method.
  - **Edge-3 (@see на другой интерфейс):** GIVEN класс `class Impl implements Agent`, метод `scan` с контрактом `/** @see {Other#scan} | @param x | @returns result */`. `Agent` и `Other` — разные интерфейсы. THEN НЕ redundant (несовпадение интерфейса).
  - **Edge-4 (@see на другой метод):** GIVEN класс `class Impl implements Agent`, метод `scan` с контрактом `/** @see {Agent#otherMethod} | @param x | @returns result */`. THEN НЕ redundant (методы не совпадают).
  - **Edge-5 (только @param redundant):** GIVEN класс `class Impl implements Agent`, метод `getInfo` (void) с контрактом `/** @see {Agent#getInfo} | @param id - идентификатор */`. THEN `@param id` → `ERR_DBC_LINT_PARAM_REDUNDANT_IN_IMPLEMENTS`. `@returns` отсутствует — ошибки нет.
  - **Edge-6 (extends — НЕ redundant):** GIVEN класс `class Child extends Base`, метод `calculate(x: number): number` с контрактом `/** @see {Base#calculate} | @param x — значение | @returns результат */`. Класс использует `extends` (наследование), а НЕ `implements`. THEN `@param x`, `@returns` — НЕ redundant. `implementsInterfaces` пуст → проверяется как обычный method. Extends ≠ implements: при наследовании субкласс может менять сигнатуру/семантику метода.
  - **Edge-7 (override — НЕ redundant):** GIVEN класс `class Child extends Base`, метод `calculate(x: number, y: number): number` (override — сигнатура расширена) с контрактом `/** @see {Base#calculate} | @param x — первое | @param y — второе | @returns сумма */`. Класс использует `extends`, метод имеет ключевое слово `override`. THEN все теги проверяются как обычный method — НЕ redundant. Override метода с изменённой сигнатурой требует собственного контракта.
  - **Edge-8 (extends + implements — implements приоритет):** GIVEN класс `class Impl extends Base implements Agent`, метод `scan(x: number): string` с контрактом `/** @see {Agent#scan} | @param x | @returns result */`. Класс одновременно `extends Base` и `implements Agent`. `implementsInterfaces` = `['Agent']`. `@see {Agent#scan}` совпадает. THEN `@param x`, `@returns` → `ERR_DBC_LINT_PARAM_REDUNDANT_IN_IMPLEMENTS` (implements-логика срабатывает).
  - **Edge-9 (extends + implements — @see на extends-класс):** GIVEN класс `class Impl extends Base implements Agent`, метод `scan(x: number): string` с контрактом `/** @see {Base#scan} | @param x | @returns result */`. `@see` ссылается на `Base` (extends-класс), а не на `Agent` (implements-интерфейс). THEN НЕ redundant — `Base` не в `implementsInterfaces`. Проверяется как обычный method.
  - **Edge-10 (implements несколько интерфейсов):** GIVEN класс `class Impl implements Agent, Stoppable`, метод `scan(x: number): string` с контрактом `/** @see {Agent#scan} | @param x | @returns result */`. `implementsInterfaces` = `['Agent', 'Stoppable']`. `@see` ссылается на `Agent#scan` — совпадает с одним из implemented. THEN redundant.
  - **Edge-11 (extends, без override, без @see):** GIVEN класс `class Child extends Base`, метод `calculate(x: number): number` (без ключевого слова override, без @see) с контрактом `/** @param x — значение | @returns результат */`. THEN проверяется как обычный method. `implementsInterfaces` пуст — implements-логика не срабатывает.
  - **Autofix-1:** GIVEN метод implements+@see с `@param x`, `@param y`, `@returns`. Autofix удаляет все три тега. Повторный autofix — идемпотентен.
  - **Autofix-2 (сохранение не-redundant тегов):** GIVEN метод implements+@see с `@purpose Сканирует | @param x | @returns результат`. Autofix удаляет `@param x` и `@returns результат`. `@purpose Сканирует` — сохранён на месте.
- **Форматы контрактов:**
