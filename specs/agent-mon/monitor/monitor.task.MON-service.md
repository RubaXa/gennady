# Task: MON-service — AgentMonitor: ядро и фабрика
...
- **Reopens:** 0- **Spec References:**
  - Contract: [`AgentMonitor`](./monitor.spec.md#agentmonitor)
  - Contract: [`createMonitor`](./monitor.spec.md#createmonitor)
  - Port: [`AgentProvider`](../model/model.spec.md#agentprovider)
  - Errors: [`DuplicateProviderError`, `ProviderNotFoundError`](../agent-mon.spec.md#4-public-api-surface)
...
- Scenario "scanAll 50 сессий < 1с" → `services/agent-mon/monitor/__tests__/agent-monitor.test.ts` :: `scanAll returns 50 sessions within one second`
...
- [ ] `<ts>` intro AgentMonitor ← ядро реестра провайдеров и координации сканирования
- [ ] `<ts>` intro createMonitor ← фабрика для создания AgentMonitor
...
- [ ] `<ts>` discovery SUT files (P1) используют `.js` расширения для value-импортов → несовместимо с `node --test` (runtime резолвит только `.ts` для value imports; `import type` OK). Кодовая база использует `.ts` для value imports (см. dbc-ts-linter.ts, anchor.check.ts).
...
- [ ] `<ts>` insight 1 предсуществующий failing test в `cli/cmd/alt-opinion/__tests__/alt-opinion.cmd.test.ts` (test 5: Synthesis failure) → не относится к P2. Все 8 тестов P2 проходят.
- [ ] `<ts>` ver `node --test services/agent-mon/monitor/__tests__/*.test.ts` → pass exit=0 (8/8, изолированно; project-wide npm run test fail=exit1 — предсуществующий сбой alt-opinion.cmd.test.ts, не относится к MON-service)
- [ ] `<ts>` DONE
