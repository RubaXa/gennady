...
- **Public Operations:**
  ...
  3. `psInfo(pids)` — если pid отсутствует в Map → `status: 'completed'` 4. Если active: `parseClaudeArgs(args)` → model, effort
  4. `readSessionTitle(cwd, sessionId)` → title
  5. `lastActivityAt` = mtime файла `<PID>.json`
  6. `cpuPercent`, `memoryMb` = `psInfo(pids).get(pid)` если alive
  7. Фильтр `opts.since` → по `startedAt`
