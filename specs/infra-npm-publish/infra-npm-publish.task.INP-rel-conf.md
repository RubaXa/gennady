# Task: INP-rel-conf — Создать `.release-it.json`

<!--SECTION:META-->

## 1. Meta

- **Task-ID:** INP-rel-conf
- **Status:** [ ] TODO
...
- **Reopens:** 0
- **Spec References:**
  - Bootstrap: [Bootstrap Requirements row 2](./infra-npm-publish.spec.md)
  - Decision: [D-001 — выбор release-it](./infra-npm-publish.spec.md)
  - Decision: [D-004 — release-it hooks](./infra-npm-publish.spec.md)
...
- **Given** release-it установлен (INP-rel-dep)
...
**Scenario:** Ошибка проверки до bump — безопасный откат [`contract`]
...
| ----------------------------------------------------------------------------- | ----------------------- | ----- |
| `node -e "JSON.parse(require('fs').readFileSync('.release-it.json','utf8'))"` | config-json-valid       | extra |
| `npx release-it --dry-run`                                                    | release-it-config-valid | extra |
