...
- **V-05 закрыта частично:** новый `shared/verify/stack-detection.ts` (`detectRepoStack`) даёт один общий факт `StackDetection` вместо per-caller угадывания; `sdd-state.cmd.ts` подключает его безусловно и печатает `STACK=`/`STACK_SOURCE=` в `[READINESS]`. Реальный второй вызов `detectStacks` (`stack-registry.ts`) есть — его запись `Usage Waiver` снята. **V-08b** также подключает `detectRepoStack` в `sdd-task.cmd.ts`/`sdd-verify/phase-context.ts`, но только когда `stack.use` явно задан в конфиге (bootstrap-safety: без `package.json` эти две команды по умолчанию остаются node, а не молча становятся anystack — `resolveProjectReadiness`, `resolvePhaseContext`) — «все три команды видят один факт безусловно» пока не достигнуто; владелец безусловного подключения не назначен.
...
<details>
<summary>Usage Waiver — 17 символов open (26 исходных V-02, минус 9 снятых V-05/V-07/V-08), по задаче-владельцу: V-09 — 5 (`C`, `I`, `Bad`, `scopeHasGoGenerate`, `isStructuralListError`); V-18 — 4 (`TreeGuard`, `TreeGuardOptions`, `GuardAcquisition`, `acquireTreeGuard`); без твёрдого владельца — 8 (`ConfigSectionLoad`, `formatDuration`, `allOf`, `validateStackConfig`, `unmatchedGateOverrides`, `applyStackConfig`, `StackRun`, `VerifyReport`) — **пересмотрено при закрытии V-16a (V-BATCH-13):** `StackRun`/`VerifyReport` перешли сюда из бакета «V-16a» — их предполагаемый владелец закрылся, формы MAIN не принял, новый владелец не назначен; кандидат-преемник для обоих (и для `formatDuration`) — задача «`extraGates`/anystack вживляются в полный профиль» (M, зависит от V-08b, остаток `V-BATCH-13`), иначе — кандидаты на удаление. Назначение владельцев для всех 8 остаётся открытым решением Lead (доска `61-TASK-BOARD.md`), не сделанным этой пачкой</summary>
...
- **Usage Waiver:** печать `timeoutMs` гейта в человекочитаемом виде. **Пересмотрено при закрытии V-07:** V-07 добавляет только загрузку+валидацию (+exit 4), без печати самого гейт-плана — печать config-driven гейтов принадлежит `--plan`/`[gate] ` выводу (V-14/V-16, вне этой волны). **Пересмотрено при закрытии V-16a:** заявленный кандидат-владелец (`gennady verify --plan --json`) вышел и не печатает `timeoutMs` — `VerifyPlanGate` несёт только `name`/`command`/`required` (`verify.types.ts`), а `full`-профиль сегодня node-only, так что per-gate timeout из `extraGates` в этот вывод вообще не попадает. Владелец исчерпан без снятия записи; новый кандидат — та же задача-преемник, что у `StackRun`/`VerifyReport` ниже («extraGates/anystack вживляются в полный профиль»), если она когда-нибудь заставит `--plan` печатать config-driven timeout; иначе V-09 (golang-пресет).
...
- **Usage Waiver:** deep-merge применения конфига к гейтам — тот же класс, что `unmatchedGateOverrides`: работает над полными `Gate[]` MAIN-модели, не над `StackPreset`. **Пересмотрено при закрытии V-08:** anystack-пресет читает `extraGates` напрямую через `pluginConfigOf` (снят), минуя `applyStackConfig` — фазовая лестница проще (нет skipGates/overrideGates для config-only стека). **Пересмотрено при закрытии V-12:** этот же символ получил новые параметры `unskipIds`/`targets` и вызывает `gateInScope` (`stack-config.ts:618`) — файл-scope `when` теперь есть и в этой ветке. Символ остаётся структурно недостижимым от продакшн-вызова (0 живых call site вне тестов, `R-V-12 §4`): живой скоуп-путь этой волны — anystack-пресет/`phase-verification-plan.ts`, не `applyStackConfig`/полная `Gate[]`-модель. Владелец не назначен, тот же критерий снятия, что у `unmatchedGateOverrides`.
...
- **Usage Waiver:** тип одного запуска стека; в MAIN потребляется движком `services/stack/gate-runner.ts`, который эта волна не переносит целиком. **Пересмотрено при закрытии V-04:** `resolvePreset`/`StackPreset` (`shared/verify/presets/node.ts`) сознательно НЕ импортируют `StackRun`/`VerifyReport` — RC's ладдер продолжает нести собственную форму результата (`GateResult[]`/`VerifyOutcome` в `sdd-verify.types.ts`), а не форму MAIN. Открытый вопрос снят отрицательно для V-04; предполагаемый владелец был CLI-фасад `gennady verify --plan --json` (V-16a). **Пересмотрено при закрытии V-16a (V-BATCH-13):** фасад вышел и намеренно определил собственный узкий JSON-контракт (`VerifyPlanDocument`/`VerifyPlanGate`, `cli/cmd/verify/verify.types.ts`) — план обязан отражать реально исполняемый node-only `full`-профиль, а не форму MAIN, рассчитанную на многостековый рантайм-запуск. Владелец исчерпан без снятия записи. **Преемник:** символ снимается, если/когда задача «`extraGates`/anystack вживляются в полный профиль» (M, зависит от V-08b — см. остатки `V-BATCH-13`) когда-нибудь заставит `--plan` нести форму, близкую к `StackRun`; до тех пор — кандидат на удаление вне этой волны.
...
- **Open risks & validation needs:**
  ...
  - `unmatchedGateOverrides`/`applyStackConfig` (§6) — **закрыто по факту V-08, пересмотрено V-12:** `pluginConfigOf` снят (real 2nd ref via `presets/anystack.ts`); V-12 gave `applyStackConfig` its own `when`/`targets` scope params, still with 0 live call sites outside tests — owner unassigned, revisit if/when `gennady verify`'s Gate model (V-16) gets its own connection layer.
