# Task: SYN-shared — Extract shared sync core + refactor sync
...
- **Dependencies:** SYN-command, SYN-tests
- **Spec References:**
  - Module spec: [`sync.spec.md`](./sync.spec.md)
  - Module spec: [`sync-skills.spec.md`](../sync-skills/sync-skills.spec.md) §6 (shared core File Structure)
  - Decision: [D-010](../cli.spec.md) — Shared sync core
  - Decision: [D-M004](./sync.spec.md) — Shared sync core (sync module)
  - Decision: [D-M004](../sync-skills/sync-skills.spec.md) — Shared sync core (sync-skills module)
  - Contract: [`SyncCmdDeps`](../sync-skills/sync-skills.spec.md) — расширенный порт
...
- **Inputs:** none
- **Exit:** `npm run type-check && npm test` pass; sync импортирует из shared, существующие тесты SYN-tests проходят
...
- **When** `npm test`
- **Then** все тесты из SYN-tests проходят
