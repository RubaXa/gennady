...
| `ChangedSymbol`          | Value Object | `{name, kind, file, exported}` — символ диффа, кандидат на проверку; `exported` различает строгий порог (D-YG005)   |
...
- Postconditions:
  - `count >= 2` → находки нет
  - Экспортируемая сущность (`ChangedSymbol.exported === true`), `count < 2` и нет `Usage Waiver` → `ERR_CLI_YAGNI_UNDERUSED`, **всегда `error`** (D-YG005: порог `< 2` строгий только для экспортов — публичная поверхность, разросшаяся без потребителей, самый дорогой случай YAGNI)
  - Приватная (не экспортируемая) сущность с `count === 1` → находки НЕТ — это обычная декомпозиция (один внутренний вызов не подозрителен), не спекулятивная поверхность (D-YG005)
...
Пять решений о самом правиле: как определяется «изменённый символ» (D-YG001, active), как считаются использования (D-YG002, active), первая попытка градации severity по `flowVersion` (D-YG003, superseded), строгое правило без деления по видимости (D-YG004, superseded), финальное правило — строгий порог только для экспортов, приватные щадятся до нуля (D-YG005, active). Остальные сущности модуля погашают собственные YAGNI-находки точечными записями `Usage Waiver` без Decision Log — их однократный вызов объясняется прямо у сущности.
