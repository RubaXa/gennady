# Task: E2E-harness — E2E-тестирование CLI-команд через npm pack
...
- **Entity References:** `E2eContext`, `SpawnResult`, `setupE2e`, `FixtureProject` (closed-world, per module Entity Inventory)
...
| `npm run test:e2e`   | node-test        | probe |
...
- should complete full setup successfully → `cli/__tests__/e2e/e2e.test.ts` :: `should complete full setup successfully`
- should fail on npm pack error → `cli/__tests__/e2e/e2e.test.ts` :: `should fail on npm pack error`
- should fail on npm install error → `cli/__tests__/e2e/e2e.test.ts` :: `should fail on npm install error`
- should fail on missing fixture directory → `cli/__tests__/e2e/e2e.test.ts` :: `should fail on missing fixture directory`
- should fail on temp dir EACCES → `cli/__tests__/e2e/e2e.test.ts` :: `should fail on temp dir EACCES`
- should rollback temp dir on partial setup failure → `cli/__tests__/e2e/e2e.test.ts` :: `should rollback temp dir on partial setup failure`
- should throw when npx is not found → `cli/__tests__/e2e/e2e.test.ts` :: `should throw when npx is not found`
- should throw on spawn timeout after 30s → `cli/__tests__/e2e/e2e.test.ts` :: `should throw on spawn timeout after 30s`
- should be idempotent → `cli/__tests__/e2e/e2e.test.ts` :: `should be idempotent`
- should pass a clean file → `cli/__tests__/e2e/lint.e2e.test.ts` :: `should pass a clean file`
- should fail on missing @file: → `cli/__tests__/e2e/lint.e2e.test.ts` :: `should fail on missing @file:`
- should fail on missing @consumers: → `cli/__tests__/e2e/lint.e2e.test.ts` :: `should fail on missing @consumers:`
- should fail on unpaired anchor → `cli/__tests__/e2e/lint.e2e.test.ts` :: `should fail on unpaired anchor`
- should report autofix on DBC errors → `cli/__tests__/e2e/lint.e2e.test.ts` :: `should report autofix on DBC errors`
- should run --staged mode → `cli/__tests__/e2e/lint.e2e.test.ts` :: `should run --staged mode`
- should lint a directory → `cli/__tests__/e2e/lint.e2e.test.ts` :: `should lint a directory`
- should fail on nonexistent path → `cli/__tests__/e2e/lint.e2e.test.ts` :: `should fail on nonexistent path`
- should show project map → `cli/__tests__/e2e/orient.e2e.test.ts` :: `should show project map`
- should search by task → `cli/__tests__/e2e/orient.e2e.test.ts` :: `should search by task`
- should search by consumer → `cli/__tests__/e2e/orient.e2e.test.ts` :: `should search by consumer`
- should search by keyword → `cli/__tests__/e2e/orient.e2e.test.ts` :: `should search by keyword`
- should show file detail → `cli/__tests__/e2e/orient.e2e.test.ts` :: `should show file detail`
- should show dependency graph → `cli/__tests__/e2e/orient.e2e.test.ts` :: `should show dependency graph`
- should sync directives on first run → `cli/__tests__/e2e/sync.e2e.test.ts` :: `should sync directives on first run`
- should report unchanged on repeat run → `cli/__tests__/e2e/sync.e2e.test.ts` :: `should report unchanged on repeat run`
- should support --dry-run → `cli/__tests__/e2e/sync.e2e.test.ts` :: `should support --dry-run`
- should filter by subdirectory → `cli/__tests__/e2e/sync.e2e.test.ts` :: `should filter by subdirectory`
- should fail on nonexistent subdirectory → `cli/__tests__/e2e/sync.e2e.test.ts` :: `should fail on nonexistent subdirectory`
- should install skills on first run → `cli/__tests__/e2e/sync-skills.e2e.test.ts` :: `should install skills on first run`
- should report unchanged on repeat run → `cli/__tests__/e2e/sync-skills.e2e.test.ts` :: `should report unchanged on repeat run`
- should support --dry-run → `cli/__tests__/e2e/sync-skills.e2e.test.ts` :: `should support --dry-run`
- should filter by skill name → `cli/__tests__/e2e/sync-skills.e2e.test.ts` :: `should filter by skill name`
<!--/SECTION:TEST_COVERAGE-->
...
_(Round = one execute-then-audit attempt. Per-phase blocks within a Round. Skeleton is minimal — event lines (`intro` / `decision` / `tried` / `discovery` / `insight` / `BLOCKED`) appear ONLY when the event happens. Token vocabulary in [tasks/README.md](../../../tasks/README.md#execution-log-template).)_
...
- [x] `2026-05-31T16:41:56Z` decision fixture-tasks-fix=@tasks в service.ts и helper.ts исправлены с TSK-060 на E2E-harness; orient.e2e.test.ts --task обновлён соответственно
...
- [x] `2026-05-31T16:41:56Z` discovery фикстуры service.ts и helper.ts теперь используют @tasks: E2E-harness — orient тест поиска по задаче обновлён на --task=E2E-harness
