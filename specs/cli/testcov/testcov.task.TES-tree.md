# Task: TES-tree — testcov: port coverage-tree as gennady CLI command
...
### P6 — test: проверка CLI и coverage policy

- **Objective:** Проверить реальные CLI-сценарии `testcov`, включая runner detection, diagnostics, tree/file-detail и coverage-gate.
- **Rules:**
  - [node-test](../../../ai/directives/testing/node-test.xml)
- **Target Files:**
  - `cli/cmd/testcov/__tests__/testcov.cmd.test.ts` (Verify)
- **Inputs:** P5 handoff
- **Exit:** целевой набор `testcov.cmd.test.ts` проходит; coverage-команда ниже завершается без diagnostics.
...
**Scenario:** Ошибка: test runner отсутствует [`unit`]

- **Given** project не объявляет supported runner и native coverage script
- **When** `testcov --check` проверяет configuration
- **Then** diagnostics содержат `NO_RUNNER`
...
- Test runner отсутствует → `testcov.cmd.test.ts` :: `still reports plain NO_RUNNER when no runner and no native-coverage script exist`
...
- **Coverage Owner Phase:** P6

## 5. Verification

| Command                                                           | Required by                                              | Role     |
| ----------------------------------------------------------------- | -------------------------------------------------------- | -------- |
| `npx gennady testcov`                                             | node-test                                                | coverage |
| `npx gennady testcov --files`                                     | P1: дерево с файлами, без `__tests__` и test-файлов      | probe    |
| `npx gennady testcov --flat --json`                               | P1: валидный JSON                                        | probe    |
| `npx gennady testcov --check`                                     | P1: диагностика конфигурации                             | probe    |
| `npx gennady testcov --check --json`                              | P1: JSON с `ok`, `runner`, `coverageFile`, `diagnostics` | probe    |
| `npx gennady testcov --help`                                      | P1: справка со всеми флагами                             | probe    |
| `npx gennady testcov cli/cmd/_shared/update-check-worker.ts`      | P3: аннотированный файл с подсветкой                     | probe    |
| `npx gennady testcov cli/cmd/_shared/update-check-worker.ts -c 0` | P3: только непокрытые строки                             | probe    |
| `npm run lint`                                                    | P5: lint → clean                                         | extra    |
| `npm run type-check`                                              | P5: tsc --noEmit → clean                                 | extra    |
...
- [x] `2026-06-07` code: `@tasks TSK-testcov-1` → `TES-tree` во всех файлах
...
- [ ] migration reopened: testcov verification and coverage evidence are incomplete
