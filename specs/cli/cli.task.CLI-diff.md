# Task: CLI-diff — vcs-diff CLI (getChanges)

<!--SECTION:META-->

## 1. Meta

- **Task-ID:** CLI-diff | **Status:** [x] DONE | **Scope:** cli | **Module:** vcs-diff | **Dependencies:** None
...
**Scenario:** Ошибка API при загрузке diff [`unit`]

- **Given** VCS adapter отклоняе `getChanges`
- **When** выполняется `vcs-diff`
...
- Ошибка API при загрузке diff → `vcs-diff.test.ts` :: `getChanges throws API error — error on stderr, exit 1`
