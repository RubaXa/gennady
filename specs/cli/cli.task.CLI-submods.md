# Task: CLI-submods — vcs-worktree: git submodule update --init --recursive (FR-WT-08)
...
- **Scope:** `cli`
- **Module:** `vcs-worktree`
- **Dependencies:** CLI-deplinks
- **Spec References:**
  - Scope spec: [cli.spec.md §4.1.12 FR-WT-08, D-020](./cli.spec.md#4112-vcs-worktree-functional-requirements)
...
| `node --import tsx --test cli/cmd/vcs-worktree/_core/logic/worktree-ops.test.ts` | node-test        | extra |
...
_(Round = one execute-then-audit attempt. Per-phase blocks within a Round. Skeleton is minimal — event lines (`intro` / `decision` / `tried` / `discovery` / `insight` / `BLOCKED`) appear ONLY when the event happens. Token vocabulary in [tasks/README.md#execution-log-template](../../tasks/README.md#execution-log-template).)_
