# Task: ORI-command — Команда orient: навигация по file-header и DBC-контрактам
...
- **Dependencies:** None
- **Spec References:**
  - Scope spec: [cli.spec.md](../cli.spec.md)
  - Module section: [cli.spec.md §3.5 orient DX](../cli.spec.md)
  - Requirements: [cli.spec.md §4.1.5](../cli.spec.md)
  - Architecture: [cli.spec.md §5.6 orient](../cli.spec.md)
  - Decision Log: [cli.spec.md D-009](../cli.spec.md)
  - DBC: [DbcJsDocParser](../../dbc/dbc-parser/dbc-parser.spec.md)
  - resolve-references: [resolve-references.fn.ts](../lint/lint.spec.md)
...
- **Given** файлы с `@tasks: DP-fields` в проекте
- **When** агент выполняет `gennady orient --task=DP-fields`
- **Then** вывод содержит строку контекста `DP-fields → task.md → spec.md`
...
- **When** агент выполняет `gennady orient --task=DP-fields --task=DP-jsdoc`
- **Then** вывод содержит группировку `DP-fields: file1, file2` / `DP-jsdoc: file1`
...
- **Given** файл с `@tasks: DL-ts-deps` и существующий `tasks/dbc/dbc-linter.task-04.md`
- **When** агент выполняет `gennady orient --task=DL-ts-deps`
- **Then** вывод содержит строку `DL-ts-deps -> tasks/dbc/.../task-04.md -> specs/dbc/.../spec.md`
...
- **When** агент выполняет `gennady orient --task=DP-fields --detail`
- **Then** под каждым файлом выводятся экспортируемые сущности с `@purpose`
...
| --------------------------------------- | ---------------- | ----- |
| `npx tsc --noEmit`                      | typescript-rules | extra |
| `npm run lint:contracts`                | typescript-rules | extra |
| `node --test cli/cmd/orient/__tests__/` | node-test        | extra |

- **Task-specific Completion additions:** все файлы должны иметь `// @file:`, `// @consumers:`, `// @tasks: ORI-command` в хедере; `cli/gennady.ts` — добавить `case 'orient'` в switch; `cli/AGENTS.md` — добавить строку `orient`; `cli/cmd/help/help.cmd.ts` — добавить `orient`.
...
- `buildIndex(files) -> Map<word, Set<FileWordRef>>` → `build-index.test.ts` :: `contract index type: returns Map<string, Set<FileWordRef>>`
- `extractHeader(content) -> FileHeader` → `extract-header.test.ts` :: `contract header type: returns FileHeader shape`
- `damerauLevenshtein(a, b) -> number` → `damerau-levenshtein.test.ts` :: `contract distance type: returns number`
- Score priority (exact > prefix > fuzzy) → `query-keyword.test.ts` :: `contract score priority: exact > prefix > fuzzy`
- Hints <=4 per scenario → `hints.test.ts` :: `contract hints count: returns at most 4 hints`
- Hints token format (`<cmd>`, `--flag=<value>`) → `hints.test.ts` :: `contract hints token format: uses --flag=<value> syntax`
- DL boundary at 5 chars (short vs long threshold) → `damerau-levenshtein.test.ts` :: `contract boundary word length 5: distance 2 passes`

### Unit-level

**S1 -- Карта проекта**

- S1: depth indicator → `render-tree.test.ts` :: `render tree with depth: shows indicator when depth exceeded`
- S1: depth singular/plural → `render-tree.test.ts` :: `depth singular plural: uses correct form for 1 item`
- S1: --max-results parsing → `orient.cmd.test.ts` :: `parses --max-results flag`
- S1: scan-files excluded dirs → `scan-files.test.ts` :: `excluded dirs: skips node_modules, .git, dist, coverage, build, out`
- S1: scan-files EACCES → `scan-files.test.ts` :: `permission error: EACCES does not crash scanning`
- S1: scan-files empty project → `scan-files.test.ts` :: `empty project: returns empty array`
- S1: scan-files .ts + .tsx filter → `scan-files.test.ts` :: `extension filter: returns only .ts and .tsx files`
- S1: scan-files dir not found → `scan-files.test.ts` :: `dir not found: normally resolves to empty or existing path`

**S2 -- Файлы по задаче**

- S2: single task-id → `query-task.test.ts` :: `single task: finds matching files`
- S2: multiple task-ids → `query-task.test.ts` :: `multiple tasks: returns grouped results`
- S2: task not found → `query-task.test.ts` :: `task not found: returns empty files array`

**S3 -- Потребители**

- S3: exact consumer → `query-consumer.test.ts` :: `exact consumer: finds exact match`
- S3: substring consumer → `query-consumer.test.ts` :: `substring consumer: case-insensitive match`
- S3: consumer not found → `query-consumer.test.ts` :: `consumer not found: returns empty files`
- S3: multiple consumers group → `query-consumer.test.ts` :: `multiple consumers group: returns results per consumer`
- S3: --fuzzy with consumer → `query-consumer.test.ts` :: `fuzzy consumer: matches with DL distance`

**S4 -- Поиск по ключевым словам**

- S4: exact keyword → `query-keyword.test.ts` :: `exact keyword: finds files with exact match`
- S4: fuzzy keyword → `query-keyword.test.ts` :: `fuzzy keyword: matches via DL distance`
- S4: keyword not found → `query-keyword.test.ts` :: `keyword not found: returns empty`
- S4: match in @purpose → `query-keyword.test.ts` :: `purpose match: entity match reflected in result`
- S4: prefix match → `query-keyword.test.ts` :: `prefix match: words starting with token scored +5`
- S4: multi-word AND → `query-keyword.test.ts` :: `multi-word AND: files must contain all tokens`
- S4: three-level scoring order → `query-keyword.test.ts` :: `scoring order: exact > prefix > fuzzy in result sort`
- S4: empty/whitespace keyword → `query-keyword.test.ts` :: `empty/whitespace keyword: returns empty array`

**S5 -- Детальный взгляд**

- S5: file without @file: → `render-detail.test.ts` :: `missing header: shows (missing) for empty @file:`
- S5: file without exports → `render-detail.test.ts` :: `no exports: shows @exports: 0 without entity list`
- S5: file without @tasks/@consumers → `render-detail.test.ts` :: `minimal header: omits @tasks: and @consumers: when empty`
- S5: class with methods → `render-detail.test.ts` :: `class entity: shows class with kind`
- S5: all DBC tags isolated → `render-detail.test.ts` :: `all dbc tags: renders @throws, @sideEffect, @invariant individually`

**S6 -- Поиск сущности**

- S6: exact entity → `query-entity.test.ts` :: `exact entity: finds exact name match`
- S6: fuzzy entity → `query-entity.test.ts` :: `fuzzy entity: matches with DL distance`
- S6: entity not found → `query-entity.test.ts` :: `entity not found`
- S6: entity in multiple files → `query-entity.test.ts` :: `multi-file entity: same name in multiple files`
- S6: repeatable --entity → `query-entity.test.ts` :: `multiple entities: finds both`
- S6: class method NOT found → `query-entity.test.ts` :: `class method excluded: only exported entities found`
- S6: fuzzy boundary short name → `query-entity.test.ts` :: `fuzzy boundary short: <=5 chars, distance >2 fails`
- S6: fuzzy boundary long name → `query-entity.test.ts` :: `fuzzy boundary long: >5 chars, distance <=3 matches`

**S7 -- Архитектурный граф**

- S7: flat graph → `query-graph.test.ts` :: `flat graph: builds from consumer annotations`
- S7: recursive graph → `query-graph.test.ts` :: `recursive graph: builds tree from root consumer`
- S7: recursive with --depth → `query-graph.test.ts` :: `recursive graph with depth: limits recursion`
- S7: circular consumers → `query-graph.test.ts` :: `circular dependencies: marks [circular] and does not loop`
- S7: empty graph → `query-graph.test.ts` :: `empty graph: returns empty map`
- S7: consumer as text → `query-graph.test.ts` :: `unresolved consumer: marks as text-only reference`
- S7: render flat → `render-graph.test.ts` :: `flat render: shows file paths under consumer`
- S7: render recursive → `render-graph.test.ts` :: `recursive render: returns tree lines unchanged`

**S8/S9 -- Спеки**

- S8: specs overview → `query-spec.test.ts` :: `specs overview: scans specs/ for .spec.md files`
- S9: spec search → `query-spec.test.ts` :: `spec search: finds spec by name`
- S9: spec not found → `query-spec.test.ts` :: `spec not found: returns null`
- S9: library-level spec → `query-spec.test.ts` :: `library spec: marks spec without own tasks as library-level`

**Render**

- S4: render search with entity match → `render-search.test.ts` :: `entity match render: shows entity name when present`
- S5: render detail full output → `render-detail.test.ts` :: `full detail render: shows header blocks`
- S8/S9: render specs overview → `render-specs.test.ts` :: `specs overview render: shows spec path with task IDs`
- S8/S9: render spec search → `render-specs.test.ts` :: `spec search render: shows spec path and task IDs`
- File list format → `render-file-list.test.ts` :: `file line format: universal format with all fields`

**CLI integration**

- File flag without value → `orient.cmd.test.ts` :: `missing value after --file flag defaults to empty string`
- Multiple files → `orient.cmd.test.ts` :: `multiple files: scan finds all .ts files`
- Conflicting: --file + --dir → `orient.cmd.test.ts` :: `file dir conflict: --file and --dir mutually exclusive`
- Conflicting: --graph + keyword → `orient.cmd.test.ts` :: `graph keyword conflict: --graph and positional keyword mutually exclusive`
- Conflicting: --specs + --file → `orient.cmd.test.ts` :: `specs file conflict: --specs and --file mutually exclusive`
- Conflicting: --specs + --task/--consumer/--entity/--graph → `orient.cmd.test.ts` :: `specs all conflicts: --specs incompatible with --task/--consumer/--entity/--graph`
- Conflicting: --spec + --file/--task/--consumer/--entity/--graph → `orient.cmd.test.ts` :: `spec all conflicts: --spec incompatible with --file/--task/--consumer/--entity/--graph`
- Conflicting: --fuzzy alone (no-op) → `orient.cmd.test.ts` :: `fuzzy noop: --fuzzy without --entity or --consumer is allowed but no-op`
...
- DL: short threshold <=2 → `damerau-levenshtein.test.ts` :: `short threshold pass: word <=5 chars, distance <=2`
- DL: long threshold <=3 → `damerau-levenshtein.test.ts` :: `long threshold pass: word >5 chars, distance <=3`

### Integration-level

- S1: map integration → `orient.cmd.test.ts` :: `map integration: returns empty for empty directory`
- Detail flag parsing → `orient.cmd.test.ts` :: `parses --detail flag`
- S5: file detail → `orient.cmd.test.ts` :: `file detail integration: scan-files + render for specific files`
- S5: multiple files detail → `orient.cmd.test.ts` :: `multiple files: scan finds all .ts files`
- S1: --dir scope → `orient.cmd.test.ts` :: `dir scope integration: scan restricted to subdirectory`
<!--/SECTION:TEST_COVERAGE-->
...
_(Round = one execute-then-audit attempt. Per-phase blocks within a Round. Skeleton is minimal — event lines appear ONLY when the event happens. Token vocabulary in [tasks/README.md#execution-log-template](../../../tasks/README.md#execution-log-template).)_
...
- [ ] `2026-05-30T14:14:31Z` intro HintsTest ← 11 cases: max 4 hints, token format "orient", --flag=<value> syntax, hints per mode (file/task/consumer/entity/graph/specs/spec/keyword/default)
