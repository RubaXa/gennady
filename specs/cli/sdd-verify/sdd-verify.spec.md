...
- **Детектор фиктивных скриптов ошибается в ОБЕ стороны, и вторая опаснее.** Пропуски: `isVacuousScript` — закрытый список однозначных shell-no-op (`echo …`, `true`, `:`, `exit 0`, пустой `node -e ""`, чистый `npm run`-переход) плюс маскировка кода возврата (no-op-фолбэк через `||`/`;`, после которого реальных команд не осталось). Он НЕ ловит написанную вручную всегда-успешную программу (`node -e "process.exit(0)"`, свой скрипт-обёртка): статически такая команда неотличима от маленького настоящего инструмента, не запуская её. Ложные срабатывания опаснее: честный проект уходит в `provisional` навсегда, кодовые фазы механически отказываются стартовать, а диагностика утверждает про его скрипты неправду — и НИКАКОГО waiver или override в цепочке нет. Разбор оболочки здесь эвристический (нет реального парсера shell): под подозрением остаются подстановки `$(...)`, `eval`, многострочные тела со сложным потоком управления и цепочки через не-npm раннеры (`pnpm`/`yarn`/`turbo` — `reachableScriptBodies` идёт только по `npm run`). **Предложенное усиление:** waiver-механизм на уровне readiness (явная строка «этот скрипт настоящий, вот почему»), чтобы ложное срабатывание было обходимо без правки детектора; и/или семантические признаки прогона у `type-check`/`test` по аналогии с `test:coverage` — отвергнуто пока как завязка на форматы вывода конкретных инструментов.
