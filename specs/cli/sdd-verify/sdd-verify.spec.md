...
- **Depends on:** `node:child_process` (spawnSync), `node:fs` (fingerprint дерева для repair-pass), `#logger`, `shared/sdd/readiness.ts` (`isVacuousScript`/`silencesExitCode` — один детектор фиктивных скриптов на readiness и на лестницу)
