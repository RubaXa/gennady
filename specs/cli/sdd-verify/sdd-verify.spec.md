...
- Coverage — только производство отчёта — `test:coverage` гоняет тесты с инструментовкой (вердикт = exit code); порог считает `testcov --min` в тест-фазе/аудите, не sdd-verify
...
- Postconditions:
  ...
  - `test:coverage` — вердикт по exit code скрипта; порог покрытия здесь НЕ считается (это `testcov --min` в тест-фазе/аудите)
...
- **Status:** active · **Supersedes:** прежнюю «семантическую проверку свежего артефакта» (введена и откачена в рамках той же реформы)
- **Why:** Была добавлена проверка «после `test:coverage` в `coverage/` появился свежий отчёт, иначе красный» — как анти-фикция «exit 0, но покрытие не мерялось». По решению оператора откачена: (1) она подделываема (пустой `touch coverage/x` проходил, 2s-допуск); (2) главное — она отвечала не на тот вопрос. Реальный контроль покрытия — это `gennady testcov --run --min=<pct> <файлы задачи>` в ТЕСТ-ФАЗЕ, scoped на Target Files задачи (агент пишет тесты по сценариям → гоняет → проверяет, что покрытие ЕГО файлов ≥ порога), плюс независимая перепроверка на аудите. `testcov` читает `coverage-final.json` сам и отдаёт текстовый вердикт; sdd-verify в это не лезет.
