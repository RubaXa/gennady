...
- **Fail-closed:** если удалить старый отчёт НЕ удалось (read-only каталог, права), probe запоминает его `mtime` и требует СТРОГО НОВЕЕ — producer, ничего не записавший, оставляет прежний mtime → gate краснеет. Подделка `touch`-ем не проходит: нужен реально перезаписанный (removed-then-recreated или более новый) отчёт.
- **Отличие от откаченной проверки (D-SV013):** та проверяла лишь «файл появился» с 2s-допуском и подделывалась `touch coverage/x`. Эта — delete-before + require-strictly-fresh, fail-closed, и НЕ парсит istanbul/порог (владелец порога — testcov).
