...
Детерминированный gate верификации фазы. Прогоняет ФИКСИРОВАННЫЙ набор проектных скриптов по точным именам — `format → lint → typecheck → test:coverage` — мутирующие первыми (autofix переписывает файлы, поэтому не гоняется параллельно с чтением). Вывод краткий: на успехе строка на gate; детали — только у упавших. Реверс-спека — `verify.sh` (идея gate), но без fuzzy-классификатора (D-SV004).

**Key properties:**

- Exact gates — ровно `npm run format/lint/typecheck/test:coverage`; никакого обнаружения/угадывания
...
- Порядок нормативен: format → lint → typecheck → test:coverage
