...
- exit `2` ⇔ `--all` резолвит ноль тикетов (v2 и легаси вместе, `ticketRefs.length === 0`) под `[project-root]` — неверный корень или дерево без единого тикета не считается «чисто», `SDD_NO_TICKETS_FOUND` (error, доминирует над обычным exit 1); `--task`/`--spec`/`--changed` этот код не производят
