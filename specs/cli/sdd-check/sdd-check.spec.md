...
Механический аудит SDD-артефактов — детерминированная половина `audit`/`check`. `sdd-check --task <ticket>` проверяет один тикет; `sdd-check --task <created-ticket-path> --authoring` до индексации проверяет authoring-инварианты всего нового тикета, а опциональный `--phase P<N>` сужает обратную связь до одной фазы и её overview/dependency boundary; `--scaffold-feasibility [root]` проверяет весь уже материализованный ticket graph против clean HEAD до semantic critic: выбранные capability adapters, единственного DAG-reachable provider для каждого requirement, точных package providers, DAG-сериализованных manifest/lock writers и exact command probes для command-bearing BDD. GREEN дополнительно печатает одну строку `critic-context: <JSON>` схемы `sdd-scaffold-critic-context/v1`: полный набор ticket paths/dependencies, clean-HEAD package/scripts, глобальные package owners, per-phase targets/profile/ladder/bootstrap, Verification routing, command-to-test-phase proof, coverage ownership и только уже известные typed schema gaps. Отсутствующий в clean HEAD будущий npm script остаётся `cleanHeadScript:null`, а не gap. Это авторитетный механический ввод critic; если семантический вывод требует отсутствующего внешнего tool/generator contract, critic возвращает `TOOL_CONTRACT_MISSING`. `--all [root]` обходит `specs/`; `--review-state <primary> [secondary...]` печатает канонические spec-only review-set, manifest-derived write-set и changed-state интегрированной пачки; `--review-ready <spec-or-dir>` fail-closed проверяет эту пачку и ровно одну primary critic history. После readiness отдельный `--review-publication <primary> [secondary...]` выводит role-bearing publication-set для VCS: write-set спеки плюс только механически атрибутированные durable research/portal/index/session-ignore outputs. Он не расширяет critic ownership. Готовность — sensor `CLEAN` без последующих edits до cap либо operator `CLEAN` в cap при `Changes: none`. Это основной потребитель механики; семантические проверки остаются за аудит-АГЕНТОМ.
...
| `checkScaffoldFeasibility`               | Utility      | Полный materialized graph против clean HEAD: capability adapters/providers, package materialization, DAG-serialized manifest/lock writers и BDD command probes до semantic critic |
...
### D-CK035 — Capability and dependency boundaries are graph facts

- **Status:** active · **Extends:** D-CK032, D-CK034
- **Why:** infrastructure scaffold selects only requirement-relevant adapters, declares capability provides/requires, and proves one current/upstream provider through the ticket+phase DAG. Dependency-install phases declare exact packages plus adapter manifest/lock targets. Multiple writers are valid when DAG-serialized; unordered writers, missing providers/packages/rules, and command probes without an owning test phase fail with distinct actionable diagnostics before execute. Core validation is adapter-neutral; Node/npm and TypeScript are separate built-ins. TypeScript test, ESLint-lint, and format tooling are independent field-backed capabilities with canonical gate requirements; only the exact ESLint capability activates `eslint-setup.xml`, so test/format-only scopes do not inherit an unrelated rule and the removed monolithic quality token cannot prove three gates at once.
<!--/SECTION:MODULE_DECISION_LOG-->
