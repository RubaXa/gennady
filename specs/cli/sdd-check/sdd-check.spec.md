...
- **Risk accepted:** Ноль новых error на всём корпусе (весь корпус на момент записи ещё v1); единственная новая находка на реальном тикете — warn (`tasks/cli/vcs-context-resolver/vcs-context-resolver.task-68.md`, ранее ложно закрытый `.skip`-кейс). Поддержанный синтаксис-subset `extractTestCaseNames` (B2-22/B2-24): `it`/`test`/`describe`/`suite`, модификаторы `skip`/`todo`/`only` в любой позиции цепочки, контейнерная неактивность (`describe`/`suite` со `skip`/`todo`) всегда перекрывает состояние вложенного `it`/`test`. НЕ поддержано: динамически вычисляемые имена кейсов/контейнеров, интерполяция в шаблонной строке имени, `.each(...)`-табличные формы, рантайм `t.skip()` внутри тела теста, алиасы контейнеров (`context`, `fdescribe`, `xdescribe`). Непокрытая форма не матчится вообще — её имя **неизвестно** экстрактору (ни observed, ни excluded), а не ложно закрыто: `checkBddCoverage` закрывает только то имя, которое реально нашло в `caseNamesByFile`. Известный остаток — `getTestFileIndex` (`cli/cmd/sdd-check/sdd-check.cmd.ts:532`) индексирует только `\.(test|spec)\.(ts|tsx|js)$`; тест-файл другого расширения (например `.swift`) не находится вовсе, и claim по нему остаётся `SDD_BDD_SCENARIO_UNTESTED` независимо от реального покрытия — это отдельный дефект индекса, вне зоны `bdd-coverage.ts` (см. доску, `B2-25`).
<!--/SECTION:MODULE_DECISION_LOG-->
