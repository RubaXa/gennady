...
- exit `2` ⇐ `--all` резолвит ноль тикетов (v2 и легаси вместе, `ticketRefs.length === 0`) под `[project-root]` — неверный корень или дерево без единого тикета не считается «чисто», `SDD_NO_TICKETS_FOUND` (error, доминирует над обычным exit 1). Не биимпликация: `--task <неизвестный Task-ID>` тоже даёт exit `2` (`ERR_CLI_SDD_CHECK_UNKNOWN_ID`, см. `sdd-check.types.ts#unknownIdError`) — код 2 общий для «нечего проверять» и «не нашли, что просили», различить причину можно только по коду находки, не по exit-коду; `--spec`/`--changed` этот код не производят
...
- `--task` на легаси-тикете классифицирует его как `--all` (`isTicket`/`isLegacyTicket`) прежде, чем гонять marker-зависимые v2-проверки: тот же файл даёт тот же набор находок и exit-код в `--task` и в `--all` (см. D-CK037)
- exit `4`, если выбран не ровно один режим из `--task`/`--spec`/`--all`/`--changed`
...
- FO-4: `--all` и `--changed` передают source header в единый file-relations resolver. Полностью V2 repo строг независимо от наличия `@spec`; в mixed repo явный `@spec` — положительный V2 marker, а нетронутый `@tasks`-only source остаётся V1 без новых ownership error/warn. Для V2 проверяются owner resolution, active-writer collision, active spec mismatch и запрет legacy `@tasks`; append-only `@tasks` действует только в V1.
- B2-12: consumers, BDD test index, changed-source selection и YAGNI используют один закрытый `shared/sdd/source-extensions.ts`. Уровень доказательства `exact` только у `.ts/.tsx`, для остальных поддержанных языков — `approximate`; это различие не повышает приблизительное наблюдение до точного.
...
| `checkFileOwnership`                     | Adapter      | Передаёт source header и canonical corpus в общий resolver; переводит только blocking→error / advisory→warn, не изобретая ownership policy                            |
| `SDD_SOURCE_EXTENSIONS`                  | Policy       | Единый закрытый набор source extensions для ownership/consumers/BDD/YAGNI; `sourceEvidenceLevel` различает exact TS/TSX и approximate остальные языки                 |
...
| `--spec <created-spec-path>` | string  | Structural draft-receipt gate для спеки (не тикета): скелет-секции/заголовки/маркеры/списки/REQ-ID; тривиальный auto-fix пробелов/отступов, контентные пробелы — только warn |
| `--authoring`                | boolean | Exact-ticket (или exact-spec с `--spec`) authoring gate перед переходом: все authoring-инварианты, без runtime/sibling/global checks                                         |
| `--phase P<N>`               | string  | Только с `--task --authoring`: проверить выбранную фазу и её overview/dependency boundary                                                                                    |
...
| `--format json`              | string  | Машиночитаемые findings (`code`/`file`/`section`/`reason`/`next`/`example`) вместо ESLint-style текста; `gennady.sdd-check.findings.v1`                                      |
...
Ровно один режим выбирается флагом: `--task`, `--spec`, `--all` или `--changed` (иначе exit 4).
...
- **Status:** active; severity-часть суждения по `SDD_BDD_COVERAGE_ROW_UNPARSED` superseded by D-CK038 (в части «severity фиксирована умышленно, без v1/v2-градации»)
...
- **Risk accepted:** Нет — оба нового кода репортят реальные, а не легаси-переходные, проблемы. `SDD_BDD_DEFERRED_TO_SELF` остаётся error, не градуируется по `flowVersion` — единичная настоящая находка, не легаси-шум. `SDD_BDD_COVERAGE_ROW_UNPARSED`'s severity по состоянию на этот тикет действительно была зафиксирована без v1/v2-градации — это суждение отменено `D-CK038` (B2-22): строка теперь градуируется по `flowVersion`, как `SDD_BDD_SCENARIO_UNTESTED`. Диагностика unparsed-row печатает две полные copy-ready формы замены, включая каноническую `- Deferred Test Ownership: <other-Task-ID> <scenario name> → \`<future-test-file>\` :: \`<canonical case name>\``, а не фрагмент грамматики.
...
- **Status:** active
- **Why:** тикеты с `PHASE_RECEIPTS:v1` не могут закрыть фазу чекбоксом или prose-строкой журнала. `--task` и `--all` читают paired receipt, пересчитывают structural phase plan и exact Target File hash и отклоняют missing/malformed/incomplete/stale evidence. Тикет без marker и без receipts не получает именно этот CLI-receipt gate; любой присутствующий receipt валидируется всегда. Отдельный B2-16b predicate канонического v2-имени включает structural Execution Log и group-receipt границы (D-CK040), но не фабрикует отсутствующие исторические CLI receipts.
...
### D-CK037 — `--all` с нулём тикетов — ошибка, не «чисто»; `--task` классифицирует легаси как `--all`

- **Status:** active
- **Why:** `sdd-check --all` на дереве без единого тикета (неверный `[project-root]`, либо специфицированный, но не заскаффолженный scope) печатал `✅ clean — 0 file(s) checked`, exit 0 — неотличимо от настоящего чистого прогона; issue #9.1. Теперь `--all` при `ticketRefs.length === 0` (ни v2, ни легаси-тикет не резолвлен) добавляет `SDD_NO_TICKETS_FOUND` (error) и форсирует exit `2`, различимый от «0 находок» (exit 0) и «есть находки» (exit 1). Отдельно найдена и исправлена асимметрия: `--task <легаси-тикет>` безусловно гонял полный v2 `checkTicket` (плюс `checkPhaseReceipts`/`checkRuleLinks`/`checkSpecRefs`/`checkTicketRulesCascade`/`checkTicketBddCoverage`/`checkTicketCoveragePolicy`) поверх голого v1-заголовка без `<!--SECTION-->`-якорей, давая ложные структурные ошибки (`SDD_MISSING_META`/`SDD_MISSING_EXECUTION_LOG` и т.п.), тогда как `--all` тот же файл классифицирует `isLegacyTicket` и репортит один `SDD_LEGACY_TICKET_UNANCHORED` (warn). `--task` (вне `--authoring`) теперь применяет ту же классификацию перед выбором проверок.
- **Risk accepted:** `--changed`/`--spec` намеренно не получают `SDD_NO_TICKETS_FOUND` — пустой diff (`--changed`) и одна spec (`--spec`) — легитимные нулевые множества, не признак неверного корня.

### D-CK038 — `SDD_BDD_COVERAGE_ROW_UNPARSED` градуируется по `flowVersion`; закрытие сценария по неактивному тесту невозможно

- **Status:** active · **Supersedes:** severity-суждение D-CK014 («severity фиксирована умышленно, без v1/v2-градации»)
- **Why:** B2-22 (`61-TASK-BOARD.md:96`, критерий `A19`, верификация `V-BATCH-16`) нашло два ложных закрытия. (1) `extractTestCaseNames` извлекало имя кейса из ЛЮБОГО модификатора `it`/`test`, включая `skip`/`todo` — заявленный сценарий, чей единственный тест был `it.skip(...)`/`it.todo(...)`, считался «наблюдаемым» и закрывался, хотя реальный тест никогда не запускался. Теперь модификатор-цепочка `it`/`test` читается целиком (`shared/sdd/bdd-coverage.ts:187-197` на момент записи): любая цепочка, содержащая `skip`/`todo`, исключает имя из «observed» — **закрытие сценария по неактивному тесту невозможно**. B2-24 (та же верификация, находка B-2) расширила это на контейнерные модификаторы: `describe.skip(...)`/`describe.todo(...)`/`suite.skip(...)`/`suite.todo(...)` делают неактивным всё вложенное `it`/`test`, независимо от их собственных модификаторов (`findInactiveContainerRanges`). (2) `checkUnparsedCoverageRows` — строка `## Test Scenario Coverage`, похожая на ряд, но не матчащая ни arrow-форму, ни `Deferred Test Ownership:` — раньше давала `warn` безусловно, вне зависимости от того, легаси это тикет или уже мигрированный на v2. Теперь severity градуируется по `flowVersion` тикета, по образцу `SDD_BDD_SCENARIO_UNTESTED`: `v1` — предупреждение (`L-3`, `D-39` — новый/переоценённый код не эскалируется на легаси-корпус, сохраняет существующий 140-строчный baseline `GAP-B-1`), `v2` — ошибка (fail-closed после миграции сценария на v2).
- **Risk accepted:** Ноль новых error на всём корпусе (весь корпус на момент записи ещё v1); единственная новая находка на реальном тикете — warn (`tasks/cli/vcs-context-resolver/vcs-context-resolver.task-68.md`, ранее ложно закрытый `.skip`-кейс). Поддержанный syntax-subset `extractTestCaseNames` (B2-22/B2-24): `it`/`test`/`describe`/`suite`, модификаторы `skip`/`todo`/`only` в любой позиции цепочки, контейнерная неактивность (`describe`/`suite` со `skip`/`todo`) всегда перекрывает состояние вложенного `it`/`test`. НЕ поддержано: динамически вычисляемые имена кейсов/контейнеров, интерполяция в шаблонной строке имени, `.each(...)`-табличные формы, рантайм `t.skip()` внутри тела теста, алиасы контейнеров (`context`, `fdescribe`, `xdescribe`). Непокрытая форма не матчится вообще — её имя **неизвестно** экстрактору (ни observed, ни excluded), а не ложно закрыто: `checkBddCoverage` закрывает только то имя, которое реально нашло в `caseNamesByFile`. B2-12 позднее сняла прежний долг индекса: Swift `FooTests.swift` индексируется общим source registry, а XCTest-style `func test…` наблюдается явно как approximate evidence.

### D-CK039 — FO-4 ownership checks используют общий resolver и V1 grandfathering
...
### D-CK040 — После самомиграции групповые receipts являются блокирующим доказательством

- **Status:** active · **Implements:** D-4, B2-16b, B2-20, A13
- **Why:** каноническое v2-имя тикета внутри v2-scope само включает receipt-aware contract, даже если literal marker потерян. Поэтому fully-DONE группа v2-тикетов без текущих audit/review receipts или со stale/invalid receipts получает `SDD_GROUP_AUDIT_MISSING` и `SDD_GROUP_REVIEW_MISSING` уровня `error`. Legacy-scope без marker остаётся вне нового гейта; частично мигрированная группа остаётся явным `SDD_GROUP_RECEIPT_PARTIALLY_MARKED` warning, а не ложным clean.
...
- **Depends on:** `shared/common/parse-args.ts`, `shared/common/changed-files.ts`, `shared/sdd/check.ts` (→ `section.ts`, `ticket.ts`), `shared/sdd/file-relations.ts`, `shared/sdd/source-extensions.ts`, `shared/sdd/portal.ts` (parseScopes + parseGraphEdges), `#logger`
