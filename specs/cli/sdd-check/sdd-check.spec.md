...
- Postconditions:
  ...
  - Тикет с текущим `<!--PHASE_RECEIPTS:v1-->` обязан иметь первый `### Round 1`, где мультимножество level-4 блоков `#### P<N>` точно равно строкам Phases Overview: отсутствие раунда/фазы, дубликат и лишняя фаза дают соответственно `SDD_EXECUTION_LOG_ROUND_MISSING`, `SDD_EXECUTION_LOG_PHASE_MISSING`, `SDD_EXECUTION_LOG_PHASE_DUPLICATE`, `SDD_EXECUTION_LOG_PHASE_ORPHAN` (error). Поздние раунды, `#### Round close` и заголовки внутри fenced code не участвуют; V2-тикеты до receipt schema marker остаются grandfathered.
