# Task: LIN-purpose — DisablesCheck: enforce purpose text (D-007 contract tightening)
...
- **Scope:** `cli`
- **Module:** `lint`
- **Dependencies:** LIN-disable
- **Spec References:**
  - Policy: [`cli.spec.md` D-007](../cli.spec.md)
  - Service: [`DisablesCheck`](./lint.spec.md)
  - DbC: [`Service: DisablesCheck`](./lint.spec.md)
  - Test Scenarios (additions DC-21..DC-25): [`§6.1 DisablesCheck`](./lint.spec.md)
...
Contract: см. Spec References. Полный список DC-NN — [`lint.spec.md §6.1`](./lint.spec.md).

**Feature:** Контракт типизации DisablesCheck (preserved from LIN-disable)
...
Полный список DC-NN — single source of truth в [`specs/cli/lint/lint.spec.md §6.1`](./lint.spec.md). Этот ticket добавляет 5 сценариев DC-21..DC-25.

- Scenario «Контракт типа сохранён» → `disables.check.test.ts` :: `DC-01 contract typing — check(content, filePath) returns LintError[]` (preserved from LIN-disable)
...
- Scenario «Приоритет UNAUTHORIZED при отсутствии D-NNN» → `cli/cmd/lint/__tests__/disables.check.test.ts` :: `DC-04 unauthorized @ts-expect-error → 1 error`
<!--/SECTION:TEST_COVERAGE-->
...
```
@audit task=LIN-purpose round=1 after-exec-round=1 triggered-reopen=none status=FAIL counts=B0·M1·m0·I0
F-01 | sev=M | type=TASK_ID_DRIFT | conf=H | loc=cli/cmd/lint/lint.types.ts:3 | src=ai/directives/sdd/audit.directive.xml#AX_TASK_ID_INTEGRITY | route=code-fix | act=обновить `// @tasks:` заголовок — добавить LIN-disable и LIN-purpose (файл изменён под обе задачи: ERR_CLI_LINT_UNAUTHORIZED_DISABLE в LIN-disable, ERR_CLI_LINT_DISABLE_MISSING_PURPOSE в LIN-purpose)
~applied | cli/cmd/lint/lint.types.ts | @tasks: LIN-types, LIN-targets → LIN-types, LIN-targets, LIN-disable, LIN-purpose; @consumers: добавлен DisablesCheck
```
