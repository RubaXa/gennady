...
| Фильтр расширений                         | Только `.ts`, `.tsx`; остальные файлы молча игнорируются                                                     |
| Дедупликация                              | Каждый файл возвращается один раз независимо от способа обнаружения                                          |
...
| Скрытые директории (`.`-префикс)          | Скрытые директории и файлы пропускаются                                                                      |
...
| DC-18 | Маркер в file-header `// @file: … D-042`                              | `[]` — file-header не несёт маркер отключения                                               |
...
- **Open risks & validation needs:**
  - `refine dbc` (DL-content) должен быть выполнен до реализации `DbcContractCheck`
