...
| `InventorySyncCheck`   | Service      | Активируется только с `--spec=<module-spec>`. Прямое направление: экспорт файла отсутствует в Entity Inventory спеки → `ERR_CLI_LINT_INVENTORY_UNDECLARED` (по каждому файлу). Обратное направление (только с `--inventory-reverse <dir>`): после полного обхода `<dir>` каждая сущность инвентаря обязана быть экспортирована хоть одним файлом → `ERR_CLI_LINT_INVENTORY_UNIMPLEMENTED` (ошибка на файле спеки), КРОМЕ сущностей, помеченных в спеке `Deferred Implementation: <TSK-id>` — такая сущность выводится информационной строкой, не считается расхождением (D-016).                |
...
- Сущность, помеченная в тексте спеки маркером `Deferred Implementation: <TSK-id>` (строка таблицы ``| `Name` | ... Deferred Implementation: TSK-1 |`` или bullet ``- `Name` — ... Deferred Implementation: TSK-1``), исключается из ошибок — обратная сверка не считает её расхождением, только собирает в `ReverseSweepResult.deferred` и печатает как info-строку с указанием task-id
