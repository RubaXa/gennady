# Task: UC-notify — Bootstrap + Impl: update-check механизм
...
- **Dependencies:** None
- **Spec References:**
  - Contracts: [`UpdateCheck`](./update-check.spec.md#updatecheck)
  - Contracts: [`UpdateCheckWorker`](./update-check.spec.md#updatecheckworker)
  - Contracts: [`UpdateCheckCache`](./update-check.spec.md#updatecheckcache)
  - Constraints: [`cli spec §4.1.3`](../cli.spec.md) — FR-SU-01..14
  - Architecture: [`cli spec §5.4`](../cli.spec.md)
- **Runtime Backing:** `real-runtime`
- **Verification Levels:** `type-check` (unit tests deferred to UC-tests)
...
Сценарии покрываются в UC-tests.

- Notification suppressed without TTY → `cli/cmd/_shared/__tests__/update-check.test.ts` :: `no TTY — notification suppressed`
...
_(Round = один execute-then-audit цикл. Per-phase блоки внутри Round. Skeleton — минимальный; строки событий (`intro` / `decision` / `tried` / `discovery` / `insight` / `BLOCKED`) появляются ТОЛЬКО когда событие произошло. Словарь токенов в [tasks/README.md#execution-log-template](../../../tasks/README.md#execution-log-template).)_
