...
- **Spec References:**
  - Module spec: [`sync-skills.spec.md`](./sync-skills.spec.md)
  - Scope spec: [`cli.spec.md §5.7`](../cli.spec.md)
  - FR: [`cli.spec.md §4.1.6`](../cli.spec.md) — FR-SS-01..16
  - DX: [`cli.spec.md §3.6`](../cli.spec.md)
  - Decision: [D-011](../cli.spec.md) — команда sync-skills
  - Decision: [D-M005](./sync-skills.spec.md) — отдельная команда
  - Decision: [D-M006](./sync-skills.spec.md) — orphan-удаление
  - Contract: [`SyncSkillsCore`](./sync-skills.spec.md)
  - Contract: [`SyncSkillsFormatter`](./sync-skills.spec.md)
...
**Feature:** `sync-skills` копирует только доказанные skill-источники

**Scenario:** Валидный source синхронизируется [`integration`]

- **Given** sourceDir с валидным skill
- **When** выполняется `sync-skills`
- **Then** skill появляется в targetDir

**Scenario:** Ошибка: sourceDir не найден [`integration`]

- **Given** несуществующий sourceDir
- **When** выполняется `sync-skills`
- **Then** команда возвращает exit 1 и не пишет target
...
- valid source is synchronized → `sync-skills.cmd.test.ts` :: `syncs all skills from source to target`
- missing sourceDir is rejected → `sync-skills.cmd.test.ts` :: `exits 1 when sourceDir does not exist`
...
- [ ] `<ts>` ver `npm run type-check && npm test` → pass exit=0
- [ ] `<ts>` DONE
