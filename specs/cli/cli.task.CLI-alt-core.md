# Task: CLI-alt-core — AltOpinion Core (types + parser + runner)

<!--SECTION:META-->

## 1. Meta & Traceability

- **Task-ID:** CLI-alt-core
- **Status:** [x] DONE
- **Reopens:** 5
...
- **Deferred Test Ownership:** CLI-alt-test (runner BDD-сценарии — тесты раннера в `alt-opinion-runner.test.ts`)
- **Scope:** cli
- **Module:** alt-opinion
- **Dependencies:** None
- **Spec References:**
...
- **Historical References:** Module spec D-006, D-007: `alt-opinion.spec.md` §6 (superseded by the scope spec)
- **§Effective Rules:**
...
```
@audit task=CLI-alt-core round=1 after-exec-round=7 triggered-reopen=Round-8 status=FAIL counts=B0·M4·m2·I1 phases_to_fix=[P1, P2]
...
F-07 | sev=I | type=BDD_COVERAGE_MISMATCH | conf=M | loc=tasks/cli/alt-opinion/cli-alt-opinion.task-23.md:114 | phase=— | src=specs/cli/alt-opinion/alt-opinion.spec.md | route=ticket-update | act=добавить в ticket Meta `Deferred Test Ownership: CLI-alt-test` для runner BDD-сценариев — тесты раннера существуют в `alt-opinion-runner.test.ts` (созданы CLI-alt-test/CLI-alt-tele)
```
...
- [x] `2026-05-21T17:05:00Z` F-07: added Deferred Test Ownership: CLI-alt-test to ticket Meta
...
```
@audit task=CLI-alt-core round=2 after-exec-round=8 triggered-reopen=none status=PASS counts=B0·M0·m0·I0
...
~applied | ticket Meta | добавлен Deferred Test Ownership: CLI-alt-test
```
