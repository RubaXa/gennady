...
каждый с полным текстом одного шага и его одношаговыми аксиомами. Пакет доставляется путём: шаг скелета несёт обязательную runtime-инструкцию `READ_AND_USE_DIRECTIVE("steps/<step-id>.xml")`; хост исполняет её штатным чтением файла, без CLI-инструмента (DA-DL-19 supersedes форму подсказки из DA-DL-12). Версия сборки — человекочитаемая npm-версия пакета (`0.8.4-draft.40`) печатается один раз в шапке скелета и первой строкой каждого файла пакета, видна в трейсе через обычный Read.
...
_Путь данных: сборка кладёт скелет и пакеты на диск, а скелет императивно активирует нужный пакет через `READ_AND_USE_DIRECTIVE("<path>")`; хост читает этот путь штатным Read — DA-REQ-4, DA-REQ-5, DA-REQ-7._
...
**Когда** скелет описывает шаг в списке шагов, **генератор должен** непосредственно перед его
исполнением потребовать точный вызов
`READ_AND_USE_DIRECTIVE("ai/directives/sdd-v2/<directive>/steps/<step-id>.xml")`. Это обязательная
runtime-активация, а не discoverable-подсказка: хост читает названный файл штатным Read, без
CLI-команды и без аргумента версии.
...
> команду без эмпирики: точный императив с путём дешевле закрывает тот же риск (DA-DL-19,
> расширяет DA-DL-12, supersedes DA-DL-4).
> `sdd-step` — DEFERRED_DECISION (DA-DL-15): возврат, если живые прогоны покажут потерю агентов на
> сырых ошибках чтения.

### DA-REQ-6 [должен]

**Скелет lazy-директивы должен** укладываться в целевой бюджет ≤6000 токенов (`SKELETON_TOKEN_TARGET`,
мягкий — превышение отчитывается предупреждением, сборку не проваливает) и никогда не превышать
жёсткий потолок 8000 токенов (`SKELETON_TOKEN_LIMIT`, жёсткий — превышение проваливает сборку);
каждый сгенерированный пакет шага должен укладываться в лимит ≤20 000 символов И ≤2000 символов на
строку — оба жёсткие (DA-DL-5, обновлено DA-DL-14, величина обновлена DA-DL-16) — все четыре
бюджета (один мягкий, три жёстких) проверяются механическим гейтом в CI (`StepBudgetGate`,
DA-REQ-14); мягкая цель скелета не была механически наблюдаемой до DA-DL-18 — до этой записи
существовала только `SKELETON_TOKEN_LIMIT`, и текст цели оставался декларативной строкой спеки, не
проверяемой ничем.
...
> пересчётом. Различие «цель — предупреждение, потолок — ошибка» введено DA-DL-18 после того, как
> реальная сборка (`e08460c3`) молча нарушила декларативную цель (`phase-execution-protocol`: 6009
> токенов против заявленных ≤6000) без единого красного гейта и пережив два раунда аудита — до
> DA-DL-18 цель нигде не была механически представлена, только упомянута в тексте этого требования.
...
> Под механизмом `READ_AND_USE_DIRECTIVE(path)` + Read (DA-DL-19) «файл пакета отсутствует» — дефект сборки, а не сценарий
...
**Если** при сборке скелет lazy-директивы превышает жёсткий потолок 8000 токенов
(`SKELETON_TOKEN_LIMIT`), ИЛИ сгенерированный пакет шага превышает 20 000 символов, ИЛИ какая-либо
строка пакета превышает 2000 символов (DA-DL-5, обновлено DA-DL-14, величина обновлена DA-DL-16),
**то механический CI-гейт должен** провалить сборку, назвав директиву (и, для пакета, конкретный
шаг), какой из трёх жёстких лимитов превышен, и величину превышения — файлы не записываются на диск
с превышением молча. **Если** при этом скелет превышает только мягкую цель ≤6000 токенов
(`SKELETON_TOKEN_TARGET`) без превышения жёсткого потолка, **то тот же гейт должен** напечатать
предупреждение с тем же составом (директива, фактический размер, цель, величина превышения) и
завершиться успешно (exit 0) — цель не проваливает сборку, только жёсткий потолок это делает
(DA-DL-18).
...
> Это применимо к жёсткому потолку без исключений; мягкая цель — намеренное исключение из этого
> правила (DA-DL-18): она остаётся аспирацией, не блокером, но перестаёт быть чисто декларативной —
> гейт теперь называет её превышение вслух, вместо того чтобы промолчать, как это произошло с
> `phase-execution-protocol` (6009 против ≤6000) до появления `SKELETON_TOKEN_TARGET`.
...
- **Provides to:** N/A (отложенный `sdd-step` — потенциальный потребитель из другого скоупа, не
  модуль этого)
- **Test-time dependency (cross-scope):** снята — доставка идёт императивом `READ_AND_USE_DIRECTIVE(path)` через штатный Read (DA-DL-19), не требует
...
| `SKELETON_TOKEN_TARGET`          | Constant     | Мягкая цель токенов скелета = 6000 — превышение отчитывается предупреждением, не проваливает сборку (DA-REQ-6, DA-DL-18)                                                        |
...
| `StepBudgetSeverity`             | Value Object | `'warning' \| 'error'` — `'warning'` только для превышения мягкой цели скелета, `'error'` для любого из трёх жёстких лимитов (DA-DL-18)                                         |
| `StepBudgetFinding`              | Value Object | Одно превышение бюджета: артефакт, вид лимита, severity, лимит, фактическое измеренное значение, величина превышения                                                            |
| `check`                          | Function     | Измеряет `DirectiveSkeleton`/`StepPackage[]` против четырёх бюджетов (одна мягкая цель + три жёстких лимита), возвращает список находок (пустой при отсутствии превышений)      |
...
runtime-ошибка выдачи) → владелец: `LazyDirectiveAssembler`. При механизме А (`READ_AND_USE_DIRECTIVE(path)` + Read) отдельного
...
- **Purpose:** Механически проверяет размер скелета и каждого пакета против бюджетов DA-REQ-6/DA-REQ-14 — три жёстких лимита и одну мягкую цель (DA-DL-18).
- **Public Operations:** `check(skeletonText, packages)` — измеряет и возвращает список
  находок, каждая с полем `severity`; лимит-константы `SKELETON_TOKEN_TARGET` (6000 токенов,
  мягкая цель, DA-DL-18), `SKELETON_TOKEN_LIMIT` (8000 токенов, жёсткий потолок),
  `PACKAGE_CHAR_LIMIT` (20 000 символов, DA-DL-16), `PACKAGE_LINE_CHAR_LIMIT` (2000 символов);
  входные типы `StepPackageInput`, `StepBudgetFinding`, `StepBudgetSeverity`
- **Errors & Degradation:** Находка с `severity: 'error'` (любой из трёх жёстких лимитов) →
  сборка (или отдельный CI-шаг) завершается ошибкой (exit 1); находка с `severity: 'warning'`
  (только превышение мягкой цели скелета) → печатается предупреждение, сборка завершается успешно
  (exit 0); список пуст → тихий успех
- **Consumers:**
...
- **Purpose:** Проваливает сборку/CI при превышении жёстких бюджетов скелета или пакета;
  предупреждает (не проваливает) при превышении мягкой цели скелета (DA-DL-18).
- **Consumers:**
...
- `check`:
  ...
  - Post: скелет ≤8000 токенов (жёсткий потолок) И пакет ≤20 000 символов И каждая строка пакета
    ≤2000 символов (DA-DL-5/14/16) → пустой список находок при этом скелет также ≤6000 токенов
    (мягкая цель); скелет в диапазоне (6000, 8000] токенов при соблюдении пакетных лимитов → ровно
    одна находка с `severity: 'warning'`, `limitKind: 'skeleton-tokens-target'`; скелет >8000
    токенов ИЛИ любой пакетный лимит превышен → находка(и) с `severity: 'error'`, каждая с
    артефактом, видом лимита, лимитом, фактическим значением и величиной превышения (DA-DL-18)
...
- `ai/kit/step-budget-gate.ts`: `StepBudgetGate` (Module: `check`, `SKELETON_TOKEN_TARGET`, `SKELETON_TOKEN_LIMIT`, `PACKAGE_CHAR_LIMIT`, `PACKAGE_LINE_CHAR_LIMIT`, `StepPackageInput`, `StepBudgetFinding`, `StepBudgetSeverity`)
...
production-сущность; плюс одна запись после финального ревью: мягкая цель скелета ≤6000 токенов
стала механически наблюдаемой (`SKELETON_TOKEN_TARGET`, предупреждение), а не только декларативной
строкой спеки — восемнадцать записей всего.
...
### DA-DL-18 2026-08-23 — мягкая цель скелета ≤6000 токенов стала механически наблюдаемой: введена `SKELETON_TOKEN_TARGET = 6000` рядом с жёстким `SKELETON_TOKEN_LIMIT = 8000`; превышение цели без превышения потолка — находка с `severity: 'warning'`, печатается CLI-гейтом, но не проваливает сборку; превышение потолка остаётся `severity: 'error'`, exit 1, без изменений (почему: финальное ревью зафиксировало, что реальная сборка (`e08460c3`) уложила `phase-execution-protocol` в 6009 токенов — на 9 больше декларативной цели ≤6000, которую DA-REQ-6 называл словами с самого начала спеки — и это прошло незамеченным сквозь `check:directive-budgets`, независимый аудит и повторный аудит: в коде существовала только `SKELETON_TOKEN_LIMIT`, константы цели не было вовсе, поэтому цель не проверялась ничем и не могла быть нарушена «механически» — она молча нарушалась текстуально; тот же дефект зафиксирован и в Execution Log тикета, и в тексте коммита `e08460c3` числом 5915 вместо фактических 6009 — ошибка измерения (не правка кода, git diff по файлу пуст с момента коммита) пережила и P6, и первый аудит, и код-ревью, потому что ни один гейт не мог её поймать; отвергнуто: поднять `SKELETON_TOKEN_LIMIT` до значения, включающего 6009 (например, оставить один потолок 8000 и молча принять 6009 как «внутри бюджета») — отклонено, это стирает различие цель/потолок, которое DA-REQ-6 всегда декларировал словами, вместо того чтобы наконец его механически закрыть; отвергнуто: оставить цель декларативной и полагаться на ручной аудит — отклонено, ровно этот путь уже дважды не поймал 6009 (сборка + первый аудит); риск: пилот `phase-execution-protocol` теперь отчитывается предупреждением при каждом запуске `check:directive-budgets`, пока скелет не сокращён под 6000 либо цель не пересмотрена — см. Handoff Open risks)

### DA-DL-19 2026-08-30 — supersedes discoverable-only форму DA-DL-12: каждый lazy-шаг несёт точный императив `READ_AND_USE_DIRECTIVE("<package-path>")` непосредственно перед исполнением шага; канал остаётся штатным чтением файла и не возвращает отложенный `sdd-step` (почему: независимый prompt-аудит показал разрыв между «путь виден» и «пакет нормативно загружен» — агент мог перейти к gist шага, ни разу не прочитав его полное тело; точный runtime-императив закрывает этот разрыв без нового CLI и без повторения его параметров; инспектор извлекает тот же путь из императива и разворачивает пакет как раньше; отвергнуто: оставить путь с пояснением `Read tool` — отклонено, это доступность, а не обязательность загрузки; отвергнуто: вернуть `sdd-step` — отклонено, для обязательности достаточно существующего `READ_AND_USE_DIRECTIVE`, а новая команда снова добавила бы отдельный API и help-поверхность)
...
- **Open risks & validation needs:**
  ...
  - Один из трёх пилотов (`phase-execution-protocol`, 6009 токенов на реальном собранном скелете)
  идёт над мягкой целью ≤6000 токенов (`SKELETON_TOKEN_TARGET`, DA-DL-18) при соблюдении жёсткого
  потолка 8000 — `check:directive-budgets` теперь печатает это как предупреждение при каждом
  прогоне, сборка не проваливается. Требуется решение оператора: либо сократить скелет
  `phase-execution-protocol` (кандидаты — уплотнить список шагов или пересмотреть состав сквозных
  аксиом per DA-REQ-9), либо осознанно пересмотреть саму цель ≤6000 для этого класса директив.
  Не блокирует текущий гейт (цель мягкая по определению), но остаётся открытым до одного из двух
  исходов.
  <!--/SECTION:HANDOFF-->
