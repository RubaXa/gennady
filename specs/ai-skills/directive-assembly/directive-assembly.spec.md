...
- **Given** после правки пакет шага вырос сверх 8000 символов ИЛИ содержит строку длиннее 2000
...
лимит ≤8000 символов И ≤2000 символов на строку (DA-DL-5, обновлено DA-DL-14) — все бюджеты
проверяются механическим гейтом в CI (`StepBudgetGate`, DA-REQ-14).
...
> пакета больше не обосновывается «хардлимитом CLI» (механизм А не идёт через CLI): факт-чек
> (`2026-08-22-truncation-and-fingerprints.research.md`, DA-DL-14) показал, что усечение не грозит
> на 8000 символах ни на одном хосте (порог хоста больше в 3.75–6.25 раза) — единственный реальный
> риск построчный, оба хоста режут строку на 2000 символах независимо от размера файла, отсюда
> отдельный лимит ≤2000/строку.
...
превышает 8000 символов, ИЛИ какая-либо строка пакета превышает 2000 символов (DA-DL-5, обновлено
DA-DL-14), **то механический CI-гейт должен** провалить сборку, назвав директиву (и, для пакета,
конкретный шаг), какой из трёх лимитов превышен, и величину превышения — файлы не записываются на
диск с превышением молча.
...
- `check`:
  ...
  - Post: скелет ≤8000 токенов И пакет ≤8000 символов И каждая строка пакета ≤2000 символов
    (DA-DL-5/14) → пустой список находок; иначе — находки с артефактом, лимитом и величиной превышения
...
Пятнадцать записей: режимы сборки и приоритет, форма скелета/пакета, CLI-выдача и честная ошибка
...
версии, обновлённое обоснование лимитов, DEFERRED_DECISION про `sdd-step`.
...
- **Open risks & validation needs:**
  - Точный файл-путь директивы `scaffold` (`scaffold.directive.xml` или иное реальное имя шаблона)
    ...
    пакетов
  - Лимиты пакета (≤8000 символов, ≤2000/строку, DA-DL-14) обоснованы факт-чеком документации/кода
    хостов (`2026-08-22-truncation-and-fingerprints.research.md`), не живым прогоном; требует
    проверки живым Read реального пакета внутри Claude Code и opencode до включения гейта в CI
