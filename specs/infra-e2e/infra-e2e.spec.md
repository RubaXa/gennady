...
| Term                | Definition                                                                                                                                              |
| ------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **E2E-набор**       | Модуль-владелец группы E2E-тестов одной поверхности: `cli/e2e`, `stack/e2e`, `config` (§4). Имеет мейнтейнера, свою npm-команду и свой временный корень |
...
4. **Тихих скипов не бывает.** Каждый скип попадает в итоговую сводку набора (`SKIPPED <n>: <ids>` с подсказкой про STRICT) и превращается в отказ в STRICT-режиме (§5).
...
6. **Ноль мутаций отслеживаемых файлов.** Набор не меняет ни одного tracked-файла репозитория; собственные артефакты (`dist/**`, `*.tgz`) — gitignored и убираются в `finally` (сегодня — `scripts/cleanup-publish-artifacts.ts` в setup наборов). Автоматической сверки пустого `git status --porcelain` после прогона в наборах пока нет — чистота держится на уборке артефактов.
...
STRICT-переменные обрабатываются одним общим раннером (`services/stack/__tests__/e2e/suite.ts`): любая из них со значением `1` включает STRICT для **всех** наборов текущего прогона, а не только для «своего». Колонка выше фиксирует, какую переменную выставляет CI-джоба набора, а не изоляцию действия.
...
- Внешний тулчейн отсутствует → фикстура **скипается**; её id попадает в итоговую сводку набора (`SKIPPED <n>: <ids>` плюс подсказка включить STRICT). Отдельная причина по каждой фикстуре не печатается — она всегда одна: недостающий тулчейн (доступность и версии тулчейнов набор печатает при старте). Локальная разработка без `golangci-lint` или `go` обязана оставаться возможной.
- `<SUITE>_STRICT=1` → тот же случай становится **отказом** с текстом `TOOLCHAIN_MISSING: <ids> not in PATH (strict mode)`; инструкции по установке в сообщении нет.
...
| `unit`       | `npm test`                                                                        | node                                 |
| `packaging`  | `npm run build` + `test:smoke` — бандл стартует, плагин-ассеты доезжают до пакета | node                                 |
...
| `stack-e2e`  | `test:stack-e2e` со `STACK_E2E_STRICT=1`                                          | node, git, **go**, **golangci-lint** |
...
- **`golangci-lint` — уже не в этом списке.** Фикстура `go-lint-exit2` его требует, поэтому джоба ставит пиннутый `v2.12.2`. Механизм отработал как задумано: probe-лист собирается из `requires` фикстур, поэтому добавление фикстуры **само** потребовало правки CI — без неё STRICT уронил бы `stack-e2e` с `TOOLCHAIN_MISSING`. Спекулятивно тулчейны по-прежнему не ставятся.
- **`docker`** — по той же причине; `requires`-предусловия проверяются фикстурами без поднятия контейнеров.

**Измеренная стоимость (локально, полный тулчейн).** `stack-e2e` — ~36s wall на стековые наборы (сегодня их три: `anystack`, `golang`, `node`). Общий `GOCACHE` (`STACK_E2E_GOCACHE`, кэшируется `actions/cache`) снижает CPU 38s → 26s, но **wall-clock почти не меняет**: время доминирует per-suite сборка артефакта (`build:publish` + `pack` + install), а не компиляция Go. Кэш оставлен как дешёвый (67MB), но ускорения от него ждать не следует.

**Оффлайн-путь `cli-e2e` — реализован.** Setup набора (`cli/__tests__/e2e/setup.ts`) использует тот же приём, что и стековые наборы: реестр параметризован (`GENNADY_E2E_REGISTRY`), при отказе установки следует `npm install --offline`-ретрай по прогретому кэшу. За корпоративным прокси, который отдаёт `403` на публичные зависимости (`ink`), достаточно выставить реестр или прогреть кэш `npm ci`.
...
- **Governs:** [`cli/e2e`](../cli/e2e/e2e.spec.md), [`stack/e2e`](../stack/e2e/e2e.spec.md), E2E-часть [`config`](../config/config.spec.md)
...
- **Files created:** `.github/workflows/ci.yml` (§6) — семь джоб, все команды не-мутирующие
- **Structural changes (сделаны):**
  - `package.json`: переименование `test:e2e` → `test:cli-e2e` (единая стратегия `test:<suite>-e2e`), новые `test:stack-e2e` и `test:config-e2e`; STRICT-переменные в `prepublishOnly` — сделано
  - `infra-base` §2.1: обязательный паттерн исключения фикстур расширен на `**/__tests__/e2e/fixtures/**` и `plugins/*/e2e/fixtures/**` (`.prettierignore`, `tsconfig.json`) — фикстуры намеренно содержат невалидный YAML/JSON — сделано
  - `cli/__tests__/e2e/setup.ts` + `cli/e2e` D-015: (а) приведён к D-IE2E-002 (`build:publish`); (б) реестр параметризован (`GENNADY_E2E_REGISTRY`) + `--offline`-ретрай — сделано; (в) у `cli/e2e` нет тулчейн-зависимых скипов, поэтому STRICT-переменной у него нет — единственный гейт `GENNADY_E2E=1`
  - `specs/README.md`: scope `infra-e2e` зарегистрирован в графе и таблице — сделано
- **Отложено (решение мейнтейнера):**
  - **Первый прогон в upstream.** Actions в `RubaXa/gennady` включены, но прогонов по PR из форка так и не появилось: ни check-run, ни статуса `action_required`. Вероятные причины — политика запуска workflow из форков либо требование, чтобы workflow существовал в дефолтной ветке базового репозитория. Диагностируется только из настроек репозитория, поэтому вынесено из этой задачи; `workflow_dispatch` в workflow добавлен именно для этого случая.
  - **Branch protection / required checks.** Пометить `lint`, `unit`, `config-e2e`, `stack-e2e`, `dogfood` обязательными имеет смысл после первого зелёного прогона: до него правило блокирует мерджи, ничего не проверяя. Без этого шага CI остаётся советующим, а STRICT-гарантия — опциональной на практике.
- **Open risks:**
  - workflow в репозитории есть (`.github/workflows/ci.yml`, §6), но включение Actions и required checks — решение мейнтейнера (см. «Отложено» выше). Спека фиксирует требуемые свойства, чтобы любая альтернативная реализация (GitLab CI, self-hosted) была проверяема
