# Task: VC-gitlab — vcs-client: GitLab adapters (RepositoryFiles + getChanges)
...
- **Spec References:**
  - [`VcsGitlabRepositoryFiles`](./vcs-client.spec.md#vcsgitlabrepositoryfiles)
  - [`VcsGitlabMergeRequests.getChanges`](./vcs-client.spec.md#vcsgitlabmergerequests)
...
| `node --test services/vcs-client/__tests__/gitlab/` | node-test        | extra |
| `npx prettier --check services/vcs-client/gitlab/`  | prettier (infra) | extra |

- **Completion additions:** file headers `// @tasks: VC-gitlab` на новые файлы; обновить `@tasks` в изменяемых файлах
...
- Scenario getChanges возвращает список изменённых файлов → `services/vcs-client/__tests__/gitlab/vcs-gitlab-merge-requests.test.ts` :: `returns modified files list`
- Scenario getChanges для нового файла → `services/vcs-client/__tests__/gitlab/vcs-gitlab-merge-requests.test.ts` :: `returns added file`
- Scenario getChanges для удалённого файла → `services/vcs-client/__tests__/gitlab/vcs-gitlab-merge-requests.test.ts` :: `returns deleted file`
- Scenario getChanges для переименованного файла → `services/vcs-client/__tests__/gitlab/vcs-gitlab-merge-requests.test.ts` :: `returns renamed file`
- Scenario getChanges пагинация → `services/vcs-client/__tests__/gitlab/vcs-gitlab-merge-requests.test.ts` :: `passes pagination params`
- Scenario getFileContent возвращает содержимое файла → `services/vcs-client/__tests__/gitlab/vcs-gitlab-repository-files.test.ts` :: `returns file content for text file`
- Scenario getFileContent для несуществующего файла → `services/vcs-client/__tests__/gitlab/vcs-gitlab-repository-files.test.ts` :: `returns null for 404`
- Scenario getFileContent для бинарного файла → `services/vcs-client/__tests__/gitlab/vcs-gitlab-repository-files.test.ts` :: `handles binary file (base64 encoding)`
- Scenario getFileContent ошибка сети → `services/vcs-client/__tests__/gitlab/vcs-gitlab-repository-files.test.ts` :: `throws on network error`
- Scenario getChanges ошибка сети → `services/vcs-client/__tests__/gitlab/vcs-gitlab-merge-requests.test.ts` :: `throws on 500 error`
