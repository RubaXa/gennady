# Task: VC-headers — vcs-client: file headers + DBC contracts
...
**Scenario:** Ошибка: `@file` отсутствует [`unit`]

- **Given** source file без leading `@file`
- **When** header checker проверяет файл
- **Then** он возвращает `ERR_CLI_LINT_MISSING_FILE`
...
- `@file` отсутствует → `file-header.check.test.ts` :: `should report ERR_CLI_LINT_MISSING_FILE when @file: is absent`
