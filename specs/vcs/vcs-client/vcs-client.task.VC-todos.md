# Task: VC-todos — todoIds + markTodoDone (Inbox порт)

<!--SECTION:META-->

## 1. Meta

- **Task-ID:** VC-todos | **Status:** [ ] TODO | **Scope:** vcs | **Module:** vcs-client | **Dependencies:** None
...
**Scenario:** Отсутствующие todo IDs для connection-only source [`unit`]

- **Given** actionable item получен из `reviewRequested` или `authored`
- **When** adapter маппит inbox
- **Then** `todoIds` остаётся пустым
...
- [ ] `2026-06-26T18:04:02Z` ver npx tsx cli/gennady.ts lint <target-files> → pass exit=0
