...
| FR-02                 | `VcsClientMergeRequests` — порт: `getList`, `getByIid`, `getChanges`, `approve`, `create`, `update`                                                                                                                                                                 |
...
| FR-11                 | `VcsGithubClient` — adapter: GitHub REST API (расширен: `MergeRequests.getChanges`, `getList`, `getByIid`, `create`, `update` + `MergeDiscussions.getList`, `getAll`, `addNote`, `createDiscussion` + `Reactions.add`, `remove` + `RepositoryFiles.getFileContent`) |
