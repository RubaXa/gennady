...
- **N4** — env-гигиена и транспортабельность: ядро управляет окружением подпроцесса — снимает прокси-переменные (иначе прокси режет провайдера) **и `OPENCODE_SERVER_PASSWORD`/`OPENCODE_SERVER_USERNAME`** (их выставляет desktop-App, протекают в shell и ломают `opencode run` → «Session not found»), а также изолирует сессию (`OPENCODE_DB=:memory:`). Запуск не зависит от окружения/клиента/других запусков.
- **N5** — нулевые новые зависимости: запуск через встроенный `node:child_process`.
- **N6** — скорость как главное правило: горячий путь `run()` без pre-flight `detect()` (оптимистичный запуск, отсутствие движка ловится по spawn-ошибке); `detect()`/`listEngines()` кэшируются и вне горячего пути; обязательный `timeout` (дефолт 1800000 мс = 30 мин — реальная работа агента длинная; таймаут — страховка от зависания, не лимит работы).
...
- **Why:** скорость — главное правило, инструмент зовут агенты в цикле; пред-проверка `--version` на каждый запуск = лишний подпроцесс. Запускаем сразу, отсутствие движка ловим по spawn-ошибке (подтверждено спайком: `error.code` ENOENT/EACCES); `detect()` остаётся для явного `listEngines()` и кэшируется. `timeout` (1800000 мс = 30 мин) против зависания.
