...
- **Таймаут:** при превышении `timeout` (дефолт 120000 мс) запуск прерывается, кидается `AgentRunError` с кодом `TIMEOUT`; подпроцесс не остаётся сиротой.
- Invariants: `mode` в v1 всегда `readonly` (другое значение запрещено).
