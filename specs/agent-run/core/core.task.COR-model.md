# Task: COR-model — Model selection (agent-run)
...
- **Dependencies:** COR-engine, OPE-adapter
- **Spec References:**
  - Core: [`run`/`listModels`/`ErrorCode`](./core.spec.md#4-entity-surfaces)
  - Port: [`AgentEngine.listModels`](./core.spec.md#agentengine)
  - Opencode: [`OpencodeEngine`/`opencodeErrorMap`](../opencode/opencode.spec.md#4-entity-surfaces)
  - Scope: [agent-run §3.1 F7, §4](../agent-run.spec.md#4-public-api-surface), [D-009](../agent-run.spec.md#6-decision-log)
...
- **Inputs:** none (модифицирует существующие модули COR-engine/63)
...
- Scenario "MODEL_UNAVAILABLE hint со списком" → `services/agent-run/engines/opencode/__tests__/opencode-engine.test.ts` :: `pre-validates explicit model and rejects MODEL_UNAVAILABLE with the list`
<!--/SECTION:TEST_COVERAGE-->
