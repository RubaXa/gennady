# Task: COR-engine — Implement core module
...
- **Dependencies:** None
- **Spec References:**
  - Module spec: [core](./core.spec.md)
  - Port: [`AgentEngine`](./core.spec.md#agentengine)
  - Service: [`run` / `registry`](./core.spec.md#5-module-contracts-dbc)
  - Entity: [`AgentRunError`](./core.spec.md#entity-agentrunerror)
  - Scope: [agent-run §4 Public API Surface](../agent-run.spec.md#4-public-api-surface)
...
- **And** само прерывание/убийство подпроцесса — забота движка (здесь не проверяется; integration в OPE-adapter)
...
- AgentEngine объявляет исполнимый контракт → `public-contract.test.ts` :: `AgentEngine declares executable detect run and listModels operations`
- RunOptions/RunResult/EngineStatus сохраняют public shape → `public-contract.test.ts` :: `RunOptions RunResult and EngineStatus retain their public shape`
- AgentRunError сохраняет ErrorCode и hint → `public-contract.test.ts` :: `AgentRunError carries a stable machine code and operator hint`
...
- Scenario "run() прокидывает timeout" → `services/agent-run/core/__tests__/run.test.ts` :: `defaults timeout to 1800000 (30 min) and passes it to engine`
