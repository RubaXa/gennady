...
#### Specificity:
1.  **Codex Implementation**: Для Codex `sessionDir` используется как значение `HOME`. Восстановление сессии происходит через `codex resume --last` *в контексте этой директории*.
...
2.  **MCP Injection (Workspace Config)**:
...
1.  **Parallel Discovery Strategy** и **Parallel Execution Policy**:
    ...
    *   **Пример логики**:
        ...
        const results = await Promise.allSettled(adapters.map(a => a.detect().then(info => ({ adapter: a, info }))));
        return results
