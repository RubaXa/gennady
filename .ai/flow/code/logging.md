...
**Rule:** If the state is e.g. `-> failed`, do not write "Action failed" (duplicates "failed"). Write the _reason_ or _type_: e.g. "Exit code 1", "Spawn error", "Not MODULE_NOT_FOUND", "Second import". Same for other states: the third part answers "what exactly" or "why", not repeat the transition.
