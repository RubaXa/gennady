...
Стек-агностичные верификационные гейты: **одна команда для любого стека**. Плагины: `node` (гейты из npm-скриптов `package.json`) и `golang` (`go generate` — sandboxed drift-контроль кодогенерации до сборки, `go build`, `go vet`, `gofmt -l`, `golangci-lint`, `go test`). Стек определяется автоматически; в одном репозитории могут быть активны оба.
...
stack:
  ...
  golang:
    ...
    extraGates:
      ...
      # через `verify --only=golang:canonical-generate`; sandbox: true = drift-гейт.
      - { id: canonical-generate, argv: [make, generate], sandbox: true, timeout: 5m }
```
