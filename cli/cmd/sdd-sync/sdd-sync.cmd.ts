 ...
 * @param root Project root the tracker paths are anchored to.
 * @returns One report line per index whose rollup table changed.
 */
function recomputeProgress(indexes: string[], root: string): string[] {
  const report: string[] = [];
 ...
 * @param [root] Explicit project root — ticket resolution and report paths are anchored here (defaults to CWD).
 * @returns SyncOutcome — a per-index report on success, else an actionable failure.
 */
export async function run(rawArgs: string[], root: string = resolve('.')): Promise<SyncOutcome> {
  ...
  report.push(...recomputeProgress(indexes, root));
