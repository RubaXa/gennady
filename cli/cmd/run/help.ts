...
export function printHelp(): void {
  console.info(
    'gennady run — Run a task via an external AI agent engine (opencode) in readonly mode'
  );
  console.info('');
  console.info('Usage:');
  ...
  console.info(
    '                      (default: the engine default; unavailable → MODEL_UNAVAILABLE + list)'
  );
  console.info('  --engine <id>       Engine to use (default: opencode, the first installed)');
  console.info(
    '  --timeout <ms>      Hard time limit for one run in ms (default: 1800000 = 30 min)'
  );
  console.info('  --help, -h          Show this help');
  console.info('');
  console.info('Behavior:');
  console.info(
    '  readonly — the agent may read, search and run shell, but must not edit files (v1).'
  );
  ...
  console.info(
    '               MODEL_FORBIDDEN, MODEL_UNAVAILABLE, CREDENTIAL_MISSING, TIMEOUT, LAUNCH_FAILED.'
  );
  console.info('');
