...
export function printHelp(): void {
  ...
  console.info('  - Per-phase read-manifest: rules · specs · ticket sections · target files · gates + DO-NOT-READ');
  console.info('  - Gates: every Verification command with its Required-by');
  console.info('');
  console.info('It never emits phase bodies, BDD, specs, or code — the phase workers read those, bounded by the manifest.');
  console.info('');
  console.info('Exit codes:');
  console.info('  0 surface emitted   1 file not found   2 not a ticket (no Meta)   4 bad invocation');
}
