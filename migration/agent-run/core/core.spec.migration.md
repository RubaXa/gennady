# План миграции: specs/agent-run/core/core.spec.md

- **Status:** APPROVED <!-- PLANNED → MAPPED → APPROVED → DONE -->
- **Scope:** agent-run | **Module:** core
...
- spec: `specs/agent-run/core/core.spec.md` · строк: 337 · scope-type: — · mermaid-блоков: 1 · якорей: 10
...
- тикеты:
  - `tasks/agent-run/agent-run.task-64.md` · `COR-model` · [x] DONE · Добавить выбор модели в agent-run: `RunOptions.model`, публичный `listModels`, код `MODEL_UNAVAILABLE`; opencode — дефолт `llm-proxy/deepseek-v4-pro`, `--model`, `listModels` через `opencode models`, ошибка-со-списком без молчаливой подмены.
  - `tasks/agent-run/core/core.task-62.md` · `COR-engine` · [x] DONE · Реализовать движок-независимое ядро `agent-run`: контракт `AgentEngine` (Port), типы (`RunOptions`/`RunResult`/`EngineStatus`), ошибки (`AgentRunError`/`ErrorCode`), реестр движков и публичные `run`/`listEngines` с оптимистичным запуском и таймаутом.
  <!--/SECTION:INVENTORY-->
...
<!-- Новый ID: `<ACR>-<slug>` — kebab-case, слаг из Meta.Purpose, слаг ≤8 символов (дефисы включительно), уникален в рамках всего репо.
     Назначение вычисляется из ID: `specs/agent-run/core/core.task.<новый-ID>.md`. -->

| Файл                                   | Task-ID | Новый ID   | Назначение                                     |
| -------------------------------------- | ------- | ---------- | ---------------------------------------------- |
| `tasks/agent-run/agent-run.task-64.md` | TSK-64  | COR-model  | `specs/agent-run/core/core.task.COR-model.md`  |
| `tasks/agent-run/core/core.task-62.md` | TSK-62  | COR-engine | `specs/agent-run/core/core.task.COR-engine.md` |
