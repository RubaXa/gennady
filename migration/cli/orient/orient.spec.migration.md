# План миграции: specs/cli/orient/orient.spec.md

- **Status:** APPROVED <!-- PLANNED → MAPPED → APPROVED → DONE -->
- **Scope:** cli | **Module:** orient
...
- spec: `specs/cli/orient/orient.spec.md` · строк: 312 · scope-type: — · mermaid-блоков: 0 · якорей: 0
...
- тикеты:
  - `tasks/cli/orient/orient.task-55.md` · `ORI-command` · [ ] TODO · Реализовать команду `gennady orient` целиком: инвертированный индекс + extraction хедеров + 9 сценариев поиска (S1-S9) + рендеринг + hints + CLI-обвязка + регистрация; покрыть unit + integration тестами.
  <!--/SECTION:INVENTORY-->
...
<!-- Новый ID: `<ACR>-<slug>` — kebab-case, слаг из Meta.Purpose, слаг ≤8 символов (дефисы включительно), уникален в рамках всего репо.
     Назначение вычисляется из ID: `specs/cli/orient/orient.task.<новый-ID>.md`. -->

| Файл                                 | Task-ID | Новый ID    | Назначение                                    |
| ------------------------------------ | ------- | ----------- | --------------------------------------------- |
| `tasks/cli/orient/orient.task-55.md` | TSK-55  | ORI-command | `specs/cli/orient/orient.task.ORI-command.md` |
