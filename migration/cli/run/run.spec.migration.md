# План миграции: specs/cli/run/run.spec.md

- **Status:** APPROVED <!-- PLANNED → MAPPED → APPROVED → DONE -->
- **Scope:** cli | **Module:** run
...
- spec: `specs/cli/run/run.spec.md` · строк: 182 · scope-type: — · mermaid-блоков: 0 · якорей: 0
...
- тикеты:
  - `tasks/cli/run/run.task-65.md` · `RUN-command` · [x] DONE · Команда `gennady run` — тонкая обёртка над `@services/agent-run`: парсит флаги (задание, `--dir`×N, `--model`, `--engine`, `--timeout`), зовёт `run()`, печатает текст в stdout или `✗ <message> [<code>]` + hint в stderr.
  <!--/SECTION:INVENTORY-->
...
<!-- Новый ID: `<ACR>-<slug>` — kebab-case, слаг из Meta.Purpose, слаг ≤8 символов (дефисы включительно), уникален в рамках всего репо.
     Назначение вычисляется из ID: `specs/cli/run/run.task.<новый-ID>.md`. -->

| Файл                           | Task-ID | Новый ID    | Назначение                              |
| ------------------------------ | ------- | ----------- | --------------------------------------- |
| `tasks/cli/run/run.task-65.md` | TSK-65  | RUN-command | `specs/cli/run/run.task.RUN-command.md` |
