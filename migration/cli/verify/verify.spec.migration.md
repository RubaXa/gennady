# План миграции: specs/cli/verify/verify.spec.md

- **Status:** APPROVED <!-- PLANNED → MAPPED → APPROVED → DONE -->
- **Scope:** cli | **Module:** verify
...
- spec: `specs/cli/verify/verify.spec.md` · строк: 384 · scope-type: — · mermaid-блоков: 2 · якорей: 11
...
| `## 11. Handoff to Tasks`               | keep     | HANDOFF                   | предзаполнено правилом |
...
<!-- Новый ID: `<ACR>-<slug>` — kebab-case, слаг из Meta.Purpose, слаг ≤8 символов (дефисы включительно), уникален в рамках всего репо.
     Назначение вычисляется из ID: `specs/cli/verify/verify.task.<новый-ID>.md`. -->
