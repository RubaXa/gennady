# План миграции: specs/cli/sdd-state/sdd-state.spec.md

- **Status:** APPROVED <!-- PLANNED → MAPPED → APPROVED → DONE -->
- **Scope:** cli | **Module:** sdd-state
...
- spec: `specs/cli/sdd-state/sdd-state.spec.md` · строк: 270 · scope-type: — · mermaid-блоков: 0 · якорей: 8
...
<!-- Новый ID: `<ACR>-<slug>` — kebab-case, слаг из Meta.Purpose, слаг ≤8 символов (дефисы включительно), уникален в рамках всего репо.
     Назначение вычисляется из ID: `specs/cli/sdd-state/sdd-state.task.<новый-ID>.md`. -->
