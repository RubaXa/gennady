# План миграции: specs/cli/e2e/e2e.spec.md

- **Status:** APPROVED <!-- PLANNED → MAPPED → APPROVED → DONE -->
- **Scope:** cli | **Module:** e2e
...
- spec: `specs/cli/e2e/e2e.spec.md` · строк: 427 · scope-type: `product` · mermaid-блоков: 1 · якорей: 10
...
- тикеты:
  - `tasks/cli/e2e/e2e.task-60.md` · `E2E-harness` · [x] DONE · Создать инфраструктуру E2E-тестирования CLI-команд: setup (npm pack → install в temp fixture), fixture-проект с 7 .ts файлами, тесты для lint/orient/sync/sync-skills (31 сценарий), npm-скрипт test:e2e
  <!--/SECTION:INVENTORY-->
...
<!-- Новый ID: `<ACR>-<slug>` — kebab-case, слаг из Meta.Purpose, слаг ≤8 символов (дефисы включительно), уникален в рамках всего репо.
     Назначение вычисляется из ID: `specs/cli/e2e/e2e.task.<новый-ID>.md`. -->

| Файл                           | Task-ID | Новый ID    | Назначение                              |
| ------------------------------ | ------- | ----------- | --------------------------------------- |
| `tasks/cli/e2e/e2e.task-60.md` | TSK-60  | E2E-harness | `specs/cli/e2e/e2e.task.E2E-harness.md` |
