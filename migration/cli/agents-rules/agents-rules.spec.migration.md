# План миграции: specs/cli/agents-rules/agents-rules.spec.md

- **Status:** APPROVED <!-- PLANNED → MAPPED → APPROVED → DONE -->
- **Scope:** cli | **Module:** agents-rules
...
- spec: `specs/cli/agents-rules/agents-rules.spec.md` · строк: 173 · scope-type: — · mermaid-блоков: 1 · якорей: 0
...
- заголовки (level 2):
  ...
  - `## Critic Rounds`
- тикеты:
  - `tasks/cli/agents-rules/agents-rules.task-59.md` · `AR-command` · [x] DONE · Реализовать команду `gennady agents-rules`: проверка `node_modules/gennady` → чтение `cli/cmd/orient/README.md` → stdout
  <!--/SECTION:INVENTORY-->
...
<!-- Новый ID: `<ACR>-<slug>` — kebab-case, слаг из Meta.Purpose, слаг ≤8 символов (дефисы включительно), уникален в рамках всего репо.
     Назначение вычисляется из ID: `specs/cli/agents-rules/agents-rules.task.<новый-ID>.md`. -->

| Файл                                             | Task-ID | Новый ID   | Назначение                                               |
| ------------------------------------------------ | ------- | ---------- | -------------------------------------------------------- |
| `tasks/cli/agents-rules/agents-rules.task-59.md` | TSK-59  | AR-command | `specs/cli/agents-rules/agents-rules.task.AR-command.md` |
