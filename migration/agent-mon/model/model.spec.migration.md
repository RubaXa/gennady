# План миграции: specs/agent-mon/model/model.spec.md

- **Status:** APPROVED <!-- PLANNED → MAPPED → APPROVED → DONE -->
- **Scope:** agent-mon | **Module:** model
...
- spec: `specs/agent-mon/model/model.spec.md` · строк: 213 · scope-type: — · mermaid-блоков: 1 · якорей: 0
...
- тикеты:
  - `tasks/agent-mon/model/model.task-35.md` · `MOD-types` · [x] DONE · Создать все типы, контракты и ошибки библиотеки — AgentSession, SessionChanges, ScanOpts, ObserveOpts, AgentProvider, DuplicateProviderError, ProviderNotFoundError
  <!--/SECTION:INVENTORY-->
...
<!-- Новый ID: `<ACR>-<slug>` — kebab-case, слаг из Meta.Purpose, слаг ≤8 символов (дефисы включительно), уникален в рамках всего репо.
     Назначение вычисляется из ID: `specs/agent-mon/model/model.task.<новый-ID>.md`. -->

| Файл                                     | Task-ID | Новый ID  | Назначение                                      |
| ---------------------------------------- | ------- | --------- | ----------------------------------------------- |
| `tasks/agent-mon/model/model.task-35.md` | TSK-35  | MOD-types | `specs/agent-mon/model/model.task.MOD-types.md` |
