# План миграции: specs/agent-mon/diff/diff.spec.md

- **Status:** APPROVED <!-- PLANNED → MAPPED → APPROVED → DONE -->
- **Scope:** agent-mon | **Module:** diff
...
- spec: `specs/agent-mon/diff/diff.spec.md` · строк: 102 · scope-type: — · mermaid-блоков: 1 · якорей: 0
...
- тикеты:
  - `tasks/agent-mon/diff/diff.task-37.md` · `DIF-compare` · [x] DONE · Реализовать чистую функцию diff для сравнения двух AgentSession[] по семантическим полям
  <!--/SECTION:INVENTORY-->
...
<!-- Новый ID: `<ACR>-<slug>` — kebab-case, слаг из Meta.Purpose, слаг ≤8 символов (дефисы включительно), уникален в рамках всего репо.
     Назначение вычисляется из ID: `specs/agent-mon/diff/diff.task.<новый-ID>.md`. -->

| Файл                                   | Task-ID | Новый ID    | Назначение                                      |
| -------------------------------------- | ------- | ----------- | ----------------------------------------------- |
| `tasks/agent-mon/diff/diff.task-37.md` | TSK-37  | DIF-compare | `specs/agent-mon/diff/diff.task.DIF-compare.md` |
