# План миграции: specs/agent-mon/monitor/monitor.spec.md

- **Status:** APPROVED <!-- PLANNED → MAPPED → APPROVED → DONE -->
- **Scope:** agent-mon | **Module:** monitor
...
- spec: `specs/agent-mon/monitor/monitor.spec.md` · строк: 125 · scope-type: — · mermaid-блоков: 1 · якорей: 0
...
- тикеты:
  - `tasks/agent-mon/monitor/monitor.task-36.md` · `MON-service` · [x] DONE · Реализовать AgentMonitor (Service) — register, unregister, scanAll, scanOne — и createMonitor (Factory)
  <!--/SECTION:INVENTORY-->
...
<!-- Новый ID: `<ACR>-<slug>` — kebab-case, слаг из Meta.Purpose, слаг ≤8 символов (дефисы включительно), уникален в рамках всего репо.
     Назначение вычисляется из ID: `specs/agent-mon/monitor/monitor.task.<новый-ID>.md`. -->

| Файл                                         | Task-ID | Новый ID    | Назначение                                            |
| -------------------------------------------- | ------- | ----------- | ----------------------------------------------------- |
| `tasks/agent-mon/monitor/monitor.task-36.md` | TSK-36  | MON-service | `specs/agent-mon/monitor/monitor.task.MON-service.md` |
