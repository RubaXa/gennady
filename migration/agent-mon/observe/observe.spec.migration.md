# План миграции: specs/agent-mon/observe/observe.spec.md

- **Status:** APPROVED <!-- PLANNED → MAPPED → APPROVED → DONE -->
- **Scope:** agent-mon | **Module:** observe
...
- spec: `specs/agent-mon/observe/observe.spec.md` · строк: 105 · scope-type: — · mermaid-блоков: 1 · якорей: 0
...
- тикеты:
  - `tasks/agent-mon/observe/observe.task-38.md` · `OBS-stream` · [x] DONE · Реализовать свободную функцию observe(monitor, opts) → AsyncIterable<SessionChanges> с поддержкой idleThresholdMs
  <!--/SECTION:INVENTORY-->
...
<!-- Новый ID: `<ACR>-<slug>` — kebab-case, слаг из Meta.Purpose, слаг ≤8 символов (дефисы включительно), уникален в рамках всего репо.
     Назначение вычисляется из ID: `specs/agent-mon/observe/observe.task.<новый-ID>.md`. -->

| Файл                                         | Task-ID | Новый ID   | Назначение                                           |
| -------------------------------------------- | ------- | ---------- | ---------------------------------------------------- |
| `tasks/agent-mon/observe/observe.task-38.md` | TSK-38  | OBS-stream | `specs/agent-mon/observe/observe.task.OBS-stream.md` |
