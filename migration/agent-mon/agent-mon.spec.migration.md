# План миграции: specs/agent-mon/agent-mon.spec.md
...
- spec: `specs/agent-mon/agent-mon.spec.md` · строк: 381 · scope-type: — · mermaid-блоков: 1 · якорей: 0
...
- тикеты:
  - `tasks/agent-mon/agent-mon.task-41.md` · `AM-exports` · [x] DONE · Создать корневой index.ts и добавить subpath-exports в package.json для импортов из Golden DX
  - `tasks/agent-mon/providers/claude/claude.task-39.md` · `AM-claude` · [x] DONE · Реализовать ClaudeProvider — сканирование сессий Claude Code через батчевый ps и DI-точки
  - `tasks/agent-mon/providers/opencode/opencode.task-40.md` · `AM-opencode` · [x] DONE · Реализовать OpenCodeProvider — сканирование сессий OpenCode через SQLite с DI-точкой для БД
  <!--/SECTION:INVENTORY-->
...
<!-- Новый ID: `<ACR>-<slug>` — kebab-case, слаг из Meta.Purpose, слаг ≤8 символов (дефисы включительно), уникален в рамках всего репо.
     Назначение вычисляется из ID: `specs/agent-mon/agent-mon.task.<новый-ID>.md`. -->

| Файл                                                     | Task-ID | Новый ID    | Назначение                                      |
| -------------------------------------------------------- | ------- | ----------- | ----------------------------------------------- |
| `tasks/agent-mon/agent-mon.task-41.md`                   | TSK-41  | AM-exports  | `specs/agent-mon/agent-mon.task.AM-exports.md`  |
| `tasks/agent-mon/providers/claude/claude.task-39.md`     | TSK-39  | AM-claude   | `specs/agent-mon/agent-mon.task.AM-claude.md`   |
| `tasks/agent-mon/providers/opencode/opencode.task-40.md` | TSK-40  | AM-opencode | `specs/agent-mon/agent-mon.task.AM-opencode.md` |
...
- [ ] S6 ✅ `npx tsx cli/gennady.ts sdd-check --all specs/agent-mon` — строгий v2-гейт (структура, фолды, разбор mermaid, язык) → **Status:** DONE
