# План миграции: specs/ai-skills/directive-assembly/directive-assembly.spec.md

- **Status:** APPROVED <!-- PLANNED → MAPPED → APPROVED → DONE -->
- **Scope:** ai-skills | **Module:** directive-assembly
...
- spec: `specs/ai-skills/directive-assembly/directive-assembly.spec.md` · строк: 878 · scope-type: — · mermaid-блоков: 4 · якорей: 14
...
| `## BDD Scenarios`             | keep     | BDD                          | сценарии сохраняются как BDD evidence  |
...
| `## Public Options & Policies` | keep     | PUBLIC_OPTIONS               | предзаполнено правилом                 |
| `## File Structure`            | keep     | FILE_STRUCTURE               | предзаполнено правилом                 |
| `## Research`                  | keep     | RESEARCH                     | исследовательское evidence сохраняется |
...
<!-- Новый ID: `<ACR>-<slug>` — kebab-case, слаг из Meta.Purpose, слаг ≤8 символов (дефисы включительно), уникален в рамках всего репо.
     Назначение вычисляется из ID: `specs/ai-skills/directive-assembly/directive-assembly.task.<новый-ID>.md`. -->
