# План миграции: specs/agent-mon-cli/cmd/cmd.spec.md

- **Status:** APPROVED <!-- PLANNED → MAPPED → APPROVED → DONE -->
- **Scope:** agent-mon-cli | **Module:** cmd
...
- spec: `specs/agent-mon-cli/cmd/cmd.spec.md` · строк: 124 · scope-type: — · mermaid-блоков: 1 · якорей: 0
...
- тикеты:
  - `tasks/agent-mon-cli/cmd/cmd.task-47.md` · `CMD-entry` · [ ] TODO · Реализовать CLI entry point (run, createProviders) и добавить switch case в gennady.ts
  <!--/SECTION:INVENTORY-->
...
<!-- Новый ID: `<ACR>-<slug>` — kebab-case, слаг из Meta.Purpose, слаг ≤8 символов (дефисы включительно), уникален в рамках всего репо.
     Назначение вычисляется из ID: `specs/agent-mon-cli/cmd/cmd.task.<новый-ID>.md`. -->

| Файл                                     | Task-ID | Новый ID  | Назначение                                      |
| ---------------------------------------- | ------- | --------- | ----------------------------------------------- |
| `tasks/agent-mon-cli/cmd/cmd.task-47.md` | TSK-47  | CMD-entry | `specs/agent-mon-cli/cmd/cmd.task.CMD-entry.md` |
