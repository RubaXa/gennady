# План миграции: specs/agent-mon-cli/state/state.spec.md

- **Status:** APPROVED <!-- PLANNED → MAPPED → APPROVED → DONE -->
- **Scope:** agent-mon-cli | **Module:** state
...
- spec: `specs/agent-mon-cli/state/state.spec.md` · строк: 189 · scope-type: — · mermaid-блоков: 1 · якорей: 0
...
- тикеты:
  - `tasks/agent-mon-cli/state/state.task-45.md` · `STA-manager` · [ ] TODO · Реализовать state manager: createStateManager, ViewModel, groupByProvider, isWaitingForUser
  <!--/SECTION:INVENTORY-->
...
<!-- Новый ID: `<ACR>-<slug>` — kebab-case, слаг из Meta.Purpose, слаг ≤8 символов (дефисы включительно), уникален в рамках всего репо.
     Назначение вычисляется из ID: `specs/agent-mon-cli/state/state.task.<новый-ID>.md`. -->

| Файл                                         | Task-ID | Новый ID    | Назначение                                            |
| -------------------------------------------- | ------- | ----------- | ----------------------------------------------------- |
| `tasks/agent-mon-cli/state/state.task-45.md` | TSK-45  | STA-manager | `specs/agent-mon-cli/state/state.task.STA-manager.md` |
