# План миграции: specs/agent-inbox/inbox-api/inbox-api.spec.md

- **Status:** APPROVED <!-- PLANNED → MAPPED → APPROVED → DONE -->
- **Scope:** agent-inbox | **Module:** inbox-api
...
- spec: `specs/agent-inbox/inbox-api/inbox-api.spec.md` · строк: 135 · scope-type: — · mermaid-блоков: 0 · якорей: 10
...
- тикеты:
  - `tasks/agent-inbox/agent-inbox.task-162.md` · `IA-rest-sse` · [x] DONE · REST (10 эндпоинтов) + SSE (фреймы+топология), DTO-проекции доски/ленты/очереди из журнала+sync (не из летучей памяти), enqueue с дедупом, decision→effect, degraded-канал.
  - `tasks/agent-inbox/agent-inbox.task-170.md` · `IA-health` · [x] DONE · Честное закрытие хвоста test-suite health (AI-fasttest R2): (а) root-cause утечки хэндлов — `gracefulShutdown` логирует `server close timed out`, процесс не выходит без `--test-force-exit`; (б) bootstrap orphan-restart RED (`fetch failed`); (в) 4 RED describe v1-пайплайна (run-mode ×3, full-flow.blackbox) — superseded by v2 (IQ-executor…161), удалить вместе с v1-модулями или починить. Skip по правилу оператора допустим ТОЛЬКО при временной недоступности инфраструктуры — текущие skip'ы держатся на этом тикете и должны уйти с его закрытием.
  - `tasks/agent-inbox/inbox-api/inbox-api.task-179.md` · `IA-proj` · [ ] TODO · Replace RoleScheduler-backed board state with journal projections, typed commands and reconciled SSE.
  <!--/SECTION:INVENTORY-->
...
<!-- Новый ID: `<ACR>-<slug>` — kebab-case, слаг из Meta.Purpose, слаг ≤8 символов (дефисы включительно), уникален в рамках всего репо.
     Назначение вычисляется из ID: `specs/agent-inbox/inbox-api/inbox-api.task.<новый-ID>.md`. -->

| Файл                                                | Task-ID | Новый ID    | Назначение                                                  |
| --------------------------------------------------- | ------- | ----------- | ----------------------------------------------------------- |
| `tasks/agent-inbox/agent-inbox.task-162.md`         | TSK-162 | IA-rest-sse | `specs/agent-inbox/inbox-api/inbox-api.task.IA-rest-sse.md` |
| `tasks/agent-inbox/agent-inbox.task-170.md`         | TSK-170 | IA-health   | `specs/agent-inbox/inbox-api/inbox-api.task.IA-health.md`   |
| `tasks/agent-inbox/inbox-api/inbox-api.task-179.md` | TSK-179 | IA-proj     | `specs/agent-inbox/inbox-api/inbox-api.task.IA-proj.md`     |
