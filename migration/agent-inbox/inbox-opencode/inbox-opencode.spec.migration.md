# План миграции: specs/agent-inbox/inbox-opencode/inbox-opencode.spec.md

- **Status:** APPROVED <!-- PLANNED → MAPPED → APPROVED → DONE -->
- **Scope:** agent-inbox | **Module:** inbox-opencode
...
- spec: `specs/agent-inbox/inbox-opencode/inbox-opencode.spec.md` · строк: 128 · scope-type: — · mermaid-блоков: 0 · якорей: 10
...
- тикеты:
  - `tasks/agent-inbox/agent-inbox.task-160.md` · `IO-session` · [x] DONE · Один opencode-сервер, жизненный цикл сессии create→work→park(TTL)→resume→close, session registry, единый приоритетный пул, единый маршрут промптов (Handlebars+кирпичи, указатели), X-ray + tool-trace (источник coverage-гейта), outcome-лесенка.
  - `tasks/agent-inbox/inbox-opencode/inbox-opencode.task-175.md` · `IO-runtime` · [x] DONE · Generalize the existing OpenCode boundary into `AgentRuntimePort` without replacing working session infrastructure.
  <!--/SECTION:INVENTORY-->
...
<!-- Новый ID: `<ACR>-<slug>` — kebab-case, слаг из Meta.Purpose, слаг ≤8 символов (дефисы включительно), уникален в рамках всего репо.
     Назначение вычисляется из ID: `specs/agent-inbox/inbox-opencode/inbox-opencode.task.<новый-ID>.md`. -->

| Файл                                                          | Task-ID | Новый ID   | Назначение                                                           |
| ------------------------------------------------------------- | ------- | ---------- | -------------------------------------------------------------------- |
| `tasks/agent-inbox/agent-inbox.task-160.md`                   | TSK-160 | IO-session | `specs/agent-inbox/inbox-opencode/inbox-opencode.task.IO-session.md` |
| `tasks/agent-inbox/inbox-opencode/inbox-opencode.task-175.md` | TSK-175 | IO-runtime | `specs/agent-inbox/inbox-opencode/inbox-opencode.task.IO-runtime.md` |
