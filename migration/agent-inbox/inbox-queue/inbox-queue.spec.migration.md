# План миграции: specs/agent-inbox/inbox-queue/inbox-queue.spec.md

- **Status:** APPROVED <!-- PLANNED → MAPPED → APPROVED → DONE -->
- **Scope:** agent-inbox | **Module:** inbox-queue
...
- spec: `specs/agent-inbox/inbox-queue/inbox-queue.spec.md` · строк: 543 · scope-type: — · mermaid-блоков: 1 · якорей: 10
...
- тикеты:
  - `tasks/agent-inbox/agent-inbox.task-159.md` · `IQ-executor` · [x] DONE · Исполнительное ядро: реестр 16 типов с формальной грамматикой ссылок, per-MR executors (ноль глобальных мьютексов), дедуп/supersede, приоритеты+aging, восстановление из журнала, маршрутизация сессий, видимое «⏳ ждёт очередь».
  - `tasks/agent-inbox/inbox-queue/inbox-queue.task-177.md` · `IQ-actions` · [x] DONE · Extend per-MR execution with one action catalog, hybrid packages, independent outcomes and safe automation.
  <!--/SECTION:INVENTORY-->
...
<!-- Новый ID: `<ACR>-<slug>` — kebab-case, слаг из Meta.Purpose, слаг ≤8 символов (дефисы включительно), уникален в рамках всего репо.
     Назначение вычисляется из ID: `specs/agent-inbox/inbox-queue/inbox-queue.task.<новый-ID>.md`. -->

| Файл                                                    | Task-ID | Новый ID    | Назначение                                                      |
| ------------------------------------------------------- | ------- | ----------- | --------------------------------------------------------------- |
| `tasks/agent-inbox/agent-inbox.task-159.md`             | TSK-159 | IQ-executor | `specs/agent-inbox/inbox-queue/inbox-queue.task.IQ-executor.md` |
| `tasks/agent-inbox/inbox-queue/inbox-queue.task-177.md` | TSK-177 | IQ-actions  | `specs/agent-inbox/inbox-queue/inbox-queue.task.IQ-actions.md`  |
