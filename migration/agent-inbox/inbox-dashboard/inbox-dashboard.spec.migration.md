# План миграции: specs/agent-inbox/inbox-dashboard/inbox-dashboard.spec.md

- **Status:** APPROVED <!-- PLANNED → MAPPED → APPROVED → DONE -->
- **Scope:** agent-inbox | **Module:** inbox-dashboard
...
- spec: `specs/agent-inbox/inbox-dashboard/inbox-dashboard.spec.md` · строк: 153 · scope-type: — · mermaid-блоков: 0 · якорей: 10
...
- тикеты:
  - `tasks/agent-inbox/agent-inbox.task-164.md` · `ID-spa` · [x] DONE · SPA трёх экранов: загрузка с фазами, доска внимания (5 групп + degraded), карточка A, лента виджетов (7 типов, жизненный цикл), S8-поверхность (decision/undo/auto-бейдж), чат-колонка с якорями, оптимизм+скелетоны+SSE.
  - `tasks/agent-inbox/agent-inbox.task-169.md` · `ID-design` · [x] DONE · ID-spa собрал работающий v2-дашборд, но НЕ по утверждённой дизайн-системе: палитра GitHub-dark (`#0d1117/#161b22/#30363d`, синий `#58a6ff`) вместо Carbon & Steel (`#121416`, primary `#fc6d26`, бордеры `#2e3440`); доска — вертикальные секции вместо kanban-колонок с акцент-барами и rail'ом; нет severity-строк находок, sticky decision-бара, step-flow плана; шрифты Geist/JetBrains Mono не забандлены. Привести UI к спеке ДО финальной приёмки (иначе приёмочные скриншоты зафиксируют неправильный UI).
  - `tasks/agent-inbox/inbox-dashboard/inbox-dashboard.task-182.md` · `ID-cockpit` · [ ] TODO · Build the usable two-queue closed-loop cockpit on one component tree and real local API.
  <!--/SECTION:INVENTORY-->
...
<!-- Новый ID: `<ACR>-<slug>` — kebab-case, слаг из Meta.Purpose, слаг ≤8 символов (дефисы включительно), уникален в рамках всего репо.
     Назначение вычисляется из ID: `specs/agent-inbox/inbox-dashboard/inbox-dashboard.task.<новый-ID>.md`. -->

| Файл                                                            | Task-ID | Новый ID   | Назначение                                                             |
| --------------------------------------------------------------- | ------- | ---------- | ---------------------------------------------------------------------- |
| `tasks/agent-inbox/agent-inbox.task-164.md`                     | TSK-164 | ID-spa     | `specs/agent-inbox/inbox-dashboard/inbox-dashboard.task.ID-spa.md`     |
| `tasks/agent-inbox/agent-inbox.task-169.md`                     | TSK-169 | ID-design  | `specs/agent-inbox/inbox-dashboard/inbox-dashboard.task.ID-design.md`  |
| `tasks/agent-inbox/inbox-dashboard/inbox-dashboard.task-182.md` | TSK-182 | ID-cockpit | `specs/agent-inbox/inbox-dashboard/inbox-dashboard.task.ID-cockpit.md` |
