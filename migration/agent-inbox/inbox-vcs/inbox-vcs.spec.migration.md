# План миграции: specs/agent-inbox/inbox-vcs/inbox-vcs.spec.md

- **Status:** APPROVED <!-- PLANNED → MAPPED → APPROVED → DONE -->
- **Scope:** agent-inbox | **Module:** inbox-vcs
...
- spec: `specs/agent-inbox/inbox-vcs/inbox-vcs.spec.md` · строк: 637 · scope-type: — · mermaid-блоков: 0 · якорей: 10
...
- тикеты:
  - `tasks/agent-inbox/agent-inbox.task-158.md` · `IV-gitlab` · [x] DONE · Слой правды GitLab: poll/detail ярусы, роль из GitLab, детерминированная ось внимания + fallback без detail, стадия (внутренняя), фон-верификация ~1/мин, эффекты с правами и failure-матрицей.
  - `tasks/agent-inbox/inbox-vcs/inbox-vcs.task-174.md` · `IV-vcs-port` · [x] DONE · Consolidate GitLab truth into one read/effect boundary with inclusive discovery, complete event ingestion and safe reconciliation.
  <!--/SECTION:INVENTORY-->
...
<!-- Новый ID: `<ACR>-<slug>` — kebab-case, слаг из Meta.Purpose, слаг ≤8 символов (дефисы включительно), уникален в рамках всего репо.
     Назначение вычисляется из ID: `specs/agent-inbox/inbox-vcs/inbox-vcs.task.<новый-ID>.md`. -->

| Файл                                                | Task-ID | Новый ID    | Назначение                                                  |
| --------------------------------------------------- | ------- | ----------- | ----------------------------------------------------------- |
| `tasks/agent-inbox/agent-inbox.task-158.md`         | TSK-158 | IV-gitlab   | `specs/agent-inbox/inbox-vcs/inbox-vcs.task.IV-gitlab.md`   |
| `tasks/agent-inbox/inbox-vcs/inbox-vcs.task-174.md` | TSK-174 | IV-vcs-port | `specs/agent-inbox/inbox-vcs/inbox-vcs.task.IV-vcs-port.md` |
