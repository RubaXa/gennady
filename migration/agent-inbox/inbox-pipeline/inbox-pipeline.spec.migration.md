# План миграции: specs/agent-inbox/inbox-pipeline/inbox-pipeline.spec.md

- **Status:** APPROVED <!-- PLANNED → MAPPED → APPROVED → DONE -->
- **Scope:** agent-inbox | **Module:** inbox-pipeline
...
- spec: `specs/agent-inbox/inbox-pipeline/inbox-pipeline.spec.md` · строк: 897 · scope-type: — · mermaid-блоков: 1 · якорей: 10
...
- тикеты:
  - `tasks/agent-inbox/agent-inbox.task-161.md` · `IP-review` · [x] DONE · Единый ревью-пайплайн: план-DAG (prepare→plan→enrich→fan-out→gate_coverage→synthesize→gate_verdict→хвост), 3 слоя дорожек, линзы-волны, мульти-модель (N артефактов + findings.jsonl), coverage-гейт по tool-trace, синтез с read-тулами, role-хвосты + delta_review мини-DAG.
  - `tasks/agent-inbox/inbox-pipeline/inbox-pipeline.task-176.md` · `IP-control` · [x] DONE · Довести `inbox-pipeline` до role-invariant review control plane: sealed inputs, total Review Contract, trusted receipts, адресный bounded repair, fresh PASS и exact queue handoff.
  <!--/SECTION:INVENTORY-->
...
<!-- Новый ID: `<ACR>-<slug>` — kebab-case, слаг из Meta.Purpose, слаг ≤8 символов (дефисы включительно), уникален в рамках всего репо.
     Назначение вычисляется из ID: `specs/agent-inbox/inbox-pipeline/inbox-pipeline.task.<новый-ID>.md`. -->

| Файл                                                          | Task-ID | Новый ID   | Назначение                                                           |
| ------------------------------------------------------------- | ------- | ---------- | -------------------------------------------------------------------- |
| `tasks/agent-inbox/agent-inbox.task-161.md`                   | TSK-161 | IP-review  | `specs/agent-inbox/inbox-pipeline/inbox-pipeline.task.IP-review.md`  |
| `tasks/agent-inbox/inbox-pipeline/inbox-pipeline.task-176.md` | TSK-176 | IP-control | `specs/agent-inbox/inbox-pipeline/inbox-pipeline.task.IP-control.md` |
