# План миграции: specs/agent-inbox/inbox-mocks/inbox-mocks.spec.md

- **Status:** APPROVED <!-- PLANNED → MAPPED → APPROVED → DONE -->
- **Scope:** agent-inbox | **Module:** inbox-mocks
...
- spec: `specs/agent-inbox/inbox-mocks/inbox-mocks.spec.md` · строк: 134 · scope-type: — · mermaid-блоков: 0 · якорей: 10
...
- тикеты:
  - `tasks/agent-inbox/inbox-mocks/inbox-mocks.task-180.md` · `IM-cases` · [ ] TODO · Expand existing factories into strict adapters/scenarios for every variable port and failure branch.
  <!--/SECTION:INVENTORY-->
...
<!-- Новый ID: `<ACR>-<slug>` — kebab-case, слаг из Meta.Purpose, слаг ≤8 символов (дефисы включительно), уникален в рамках всего репо.
     Назначение вычисляется из ID: `specs/agent-inbox/inbox-mocks/inbox-mocks.task.<новый-ID>.md`. -->

| Файл                                                    | Task-ID | Новый ID | Назначение                                                   |
| ------------------------------------------------------- | ------- | -------- | ------------------------------------------------------------ |
| `tasks/agent-inbox/inbox-mocks/inbox-mocks.task-180.md` | TSK-180 | IM-cases | `specs/agent-inbox/inbox-mocks/inbox-mocks.task.IM-cases.md` |
