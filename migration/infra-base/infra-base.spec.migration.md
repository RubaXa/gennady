# План миграции: specs/infra-base/infra-base.spec.md
...
- spec: `specs/infra-base/infra-base.spec.md` · строк: 29 · scope-type: — · mermaid-блоков: 0 · якорей: 0
...
| `## 2. Tool Stack (minimal bootstrap)` | keep     | TOOL_STACK     | предзаполнено правилом                  |
...
<!-- Новый ID: `<ACR>-<slug>` — kebab-case, слаг из Meta.Purpose, слаг ≤8 символов (дефисы включительно), уникален в рамках всего репо.
     Назначение вычисляется из ID: `specs/infra-base/infra-base.task.<новый-ID>.md`. -->
...
- [ ] S6 ✅ `npx tsx cli/gennady.ts sdd-check --all specs/infra-base` — строгий v2-гейт (структура, фолды, разбор mermaid, язык) → **Status:** DONE
