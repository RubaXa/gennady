// @file: TypeScriptCodingRules — TSX port of ai/directives/coding/typescript-rules.xml
...
import { Pattern, Snippet, Hook, AntiPattern, Good, Definition } from '../elements/index.js';

/** @purpose Canonical TypeScript coding rules for all project execution agents. */
export default function TypeScriptCodingRules() {
  return (
    <Prompt
      is="TypeScriptCodingRules"
      keywords="typescript, executable_specification, design_by_contract, jsdoc, ai_to_ai, structural_anchors, log_driven_development, yagni, ts_native, modern_ts"
      type="coding-rules"
      ver="2.0"
    >
      <Group is="Mission">
      ...
      <BeliefState>
        ...
        <Axiom id="AX_ANCHOR_FORMAT">
          {[
            ...
            '',
            '      When a block has a non-obvious invariant or failure boundary, attach payload inside the anchor (outside it is detached from the block it protects): ONE payload kind → inline on the opening line `// #region START_[NAME] — ',
          ].join('\n')}
          <Group is="key">
            {': '}
            <Group is="value">
              {[
                '`; 2+ kinds → separate comment lines immediately after the opening anchor. Greppable: `#(region|endregion)\\s+(START|END)_[A-Z0-9_]+`.',
                '    ',
              ].join('\n')}
            </Group>
            ...
            <Axiom id="AX_COMMENT_PLACEMENT">
              {[
                '',
                '      Function body starts with executable code. Hidden constraints belong in JSDoc (`@invariant`, `@sideEffect`, `@throws`, `@pre`, `@post`); block-level invariants inside `// #region START_',
              ].join('\n')}
              <Group is="NAME">
                {[
                  '` anchors. No narrative brief or rule-summary at method head — JSDoc + signature is the canonical contract surface.',
                  '    ',
                ].join('\n')}
              </Group>
              ...
              <Axiom id="AX_FILE_LEVEL_CONTEXT">
                ...
                <Group is="what">
                  {['` *(mandatory)*', '      `// @consumers: '].join('\n')}
                  <Group is="ConsumerName">
                    {['` *(optional; omit when no uniform consumer)*', '      `// @tasks: '].join(
                      '\n'
                    )}
                    <Group is="TASK-ID">
                      {[
                        '[, TASK-ID...]` *(optional; may be added later)*',
                        '',
                  ...
                  <DefinitionsBlock>
                    <Definition id="DEF_TRACE_PREFIX">
                      {
                        ...
                        <AntiPatternsBlock>
                          ...
                          <AntiPattern id="AP_FALSE_PRIVATE_AND_ENUM">
                            ...
                            <AntiPattern id="AP_BARE_CONSOLE_AND_LOST_CAUSE">
                              ...
                              <AntiPattern id="AP_REDUNDANT_JSDOC_DUPLICATION">
                                <Group is="Bad">
                                  {
                            ...
                            <VerificationHooksBlock>
                              <Hook id="HOOK_TYPECHECK">
                                ...
                                <Group is="Expected">{'Exit 0; empty stdout/stderr.'}</Group>
                              </Hook>
                              <Hook id="HOOK_PROJECT_LINT">
                                ...
                                <Group is="Command">{'npm run lint'}</Group>
                                <Group is="Expected">{'Exit 0.'}</Group>
                              </Hook>
                              ...
                              <Hook id="HOOK_ANCHOR_PAIRING">
                                <Group is="Purpose">
                                  {
                                    'Verify anchor parity and enable agent grep-navigation: every `// #region START_[NAME]` must have `// #endregion END_[NAME]` in the same file.'
                                  }
                                </Group>
                                <Group is="Command">
                                  {
