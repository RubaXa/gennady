// @file: NodeTestRules — TSX port of ai/directives/testing/node-test.xml
// @consumers: ai-tsx directives module
// @tasks: TSK-76

import { Prompt, BeliefState, Axiom, Group } from '../../prompt-kit/elements/index.js';
import { AntiPatternsBlock, VerificationHooksBlock } from '../components/index.js';
import { Pattern, Snippet, Hook, AntiPattern, Good } from '../elements/index.js';

/** @purpose node:test runner rules — assert API, mock.fn(), HTTP mock-agent, snapshots. */
export default function NodeTestRules() {
  return (
    <Prompt
      is="NodeTestRules"
      keywords="node-test, runner, assertions, native_mock_fn, http_mock_agent, snapshot_files"
      type="testing-rules"
      ver="3.0"
    >
      <Group is="Mission">
      ...
      <Group is="TestPatterns">
        <Pattern id="PT_DIRECT_SCALAR_CASE">
          <Group is="Intent">
            {
              'Trivial scalar case without an opening brief: title + direct input + a single assertion already expose the scenario. Anchors omitted on the one-line TRIGGER and ASSERT phases per `AX_PHASE_ANCHORS`; kept only on SETUP because it has its own intent.'
            }
          </Group>
          ...
          <Group is="Why">
            {
              'Title + direct input + single `strictEqual` make the scenario self-evident → brief skipped. Each phase is one statement with no hidden policy → anchors skipped. Phase grammar still readable for the next agent because the three lines naturally map SETUP/TRIGGER/ASSERT.'
            }
          </Group>
        </Pattern>
        <Pattern id="PT_ASYNC_RETRY_WITH_BRIEF">
          <Group is="Intent">
            {
              'Async scenario with retry policy: opening brief records invariant + failure mode; phase anchors separate multi-line SETUP, the TRIGGER call, and the composite ASSERT with main + orthogonal observation.'
            }
          </Group>
          <Snippet language="typescript">
            {[
            ...
            <Group is="Why">
              {
                'Brief reveals retry invariant; SETUP has multiple statements + intercept policy → anchored; TRIGGER is a single call → no anchor; ASSERT carries two related observations → anchored. Orthogonal `attemptCount` lives next to the main contract assertion without aggregation noise.'
              }
            </Group>
          </Snippet>
          <Pattern id="PT_ERROR_PATH_WITH_ASSERT_REJECTS">
            <Group is="Intent">
              {
                'Error-path scenario: focused assertions via `assert.rejects()` + `assert.match()` on a stable message fragment; no `try/catch + flag` antipatterns.'
              }
            </Group>
            <Snippet language="typescript">
              {[
            ...
            <Group is="Why">
              {
                'Combined TRIGGER+ASSERT via `assert.rejects` is intentional — forced phase split would yield an empty TRIGGER. The single `rejects` call needs no anchor; SETUP has multiple lines + interception policy → anchored.'
              }
            </Group>
          </Pattern>
          <Pattern id="PT_MOCK_FN_INTERACTION_CONTRACT">
            <Group is="Intent">
              {
                'Native `mock.fn()` for tracking calls on injected collaborators + assertions on interaction contract without inspecting SUT internals.'
              }
            </Group>
            <Snippet language="typescript">
              {[
            ...
            <Group is="Why">
              {
                'Demonstrates `AX_PREFER_NATIVE_MOCK_FN` and contract-boundary observation of an emitted event (per inherited `AX_CONTRACT_OVER_IMPLEMENTATION`). Multi-statement SETUP and multi-statement ASSERT anchored; single-line TRIGGER is not.'
              }
            </Group>
          </Pattern>
          <Pattern id="PT_FIXTURE_AND_OBSERVE_AGGREGATION">
            <Group is="Intent">
              {
                'Composite contract (generated text + observability milestone) where an OBSERVE phase normalizes result + side-effect log into one diagnostic slice. Justified case for aggregation per inherited `AX_DEFAULT_DIRECT_ASSERTION_PROTOCOL`.'
              }
            </Group>
            <Snippet language="typescript">
              {[
            ...
            <Group is="Why">
              {
                'Aggregation in OBSERVE justified: a single `deepStrictEqual` shows drift in both text and milestones. SETUP and TRIGGER are one-liners → no anchors; OBSERVE has aggregation policy → anchored. Fixture read allowed, write forbidden (inherited `AX_FIXTURE_IO_POLICY`).'
              }
            </Group>
          </Pattern>
        </Pattern>
        <AntiPatternsBlock>
          <AntiPattern id="AP_PRIVATE_INSPECTION">
            <Group is="Bad">
              {
                'Test inspects `client._inflightCount` (protected/internal field) via `@ts-expect-error` to bypass encapsulation.'
              }
            </Group>
            <Group is="WhyBad">
              {
                'Violates inherited `AX_CONTRACT_OVER_IMPLEMENTATION`. A refactor renaming `_inflightCount` to `_pending` breaks the test without changing the contract.'
              }
            </Group>
            <Good>
              {
                "Subscribe to `client.on('reservation.requested', e => events.push(e))` and assert `events.length` + `events[0].idempotencyKey`. Public lifecycle event is the observable."
              }
            </Good>
          </AntiPattern>
          <AntiPattern id="AP_TRY_CATCH_FLAG_ANTIPATTERN">
            <Group is="Bad">
              {
                "`let threw = false; try { await client.reserveStock(''); } catch (e) { threw = true; assert.ok((e as Error).message.includes('idempotency')); } assert.ok(threw);`"
              }
            </Group>
            <Group is="WhyBad">
              {
                'Manual try/catch + boolean flag instead of `assert.rejects()` (`AX_ASSERT_API_CHOICE`). `includes()` on error message (inherited `AX_PARTIAL_STRING_VIA_MATCH`). Test can silently pass if the code stops throwing — `threw` stays false, only fallback gate is `assert.ok(threw)`.'
              }
            </Group>
            <Good>
              {
                "`await assert.rejects(() => client.reserveStock(''), (error) => { assert.match((error as Error).message, /idempotency.*empty|empty.*idempotency/i); return true; });`"
              }
            </Good>
          </AntiPattern>
          <AntiPattern id="AP_SHARED_MUTABLE_STATE_AND_NON_ENGLISH_NARRATION">
            <Group is="Bad">
              {
                '`const cart = new Cart()` shared between tests; test names in Russian («добавляет товар»); `Шаг 1: добавляем` / `Шаг 2: проверяем` comments; second test depends on state from first.'
              }
            </Group>
            <Group is="WhyBad">
              {
                'Shared mutable state breaks isolation (inherited `AX_NO_DESCRIBE_LET_PILEUP`); creates order dependency. Non-English names/comments (inherited `AX_ENGLISH_ONLY_NAMES_AND_COMMENTS`). Step-by-step human narration (inherited `AX_TESTS_NO_HUMAN_NARRATION`).'
              }
            </Group>
            <Good>
              {
                'Single context factory `createCartContext(overrides?)` called inside each case; English titles like `should expose count = 1 after a single add`; direct assertions.'
              }
            </Good>
          </AntiPattern>
          <AntiPattern id="AP_SNAPSHOT_FOR_SCALAR">
            <Group is="Bad">
              {
                "`it('returns 42', (t) => { const result = compute(); t.assert.snapshot(result); });`"
              }
            </Group>
            <Group is="WhyBad">
              {
                'Snapshot for a scalar — violates inherited `AX_SNAPSHOT_USAGE_GATE`. Failure shows diff of snapshot blobs instead of «42 ≠ 41».'
              }
            </Group>
            <Good>
              {'`assert.strictEqual(result, 42)` — direct, clear, no snapshot file overhead.'}
            </Good>
          </AntiPattern>
        </AntiPatternsBlock>
        <VerificationHooksBlock>
          <Hook id="HOOK_RUN_TESTS">
            <Group is="Command">{'npm test'}</Group>
            <Group is="Expected">
              {
                'Exit 0; all cases pass; no `.skip` / `.todo` without explicit deferred-ownership in the ticket.'
              }
            </Group>
          </Hook>
          <Hook id="HOOK_RUN_SINGLE_FILE">
            <Group is="Command">{'node --test path/to/subject.test.ts'}</Group>
            <Group is="Expected">{'Exit 0; targeted file passes.'}</Group>
          </Hook>
          <Hook id="HOOK_COVERAGE">
            <Group is="Command">{'node --test --experimental-test-coverage'}</Group>
            <Group is="Expected">
              {
                'Exit 0; report shows public methods with ≥1 happy + ≥1 boundary (when applicable) + ≥1 failure-path per inherited `AX_COVERAGE_BY_CONTRACT_NOT_BY_LINE`. Coverage cannot run → declare blocker EXPLICITLY.'
              }
            </Group>
          </Hook>
          <Hook id="HOOK_TYPECHECK_BEFORE_TEST">
            <Group is="Command">{'npx tsc --noEmit'}</Group>
            <Group is="Expected">{'Exit 0; no TS errors.'}</Group>
          </Hook>
          <Hook id="HOOK_NO_FORBIDDEN_TEST_PATTERNS">
            <Group is="Purpose">
              {
                'Smoke-grep for forbidden idioms: human narration `Step N`, `includes(` on error message, manual try-catch flag.'
              }
            </Group>
            <Group is="Command">
              {
                'rg --no-heading -n "Step \\d|\\.message.*\\.includes\\(|let\\s+threw\\s*=" -t ts --glob \'**/*.test.ts\''
              }
            </Group>
            <Group is="Expected">{'Empty on new/changed test files.'}</Group>
          </Hook>
        </VerificationHooksBlock>
      </Group>
    </Prompt>
